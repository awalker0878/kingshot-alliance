...
{
    public function __construct(private TotpService $totp, private AuditRecorder $audit) {}
    ...
    public function begin(User $user): array
    {
        ...
        DB::transaction(function () use ($user, $secret): void {
            ...
            $locked->forceFill(['two_factor_secret' => $secret, 'two_factor_recovery_codes' => null, 'two_factor_confirmed_at' => null])->save();
            $this->audit->record(event: 'auth.mfa.enrollment_started', actor: $locked, subject: $locked);
        });

        return ['secret' => $secret, 'provisioning_uri' => $this->totp->provisioningUri((string) $user->email, $secret, (string) config('app.name'))];
    }
    ...
    public function confirm(User $user, string $code): array
    {
        return DB::transaction(function () use ($user, $code): array {
            ...
            $locked->forceFill(['two_factor_recovery_codes' => array_map(static fn (string $recoveryCode): string => hash('sha256', $recoveryCode), $plainCodes), 'two_factor_confirmed_at' => now()])->save();
    ...
    public function regenerateRecoveryCodes(User $user): array
    {
        return DB::transaction(function () use ($user): array {
            ...
            $plainCodes = $this->generateRecoveryCodes();
            $locked->forceFill(['two_factor_recovery_codes' => array_map(static fn (string $recoveryCode): string => hash('sha256', $recoveryCode), $plainCodes)])->save();
    ...
    public function disable(User $user): void
    {
        DB::transaction(function () use ($user): void {
            $locked = User::query()->lockForUpdate()->findOrFail($user->id);
            $locked->forceFill(['two_factor_secret' => null, 'two_factor_recovery_codes' => null, 'two_factor_confirmed_at' => null])->save();
    ...
    {
        OutboxMessage::query()->create(['alliance_id' => null, 'event_type' => $eventType, 'aggregate_type' => User::class, 'aggregate_id' => (string) $user->id, 'idempotency_key' => $eventType.':'.$user->id.':'.now()->format('Uu'), 'payload' => ['user_id' => $user->id], 'occurred_at' => now(), 'available_at' => now(), 'attempts' => 0]);
    }
}
