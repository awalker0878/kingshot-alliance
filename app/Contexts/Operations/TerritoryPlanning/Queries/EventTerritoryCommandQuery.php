...
final readonly class EventTerritoryCommandQuery
{
    ...
    public function forOccurrence(string $actorPlayerId, EventOccurrence $occurrence): array
    {
        ...
        $revisions = TerritoryPlanRevision::query()->whereIn('id', $revisionIds)->get()->keyBy('id');
        $planIds = array_values(array_unique(array_map(
            static fn (array $reference): string => (string) $reference['planId'],
            $references,
        )));
        $plans = TerritoryPlan::query()->whereIn('id', $planIds)->get()->keyBy('id');
        $currentDraftDiffers = false;
        $violationCount = 0;
        $warningCount = 0;
        ...
        foreach ($references as $reference) {
            ...
            foreach (is_array($snapshot['objects'] ?? null) ? $snapshot['objects'] : [] as $object) {
                ...
                $x = $object['x'] ?? null;
                $y = $object['y'] ?? null;
                if (! is_string($key) || ! is_string($type) || ! is_string($allianceKey) || ! is_numeric($x) || ! is_numeric($y)) {
                    continue;
                }
                $objects[] = [
                    'key' => $key,
            ...
            $preferences = is_array($snapshotPlan['planning_preferences'] ?? null)
                ...
                : [];
            $dataset = $this->datasets->require((string) $revision->map_dataset_id, (string) $revision->map_dataset_checksum);
            $analysis = $this->analyzer->analyze($dataset, $objects, $preferences);
            foreach (is_array($analysis['alliances'] ?? null) ? $analysis['alliances'] : [] as $allianceAnalysis) {
