...
final class TerritoryPlanController extends Controller
{
    ...
    {
        $player = $players->playerOrNull(); abort_unless($player !== null, 403);
        $data = $request->validate(['scope' => ['required','in:alliance,kingdom'], 'kingdom_id' => ['required','ulid'], 'owner_alliance_id' => ['nullable','ulid'], 'name' => ['required','string','max:160'], 'map_dataset_id' => ['required','string','max:120']]);
    ...
    {
        $player = $players->playerOrNull(); abort_unless($player !== null, 403);
        $data = $request->validate(['expected_revision' => ['required','integer','min:1'], 'alliances' => ['required','array','min:1','max:50'], 'groups' => ['present','array','max:500'], 'objects' => ['present','array','max:5000'], 'planning_preferences' => ['sometimes','array']]);
        ...
        return response()->json(['receipt' => $this->receipt($receipt)]);
    }
    ...
    public function publish(Request $request, string $plan, PlayerContext $players, PublishTerritoryPlan $publish): JsonResponse
    {
        ...
        return response()->json(['receipt' => $this->receipt($publish->handle($player->playerId, $plan, (int) $data['expected_revision']))]);
    }
    ...
    public function archive(Request $request, string $plan, PlayerContext $players, ArchiveTerritoryPlan $archive): JsonResponse
    {
        $player = $players->playerOrNull(); abort_unless($player !== null, 403);
        $data = $request->validate(['expected_revision' => ['required','integer','min:1']]);
        return response()->json(['receipt' => $this->receipt($archive->handle($player->playerId, $plan, (int) $data['expected_revision']))]);
    }
    ...
    public function restore(Request $request, string $plan, string $revision, PlayerContext $players, RestoreTerritoryPlanRevision $restore): JsonResponse
    {
        $player = $players->playerOrNull(); abort_unless($player !== null, 403);
        $data = $request->validate(['expected_revision' => ['required','integer','min:1']]);
        return response()->json(['receipt' => $this->receipt($restore->handle($player->playerId, $plan, $revision, (int) $data['expected_revision']))]);
    }
    ...
    public function clone(Request $request, string $plan, PlayerContext $players, CloneTerritoryPlan $clone): JsonResponse
    {
        ...
        return response()->json(['receipt' => $this->receipt($clone->handle($player->playerId, $plan, $data['name']))]);
    }

    public function revision(string $plan, string $revision, PlayerContext $players, TerritoryPlanRevisionQuery $revisions): JsonResponse
    {
        $player = $players->playerOrNull(); abort_unless($player !== null, 403);
        return response()->json(['revision' => $revisions->snapshot($player->playerId, $plan, $revision)]);
    }

    public function attachEvent(Request $request, PlayerContext $players, AttachTerritoryPlanRevisionToEvent $attach): JsonResponse
    {
        $player = $players->playerOrNull(); abort_unless($player !== null, 403);
        $data = $request->validate(['event_occurrence_id' => ['required','ulid'], 'territory_plan_revision_id' => ['required','ulid'], 'purpose' => ['sometimes','string','max:40']]);
        return response()->json(['link_id' => $attach->handle($player->playerId, $data['event_occurrence_id'], $data['territory_plan_revision_id'], $data['purpose'] ?? 'positioning')]);
    }
    ...
    {
        $data = $request->validate(['style' => ['required','in:swirl,banner_pad'], 'alliance_key' => ['required','string','max:120'], 'center_x' => ['required','integer','between:0,1199'], 'center_y' => ['required','integer','between:0,1199'], 'city_count' => ['sometimes','integer','between:1,100']]);
    ...
    private function receipt(object $receipt): array
    {
        return ['plan_id' => $receipt->planId, 'revision' => $receipt->revision, 'status' => $receipt->status, 'published_revision_id' => $receipt->publishedRevisionId];
    }
}
