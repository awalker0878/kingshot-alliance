...
final class BearHuntResultSnapshotQuery
{
     ...
     * @return array<string, array{score: int, rank: int|null}>
     */
    public function players(string $occurrenceId, array $playerIds): array
    {
        $ids = array_values(array_unique(array_filter(
            array_map('trim', $playerIds),
        ...
        $rows = [];
        foreach (EventPlayerResult::query()
            ->where('occurrence_id', $occurrenceId)
            ->whereIn('player_id', $ids)
            ->get() as $result) {
            $rows[(string) $result->player_id] = [
    ...
    {
        return BearHuntBattleReport::query()
            ->where('occurrence_id', $occurrenceId)
