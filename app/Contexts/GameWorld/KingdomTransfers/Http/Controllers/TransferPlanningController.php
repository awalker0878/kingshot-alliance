...
final class TransferPlanningController extends Controller
{
    ...
    {
        /** @var array{official_label:string,kingdom_numbers:list<int>,source_type:string,source_reference:string,observed_at:string,evidence_id?:string|null} $v */
        $v = $request->validate([
            'official_label' => ['required', 'string', 'max:160'],
            'kingdom_numbers' => ['required', 'array', 'min:1', 'max:500'],
            'kingdom_numbers.*' => ['integer', 'min:1'],
    ...
    {
        /** @var array{kingdom_number:int,power_cap?:int|null,classification:string,hero_generation?:int|null,truegold_level?:int|null,character_age_threshold_days?:int|null,source_type:string,source_reference:string,observed_at:string,is_correction?:bool,evidence_id?:string|null} $v */
        $v = $request->validate([
            'kingdom_number' => ['required', 'integer', 'min:1'],
            'power_cap' => ['nullable', 'integer', 'min:0'],
            'classification' => ['required', Rule::in(array_column(TransferKingdomClassification::cases(), 'value'))],
            'hero_generation' => ['nullable', 'integer', 'min:1'],
            'truegold_level' => ['nullable', 'integer', 'min:0'],
            'character_age_threshold_days' => ['nullable', 'integer', 'between:90,180'],
            'source_type' => ['required', Rule::in(array_column(TransferSourceType::cases(), 'value'))],
            'source_reference' => ['required', 'string', 'max:2048'],
            'observed_at' => ['required', 'date'],
    ...
    {
        /** @var array{kind:string,source_type:string,source_reference:string,observed_at:string,valid_until?:string|null,details?:string|null,evidence_id?:string|null} $base */
        $base = $request->validate([
            'kind' => ['required', Rule::in(array_column(TransferObservationKind::cases(), 'value'))],
            'source_type' => ['required', Rule::in(array_column(TransferSourceType::cases(), 'value'))],
            'source_reference' => ['required', 'string', 'max:2048'],
            'observed_at' => ['required', 'date'],
        ...
        if ($kind->usesNumericValue()) {
            /** @var array{value:int} $valueRow */
            $valueRow = $request->validate(['value' => ['required', 'integer', 'min:0']]);
            $value = $valueRow['value'];
        } elseif ($kind->usesBooleanValue()) {
            $request->validate(['value' => ['required', 'boolean']]);
            $value = $request->boolean('value');
        } else {
            /** @var array{value:string} $valueRow */
            $valueRow = $request->validate(['value' => ['required', Rule::in(array_column(TransferInvitationStatus::cases(), 'value'))]]);
            $value = $valueRow['value'];
        }
        $s = $context->scope();
    ...
    {
        /** @var array{label:string,pre_transfer_starts_at:string,invitational_starts_at:string,transfer_opens_at:string,ends_at:string,source_type:string,source_reference:string,observed_at:string,evidence_id?:string|null} $v */
        $v = $request->validate([
            'label' => ['required', 'string', 'max:160'],
            'pre_transfer_starts_at' => ['required', 'date'],
            'invitational_starts_at' => ['required', 'date'],
            'transfer_opens_at' => ['required', 'date'],
