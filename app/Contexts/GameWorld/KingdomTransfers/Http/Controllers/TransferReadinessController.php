...
final class TransferReadinessController extends Controller
{
    ...
    {
        $scope=$context->scope();$account=$this->account($request,$accounts);$alliance=$alliances->require($scope->allianceId);$kingdom=$kingdoms->require($alliance->kingdomId);if(!$authorization->allows($scope->playerId,$scope->allianceId,TransferPermission::Manage))throw new AuthorizationException;$plan=$plans->currentForAlliance($scope->allianceId);$rows=$plan===null?collect():$participants->forPlan($scope->allianceId,(string)$plan->id,true);$planning=$plan===null?[]:$eligibility->forPlan($scope->allianceId,$plan,$rows);
