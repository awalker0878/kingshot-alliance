...
final class TransferEligibilityEvaluator
{
    ...
    private function passes(TransferEligibilityInput $input): TransferRequirement
    {
        if ($input->passesRequired->state !== TransferRequirementState::Met) {
            return $this->fromObserved(TransferRequirementKey::TransferPasses, $input->passesRequired, 'Observe the required Transfer Pass count in-game; the public formula is not published.');
        }
    ...
    private function hasHardBlocker(array $requirements): bool
    {
        foreach ($requirements as $row) {
            ...
            if ($row->key === TransferRequirementKey::TransferGroup) {
                return true;
            }
