    ...
    public function handle(string $allianceId, string $actorPlayerId, string $planId, string $participantId, TransferObservationKind $kind, int|string|bool $value, TransferSourceType $sourceType, string $sourceReference, string $observedAt, ?string $validUntil, ?string $details = null, ?string $evidenceId = null): string
    {
        return DB::transaction(function () use ($allianceId, $actorPlayerId, $planId, $participantId, $kind, $value, $sourceType, $sourceReference, $observedAt, $validUntil, $details, $evidenceId): string {
            ...
            if ($participant->withdrawn_at !== null) {
                throw ValidationException::withMessages(['participant' => 'Withdrawn participants cannot receive current transfer observations.']);
            }
            ...
            if ($kind->requiresTargetKingdom() && $targetId === null) {
                throw ValidationException::withMessages(['target' => 'Set a target Kingdom before recording this observation.']);
            }
            ...
            if ($kind === TransferObservationKind::InGameRulesVerified && ! is_bool($value)) {
                throw ValidationException::withMessages(['value' => 'In-game rule verification requires a boolean value.']);
            }
            if ($kind === TransferObservationKind::InvitationStatus && (! is_string($value) || TransferInvitationStatus::tryFrom($value) === null)) {
                throw ValidationException::withMessages(['value' => 'Invitation status is invalid.']);
            }
