...
final class TransferCohortQuery
{
    ...
    {
        $query=TransferCohort::query()->where('alliance_id',$allianceId)->where('transfer_plan_id',$planId)->with(['coordinator:id,current_name','destinationKingdom:id,number']);if(!$includeArchived)$query->where('state',TransferCohortState::Active->value);return $query->orderByRaw("case state when 'active' then 0 else 1 end")->orderBy('name')->orderBy('id')->get();
    }
}
