...
final class OfficialXGiftCodeSourceAdapter implements GiftCodeIncrementalPaginatedSourceAdapter
{
    use HandlesGiftCodeProviderResponses;

    public const KEY = 'x-api-v2-kingshot-v2';
    ...
    {
        return $this->acquireIncremental($source, $cursor, null, $limit);
    }
    ...
    public function acquireIncremental(
        ...
        $sinceId = $this->optionalCursor($sinceId, '/^[0-9]{1,32}$/D', 32, 'The X API since id must be a post id high-water mark.');
        $pageToken = $this->optionalCursor($pageToken, '/^[A-Za-z0-9._~-]{1,2048}$/D', 2048, 'The X API pagination token is invalid.');
        ...
        foreach ($posts as $position => $post) {
            ...
            $postIds[] = $postId;
            $candidateHighWater = $this->greaterNumericId($candidateHighWater, $postId);
            ...
            foreach ($this->extractExplicitCodes($text) as $code) {
                ...
                $observations[] = new GiftCodeIngestionObservation(
                    ...
                    contentFingerprint: hash('sha256', implode('|', [$postId, $code, $text])),
        ...
        $nextToken = is_array($meta) ? $this->optionalString($meta['next_token'] ?? null, 2048) : null;
