...
final readonly class QueueGiftCodeTransitionNotifications
{
    ...
    public function handle(int $limit = 200): GiftCodeNotificationSweep
    {
        ...
        $campaign = GiftCodeNotificationCampaign::query()
        ...
        $currentRevision = $campaign->status_revision === $giftCode->status_revision
            && $campaign->expires_revision === $giftCode->expires_revision;
        $currentlyEligible = match ($campaign->notification_type) {
            ScheduleGiftCodeNotificationCampaign::AVAILABLE => $giftCode->status === GiftCodeStatus::Valid
                && ($giftCode->expires_at === null || $giftCode->expires_at->isFuture()),
        ...
        foreach ($userIds as $userId) {
            ...
            $title = $campaign->notification_type === ScheduleGiftCodeNotificationCampaign::AVAILABLE
                ? 'Gift Code available'
                : 'Gift Code trust changed';
            ...
            $batch = $this->deliveries->queueEnabledAccountChannelBatch(
                ...
                idempotencyKey: implode(':', [
                    ...
                    (string) $giftCode->id,
                    (string) $userId,
                    (string) $campaign->status_revision,
                    (string) $campaign->expires_revision,
                ]),
                ...
                metadata: [
                    'title' => $title,
                    'body' => $body,
                    'action_url' => '/gift-codes/'.$giftCode->id,
        ...
        $result = $this->result(
            ...
            $created,
            $skipped,
            $truncated ? (string) $lastUserId : null,
            $truncated,
        );
        Log::info('gift_codes.transition_notification_sweep', [
            ...$result->toArray(),
    ...
    {
        $governors = $this->players->ownedByUserUpTo(
            $userId,
            max(1, min(50, (int) config('game_world.gift_codes.max_governors_per_account', 50))),
        );
        if ($governors === []) {
            return [];
        }
        ...
        $redemptions = GiftCodeRedemption::query()
            ...
            ->whereIn('player_id', array_map(static fn (PlayerReference $player): string => $player->playerId, $governors))
            ->get()
            ->keyBy('player_id');
        ...
        return array_values(array_filter($governors, static function (PlayerReference $player) use ($redemptions, $notificationType): bool {
            ...
            return $player->gamePlayerId !== null
    ...
    ): GiftCodeNotificationSweep {
        return new GiftCodeNotificationSweep(
            $examined,
            $eligible,
            $deliveryCount,
            $created,
