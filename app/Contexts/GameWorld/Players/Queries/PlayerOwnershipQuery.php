...
final class PlayerOwnershipQuery
{
    ...
    {
        return Player::query()->where('user_id', $user->id)->with('currentKingdom')->orderBy('id')->get();
    }

    /** @return Collection<int, Player> */
    public function upTo(User $user, int $limit): Collection
    {
        return Player::query()->where('user_id', $user->id)->with('currentKingdom')->orderBy('id')->limit(max(1, $limit))->get();
    }

    public function find(User $user, string $playerId): ?Player
    {
        return Player::query()->whereKey($playerId)->where('user_id', $user->id)->with('currentKingdom')->first();
    }

    public function countFor(User $user): int
    {
        return Player::query()->where('user_id', $user->id)->count();
    }
