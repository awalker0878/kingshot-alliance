...
final readonly class RecordSpatialObservationEvidence
{
    ...
    public function handle(
        ...
        if ($schemaVersion !== self::SCHEMA_VERSION) {
            throw ValidationException::withMessages(['schema_version' => 'The reviewed Territory screenshot schema version is not supported.']);
        }
        if (preg_match('/^[a-f0-9]{64}$/', $idempotencyKey) !== 1) {
            throw ValidationException::withMessages(['idempotency_key' => 'The spatial-observation destination idempotency key is invalid.']);
        }
        try {
            $captured = CarbonImmutable::parse($capturedAt)->utc();
        } catch (Throwable) {
            throw ValidationException::withMessages(['captured_at' => 'The observation capture time is invalid.']);
        }
        if ($captured->isAfter(now()->addMinutes(5))) {
            throw ValidationException::withMessages(['captured_at' => 'The observation capture time cannot be more than five minutes in the future.']);
        }
        ...
        return DB::transaction(function () use ($actorPlayerId, $allianceId, $kingdomId, $evidenceId, $reviewId, $schemaVersion, $mapDatasetId, $mapDatasetChecksum, $captured, $coverageKind, $completeness, $coverageBounds, $objects, $idempotencyKey, $correctsObservationId, $correctionReason): SpatialObservationRecordResult {
            ...
            if ($scope->kingdomId !== $kingdomId) {
                throw ValidationException::withMessages(['kingdom_id' => 'The observed Kingdom no longer matches the active Alliance scope.']);
            }
            ...
            if ($existing instanceof SpatialObservationEvidenceReceipt) {
                if ((string) $existing->alliance_id !== $allianceId
                    ...
                    throw ValidationException::withMessages(['idempotency_key' => 'The destination idempotency key is already bound to different spatial evidence.']);
                }
            ...
            if (! $this->evidence->isApprovedSpatialReview(
                ...
                throw ValidationException::withMessages(['evidence' => 'The exact approved Territory observation review is no longer valid for this destination scope.']);
            }

            $dataset = $this->datasets->require($mapDatasetId, $mapDatasetChecksum);
            $bounds = $this->validateCoverage($dataset->data, $coverageKind, $completeness, $coverageBounds, count($objects));
            ...
            if ($correctsObservationId !== null) {
                ...
                if (mb_strlen($reason) < 8 || mb_strlen($reason) > 1000) {
                    throw ValidationException::withMessages(['correction_reason' => 'A correction reason between 8 and 1,000 characters is required.']);
                }
                ...
                if ($corrects->invalidated_at !== null) {
                    throw ValidationException::withMessages(['corrects_observation_id' => 'Only a current accepted observation may be corrected.']);
                }
            }
    ...
    private function validateCoverage(
        ...
        if ($kind === SpatialObservationCoverageKind::CompleteHive && $completeness !== SpatialObservationCompleteness::Complete) {
            throw ValidationException::withMessages(['completeness' => 'A complete-hive observation must be explicitly complete.']);
        }
        if ($kind === SpatialObservationCoverageKind::CompleteVisibleRegion && $completeness !== SpatialObservationCompleteness::Complete) {
            throw ValidationException::withMessages(['completeness' => 'A complete visible-region observation must be explicitly complete.']);
        }
        if ($kind === SpatialObservationCoverageKind::SingleObject && $objectCount !== 1) {
            throw ValidationException::withMessages(['objects' => 'A single-object observation must contain exactly one reviewed object.']);
        }
        if ($kind === SpatialObservationCoverageKind::CompleteVisibleRegion && $bounds === null) {
            throw ValidationException::withMessages(['coverage_bounds' => 'Complete visible-region observations require explicit visible bounds.']);
        }
        if ($bounds === null) {
            return null;
        }
        foreach (['x', 'y', 'width', 'height'] as $field) {
            if (! isset($bounds[$field]) || ! is_int($bounds[$field])) {
                throw ValidationException::withMessages(['coverage_bounds' => 'Coverage bounds require integer x, y, width and height values.']);
            }
        }
        if ($bounds['width'] < 1 || $bounds['height'] < 1) {
            throw ValidationException::withMessages(['coverage_bounds' => 'Coverage bounds must have positive width and height.']);
        }
        $map = $dataset['bounds'] ?? null;
        if (! is_array($map)) {
            throw ValidationException::withMessages(['map_dataset_id' => 'The pinned map dataset does not expose coordinate bounds.']);
        }
        $mapX = (int) ($map['x'] ?? 0);
        $mapY = (int) ($map['y'] ?? 0);
        $mapWidth = (int) ($map['width'] ?? 0);
        $mapHeight = (int) ($map['height'] ?? 0);
        if ($bounds['x'] < $mapX || $bounds['y'] < $mapY
            || $bounds['x'] + $bounds['width'] > $mapX + $mapWidth
            || $bounds['y'] + $bounds['height'] > $mapY + $mapHeight) {
            throw ValidationException::withMessages(['coverage_bounds' => 'Coverage bounds must stay inside the pinned Kingdom map.']);
        }
    ...
    private function validateObjects(array $dataset, string $kingdomId, array $objects): array
    {
        if (count($objects) > 5000) {
            throw ValidationException::withMessages(['objects' => 'A spatial observation may contain at most 5,000 reviewed objects.']);
        }
        ...
        foreach ($objects as $index => $object) {
            ...
            if ($key === '' || mb_strlen($key) > 120 || isset($seen[$key])) {
                throw ValidationException::withMessages(["objects.{$index}.key" => 'Observed object keys must be unique and no longer than 120 characters.']);
            }
            $seen[$key] = true;
            $type = SpatialObservedObjectType::tryFrom((string) ($object['type'] ?? ''));
            if (! $type instanceof SpatialObservedObjectType) {
                throw ValidationException::withMessages(["objects.{$index}.type" => 'The observed object type is unsupported.']);
            }
            if (! is_int($object['x'] ?? null) || ! is_int($object['y'] ?? null)) {
                throw ValidationException::withMessages(["objects.{$index}" => 'Observed coordinates must be whole-number game coordinates.']);
            }
            $definition = is_array($definitions[$type->value] ?? null) ? $definitions[$type->value] : [];
            $size = max(1, (int) ($definition['size'] ?? 1));
            $x = $object['x'];
            $y = $object['y'];
            if ($x < $minX || $y < $minY || $x + $size > $maxX || $y + $size > $maxY) {
                throw ValidationException::withMessages(["objects.{$index}" => 'An observed object lies outside the pinned Kingdom map bounds.']);
            }
            ...
            if (! $identity instanceof SpatialObservedIdentityState) {
                throw ValidationException::withMessages(["objects.{$index}.identity_state" => 'The observed identity state is unsupported.']);
            }
            $playerId = is_string($object['player_id'] ?? null) && trim($object['player_id']) !== '' ? trim($object['player_id']) : null;
            $planLocal = is_string($object['plan_local_identity'] ?? null) && trim($object['plan_local_identity']) !== '' ? trim($object['plan_local_identity']) : null;
            if ($type !== SpatialObservedObjectType::GovernorCity) {
                if ($playerId !== null || $planLocal !== null || $identity !== SpatialObservedIdentityState::Unresolved) {
                    throw ValidationException::withMessages(["objects.{$index}.identity_state" => 'Only Governor-city observations may carry Governor identity mapping.']);
                }
            } elseif ($identity === SpatialObservedIdentityState::ResolvedPlayer) {
                if ($playerId === null || $planLocal !== null) {
                    throw ValidationException::withMessages(["objects.{$index}.player_id" => 'Resolved Player identity requires exactly one Player reference.']);
                }
                ...
                if ($planLocal === null || $playerId !== null || mb_strlen($planLocal) > 191) {
                    throw ValidationException::withMessages(["objects.{$index}.plan_local_identity" => 'Resolved plan-local identity requires exactly one reviewed plan-local identity.']);
                }
            } elseif ($playerId !== null || $planLocal !== null) {
                throw ValidationException::withMessages(["objects.{$index}.identity_state" => 'Ambiguous or unresolved identity cannot retain a definitive Player/plan-local mapping.']);
            }
        }
        ...
        foreach (array_keys($playerIds) as $playerId) {
            ...
            if ($reference === null || $reference->kingdomId !== $kingdomId) {
                throw ValidationException::withMessages(['objects' => 'A resolved Governor identity must reference a current Player in the observed Kingdom.']);
            }
        }
        ...
        foreach ($objects as $object) {
            ...
            if ($confidence !== null && ($confidence < 0 || $confidence > 1)) {
                throw ValidationException::withMessages(['objects' => 'Observed-object confidence must be between 0 and 1.']);
            }
            $validated[] = [
                'key' => trim((string) $object['key']),
                ...
                'identity_state' => SpatialObservedIdentityState::from((string) ($object['identity_state'] ?? SpatialObservedIdentityState::Unresolved->value)),
