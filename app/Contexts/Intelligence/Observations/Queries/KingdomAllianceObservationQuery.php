...
final class KingdomAllianceObservationQuery
{
    ...
    public function latestAccepted(string $allianceId, string $trackingId): ?KingdomAllianceObservation
    {
    ...
    public function history(string $allianceId, string $trackingId, bool $includeInvalidated): Collection
    {
        return KingdomAllianceObservation::query()
            ->where('alliance_id', $allianceId)
            ->where('tracked_kingdom_alliance_id', $trackingId)
