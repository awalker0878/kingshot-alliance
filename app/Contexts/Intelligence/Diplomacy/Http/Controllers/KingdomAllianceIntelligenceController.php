...
{
    public function index(Request $request, AllianceContext $context, AllianceIntelligenceAuthorization $intelligenceAuthorization, KingdomAllianceIntelligence $intelligence): Response
    {
    ...
    {
        $diplomacyStates = array_map(static fn (KingdomAllianceDiplomacyState $state): string => $state->value, KingdomAllianceDiplomacyState::cases());
        ...
        return ['tracking'=>$validated['tracking']??'active','freshness'=>$validated['freshness']??'all','diplomacy'=>$validated['diplomacy']??'all','sort'=>$validated['sort']??'name','direction'=>$validated['direction']??'asc'];
    }

    private function allianceSummary(Alliance $alliance): array
    {
        return ['id'=>(string)$alliance->id,'name'=>(string)$alliance->name,'kingdom'=>$alliance->kingdom===null?null:(string)$alliance->kingdom->number];
    }
