...
final readonly class DeleteTerritorySpatialEvidence
{
    ...
    public function handle(string $actorPlayerId, string $allianceId, string $kingdomId, string $evidenceId, string $reason = 'user_requested'): void
    {
        $storage = DB::transaction(function () use ($actorPlayerId, $allianceId, $kingdomId, $evidenceId, $reason): ?array {
            ...
            if ($evidence->expected_kind !== EvidenceKind::TerritoryMapObservation) {
                throw ValidationException::withMessages(['evidence' => 'The selected Evidence is not Territory spatial evidence.']);
            }
            if ($evidence->lifecycle_status === EvidenceLifecycleStatus::Deleted) {
                return null;
            }
            $storage = $evidence->path === null ? null : ['disk' => (string) $evidence->disk, 'path' => (string) $evidence->path];
            $evidence->forceFill([
            ...
            $this->audit->record('evidence.territory_spatial_deleted', $actor, $evidence, $allianceId, ['evidence_id' => $evidenceId, 'kingdom_id' => $kingdomId]);
            return $storage;
        });
        if (is_array($storage)) {
