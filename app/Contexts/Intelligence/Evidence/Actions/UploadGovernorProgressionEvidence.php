...
final readonly class UploadGovernorProgressionEvidence
{
    ...
    public function handle(
        ...
        if (! $expectedKind->isGovernorProgression()) {
            throw ValidationException::withMessages(['evidence_kind' => 'Select a supported Governor Progression screenshot class.']);
        }
        ...
        if (! $scan->clean) {
            $this->audit->record('evidence.governor_progression_upload_rejected', $actor, null, $allianceId, [
        ...
        $existing = $this->exactDuplicate($allianceId, $rosterEntryId, $expectedKind, $sha256);
        if ($existing instanceof GameEvidence) {
            $this->audit->record('evidence.exact_duplicate_detected', $actor, $existing, $allianceId, [
                'evidence_kind' => $expectedKind->value,
        ...
        $path = 'evidence/'.$allianceId.'/governor/'.$entry->playerId.'/'.Str::ulid().'.'.$extension;
        ...
        try {
            $result = DB::transaction(function () use ($actorPlayerId, $allianceId, $rosterEntryId, $expectedKind, $file, $disk, $path, $mimeType, $size, $sha256, $imageInfo, $perceptualHash, $visualDuplicateId, $visualDistance): EvidenceUploadResult {
                [, $actor] = $this->writeState->authorize($actorPlayerId, $allianceId, IntelligencePermission::KingdomManage);
                $entry = $this->roster->requireActiveOrTracked($allianceId, $rosterEntryId);
                $duplicate = $this->exactDuplicate($allianceId, $rosterEntryId, $expectedKind, $sha256, true);
                ...
                $this->audit->record('evidence.governor_progression_uploaded', $actor, $evidence, $allianceId, $metadata);
                $this->outbox->record('evidence.governor_progression_uploaded', $allianceId, $evidence, $metadata);
                if ($visualDuplicateId !== null) {
                    $this->audit->record('evidence.visual_duplicate_detected', $actor, $evidence, $allianceId, $metadata);
                }
