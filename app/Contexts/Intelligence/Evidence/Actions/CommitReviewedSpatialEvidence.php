...
final readonly class CommitReviewedSpatialEvidence
{
    ...
    public function handle(string $actorPlayerId, string $allianceId, string $kingdomId, string $reviewId): SpatialObservationRecordResult
    {
        $prepared = DB::transaction(function () use ($actorPlayerId, $allianceId, $kingdomId, $reviewId): array {
            ...
            if ($review->status !== EvidenceReviewStatus::Approved) {
                throw ValidationException::withMessages(['review' => 'Only an approved Territory spatial review can be committed.']);
            }
            ...
            if ($evidence->kind !== EvidenceKind::TerritoryMapObservation
                ...
                throw ValidationException::withMessages(['evidence' => 'The approved Territory screenshot provenance is no longer valid.']);
            }
            $idempotencyKey = hash('sha256', implode(':', ['territory-spatial', (string) $review->id, (string) $review->semantic_fingerprint, (string) $review->map_dataset_checksum]));
            $attempt = SpatialEvidenceCommitAttempt::query()
                ->where('spatial_review_id', $review->id)
                ->where('idempotency_key', $idempotencyKey)
                ->lockForUpdate()
                ->first();
            if ($attempt instanceof SpatialEvidenceCommitAttempt && $attempt->status === EvidenceCommitStatus::Succeeded && $attempt->destination_receipt_id !== null && is_array($attempt->destination_receipt)) {
            ...
            if (! $attempt instanceof SpatialEvidenceCommitAttempt) {
                ...
                $attempt->forceFill(['status' => EvidenceCommitStatus::Pending, 'failure_code' => null, 'started_by_player_id' => $actorPlayerId, 'started_at' => now(), 'completed_at' => null])->save();
            }
            $evidence->forceFill(['lifecycle_status' => EvidenceLifecycleStatus::Committing])->save();
            $this->audit->record('evidence.territory_spatial_commit_started', $actor, $evidence, $allianceId, ['review_id' => $reviewId, 'commit_attempt_id' => (string) $attempt->id]);
            return [
        ...
        try {
            ...
            DB::transaction(function () use ($actorPlayerId, $allianceId, $prepared, $receipt): void {
                ...
                $attempt = SpatialEvidenceCommitAttempt::query()->whereKey($prepared['attempt_id'])->lockForUpdate()->firstOrFail();
                ...
                $attempt->forceFill([
                    ...
                    'destination_receipt' => ['receipt_id' => $receipt->receiptId, 'observation_id' => $receipt->observationId, 'idempotent_replay' => $receipt->idempotentReplay],
                    'failure_code' => null,
                    'completed_at' => now(),
                ])->save();
                $evidence->forceFill(['lifecycle_status' => EvidenceLifecycleStatus::Committed])->save();
                $event = $receipt->idempotentReplay ? 'evidence.territory_spatial_commit_recovered' : 'evidence.territory_spatial_committed';
                $metadata = ['review_id' => (string) $prepared['review_id'], 'observation_id' => $receipt->observationId, 'receipt_id' => $receipt->receiptId];
            ...
            try {
                DB::transaction(function () use ($actorPlayerId, $allianceId, $prepared, $exception): void {
                    [, $actor] = $this->writeState->authorize($actorPlayerId, $allianceId, IntelligencePermission::KingdomManage);
                    $attempt = SpatialEvidenceCommitAttempt::query()->whereKey($prepared['attempt_id'])->lockForUpdate()->first();
                    $evidence = GameEvidence::query()->whereKey($prepared['evidence_id'])->lockForUpdate()->first();
                    $failureCode = substr(hash('sha256', $exception::class.':'.$exception->getMessage()), 0, 24);
                    if ($attempt instanceof SpatialEvidenceCommitAttempt && $attempt->status !== EvidenceCommitStatus::Succeeded) {
                        $attempt->forceFill(['status' => EvidenceCommitStatus::Failed, 'failure_code' => $failureCode, 'completed_at' => now()])->save();
                    }
                    if ($evidence instanceof GameEvidence && $evidence->lifecycle_status !== EvidenceLifecycleStatus::Committed) {
                        ...
                        $this->audit->record('evidence.territory_spatial_commit_failed', $actor, $evidence, $allianceId, ['failure_code' => $failureCode]);
                    }
                });
            } catch (Throwable) {
            }
