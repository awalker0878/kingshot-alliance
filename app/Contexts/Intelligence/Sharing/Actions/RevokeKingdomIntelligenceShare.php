...
final readonly class RevokeKingdomIntelligenceShare
{
    ...
    public function handle(string $sourceAllianceId,string $actorPlayerId,string $shareId): string
    {
        return DB::transaction(function() use($sourceAllianceId,$actorPlayerId,$shareId): string {
            ...
            if(in_array($share->state,[KingdomIntelligenceShareState::Revoked,KingdomIntelligenceShareState::Declined],true)){return (string) $share->id;}
            if(!in_array($share->state,[KingdomIntelligenceShareState::Pending,KingdomIntelligenceShareState::Active],true)){throw ValidationException::withMessages(['sharing'=>'Only pending or active sharing agreements can be revoked.']);}
