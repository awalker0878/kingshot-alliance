...
final readonly class AddKingdomIntelligenceShareTarget
{
    ...
    public function handle(string $sourceAllianceId, string $actorPlayerId, string $shareId, string $trackingId): string
    {
        return DB::transaction(function () use ($sourceAllianceId, $actorPlayerId, $shareId, $trackingId): string {
            ...
            if ($share->recipient_alliance_id === null) {
                throw ValidationException::withMessages(['sharing' => 'Only an active sharing agreement with a recipient can receive shared targets.']);
            }
            $recipientId = (string) $share->recipient_alliance_id;
            $recipient = $this->alliances->lockCurrent($recipientId);
            if (! $recipient->active() || $scope->kingdomId !== (string) $share->kingdom_id || $recipient->kingdomId !== (string) $share->kingdom_id) {
                throw ValidationException::withMessages(['sharing' => 'Both alliances must be active in the captured Kingdom before a target can be shared.']);
            }
            $tracking = TrackedKingdomAlliance::query()->whereKey($trackingId)->where('alliance_id', $sourceAllianceId)->lockForUpdate()->firstOrFail();
            if ($tracking->state !== TrackedKingdomAllianceState::Active || (string) $tracking->kingdom_id !== (string) $share->kingdom_id) {
                throw ValidationException::withMessages(['tracking' => 'Only an actively tracked game alliance in the captured Kingdom can be shared.']);
            }
            ...
            if ($target instanceof KingdomIntelligenceShareTarget && $target->state === KingdomIntelligenceShareTargetState::Active) {
                return (string) $target->id;
            }
            $sharedAt = now();
            if ($target instanceof KingdomIntelligenceShareTarget) {
                $target->forceFill(['state' => KingdomIntelligenceShareTargetState::Active, 'shared_by_player_id' => $actor->playerId, 'removed_by_player_id' => null, 'shared_at' => $sharedAt, 'removed_at' => null])->save();
            } else {
                $target = KingdomIntelligenceShareTarget::query()->create(['kingdom_intelligence_share_id' => $share->id, 'tracked_kingdom_alliance_id' => $tracking->id, 'state' => KingdomIntelligenceShareTargetState::Active, 'shared_by_player_id' => $actor->playerId, 'shared_at' => $sharedAt]);
            }
    ...
    private function recordForAlliance(string $allianceId, ?PlayerReference $actor, KingdomIntelligenceShareTarget $target, array $metadata, \DateTimeInterface $occurredAt): void
    {
        $event = 'kingdoms.shared_intelligence_target_shared';
        $this->audit->record($event, $actor, $target, $allianceId, $metadata);
        $this->outbox->record($event, $allianceId, $target, $metadata, $event.':'.$target->id.':'.$allianceId.':'.$occurredAt->format('YmdHis.u'));
    }
}
