...
final class PlatformUsageService
{
    ...
    {
        return [
            'activeMembers' => AllianceMembership::query()->where('alliance_id', $alliance->id)->where('status', MembershipStatus::Active->value)->count(),
            'storageBytes' => (int) MediaAsset::query()->where('alliance_id', $alliance->id)->sum('size_bytes'),
            'activeApiCredentials' => ApiCredential::query()->where('alliance_id', $alliance->id)->whereNull('revoked_at')->where(static function ($query): void {$query->whereNull('expires_at')->orWhere('expires_at', '>', now());})->count(),
            'activeWebhookSubscriptions' => WebhookSubscription::query()->where('alliance_id', $alliance->id)->where('is_active', true)->whereNull('revoked_at')->count(),
            'pendingOutboxMessages' => OutboxMessage::query()->where('alliance_id', $alliance->id)->whereNull('published_at')->count(),
        ];
    }
    ...
    {
        $count = 0;
        Alliance::query()->orderBy('id')->limit($limit)->each(function (Alliance $alliance) use (&$count): void {$this->capture($alliance); $count++;});
        return $count;
    }
}
