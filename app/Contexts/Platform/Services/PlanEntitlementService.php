...
final class PlanEntitlementService
{
    ...
    {
        $planCode = DB::table('alliance_plan_assignments')
    ...
    {
        return [
            'members' => $this->limit($alliance, 'members.max'),
            'storageBytes' => $this->limit($alliance, 'storage.bytes.max'),
            'apiCredentials' => $this->limit($alliance, 'api_credentials.max'),
            'webhookSubscriptions' => $this->limit($alliance, 'webhook_subscriptions.max'),
        ];
    }
