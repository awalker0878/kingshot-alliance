...
{
    public function __construct(private InvitationTokenService $tokens, private AuditRecorder $audit, private MemberCapacityPolicy $capacity) {}

    public function handle(AllianceMutationContext $context, Player $target, string $email): IssuedInvitation
    {
        ...
        $invitation=Invitation::query()->create(['alliance_id'=>$context->alliance->id,'player_id'=>$lockedTarget->id,'email'=>$email,'token_hash'=>$this->tokens->hash($token),'status'=>InvitationStatus::Pending,'invited_by_player_id'=>$context->actor->id,'expires_at'=>now()->addHours($ttlHours)]);
        $this->audit->record('invitation.created',$context->actor,$invitation,$context->alliance,['player_id'=>(string)$lockedTarget->id,'email'=>$email]);
        OutboxMessage::query()->create(['alliance_id'=>$context->alliance->id,'partition_key'=>'alliance:'.$context->alliance->id,'event_type'=>'invitation.created','aggregate_type'=>Invitation::class,'aggregate_id'=>$invitation->id,'idempotency_key'=>'invitation.created:'.$invitation->id,'payload'=>['invitation_id'=>$invitation->id,'alliance_id'=>$context->alliance->id,'player_id'=>$lockedTarget->id,'email'=>$email],'occurred_at'=>now(),'available_at'=>now(),'attempts'=>0]);
