...
final readonly class RosterIntelligence
{
    ...
    public function forAlliance(string $allianceId, ?Carbon $asOf = null): array
    {
        $asOf ??= now();
        $entries = $this->roster->forAlliance($allianceId);
        $tracked = $entries
        ...
        $linkedPlayerIds = AllianceMembership::query()
            ...
            ->flip();
        $latest = $this->snapshots->latestForEntries($allianceId, $tracked);
        $sevenDayBaselines = $this->snapshots->baselinesForEntries($allianceId, $tracked, 7, $asOf);
        $thirtyDayBaselines = $this->snapshots->baselinesForEntries($allianceId, $tracked, 30, $asOf);
