...
final readonly class EventCommandCloseoutQuery
{
    ...
    /** @param list<string> $capabilities @return list<array<string,mixed>> */
    public function forOccurrence(PlayerReference $actor, Event $event, EventOccurrence $occurrence, array $capabilities): array
    {
        $sections = [];
    ...
    private function attendance(Event $event, EventOccurrence $occurrence): array
    {
        ...
        if ($summary === null) {
            return Items::section('attendance', 'events.command.sections.attendance', 'closeout', [
                $this->unknown($event, $occurrence, 'closeout.attendance_unavailable', 'operations.participation', 'events.command.items.attendanceUnavailable', 'participation'),
            ]);
        }
        ...
        return Items::section('attendance', 'events.command.sections.attendance', 'closeout', [
            Items::make(
                'closeout.attendance_missing', 'closeout',
                ...
                $missing > 0 ? 'events.command.items.attendanceMissing' : 'events.command.items.attendanceComplete',
                ['count' => $missing], $missing,
                handoff: Items::handoff($event, $occurrence, 'participation', 'events.command.actions.recordAttendance'),
            ),
        ]);
    }
    ...
    private function rallies(Event $event, EventOccurrence $occurrence): array
    {
        ...
        if ($summary === null) {
            return Items::section('rallies', 'events.command.sections.rallies', 'closeout', [
                $this->unknown($event, $occurrence, 'closeout.rallies_unavailable', 'operations.rallies', 'events.command.items.ralliesUnavailable', 'rallies'),
            ]);
        }
        if ((int) $summary['plannedAssignmentCount'] === 0) {
            return Items::section('rallies', 'events.command.sections.rallies', 'closeout', [
                Items::make(
                    'closeout.rallies_not_planned', 'closeout', Status::NotApplicable, Severity::Informational,
                    'operations.rallies', 'events.command.items.rallyActualsNotApplicable',
                ),
            ]);
        }
        ...
        return Items::section('rallies', 'events.command.sections.rallies', 'closeout', [
            Items::make(
                'closeout.rally_actuals_missing', 'closeout',
                $missing > 0 ? Status::NeedsAttention : Status::Complete,
                $missing > 0 ? Severity::Blocking : Severity::Informational,
                'operations.rallies',
                $missing > 0 ? 'events.command.items.rallyActualsMissing' : 'events.command.items.rallyActualsComplete',
                ['count' => $missing], $missing,
                handoff: Items::handoff($event, $occurrence, 'rallies', 'events.command.actions.recordRallyParticipation'),
            ),
        ]);
    }
    ...
    private function results(Event $event, EventOccurrence $occurrence): array
    {
        ...
        if ($summary === null) {
            return Items::section('results', 'events.command.sections.results', 'closeout', [
                $this->unknown($event, $occurrence, 'closeout.results_unavailable', 'operations.results', 'events.command.items.resultsUnavailable', 'results'),
            ]);
        }
        ...
        $items = [
            Items::make(
                'closeout.results_summary', 'closeout',
                $summaryExists ? Status::Complete : Status::NeedsAttention,
                $summaryExists ? Severity::Informational : Severity::Blocking,
                'operations.results',
                $summaryExists ? 'events.command.items.resultsComplete' : 'events.command.items.resultsMissing',
                handoff: Items::handoff($event, $occurrence, 'results', 'events.command.actions.recordResults'),
            ),
        ];
        $missingPlayerResults = (int) $summary['missingPlayerResultCount'];
        if ($summaryExists && $missingPlayerResults > 0) {
            $items[] = Items::make(
                'closeout.player_results_missing', 'closeout', Status::Warning, Severity::Warning,
                'operations.results', 'events.command.items.playerResultsMissing',
                ['count' => $missingPlayerResults], $missingPlayerResults,
                handoff: Items::handoff($event, $occurrence, 'results', 'events.command.actions.recordResults'),
            );
        }
        if (! (bool) $summary['correctionStateSupported']) {
            $items[] = Items::make(
                'closeout.corrections_unsupported', 'closeout', Status::NotApplicable, Severity::Informational,
                'operations.results', 'events.command.items.correctionsUnsupported',
            );
        }
    ...
    private function evidence(PlayerReference $actor, Event $event, EventOccurrence $occurrence): array
    {
        $summary = $this->owners->read(
            ...
            $event,
            $occurrence,
            fn (): array => $this->evidence->forBearHuntOccurrence($actor->playerId, (string) $occurrence->id),
        );
        if ($summary === null) {
            return Items::section('evidence', 'events.command.sections.evidence', 'closeout', [
                $this->unknown($event, $occurrence, 'closeout.evidence_unavailable', 'intelligence.evidence', 'events.command.items.evidenceUnavailable', 'results'),
            ]);
        }
        ...
        $items = [];
        $this->appendEvidenceItem($items, 'closeout.evidence_processing', (int) $summary['processingCount'], 'events.command.items.evidenceProcessing', $handoff);
        $this->appendEvidenceItem($items, 'closeout.evidence_review', (int) $summary['awaitingReviewCount'], 'events.command.items.evidenceAwaitingReview', $handoff);
        $this->appendEvidenceItem($items, 'closeout.evidence_unmatched', (int) $summary['unmatchedGovernorCount'], 'events.command.items.evidenceUnmatched', $handoff);
        $this->appendEvidenceItem($items, 'closeout.evidence_commit_pending', (int) $summary['commitPendingCount'], 'events.command.items.evidenceCommitPending', $handoff);
        $failed = (int) $summary['commitFailedCount'] + (int) $summary['processingFailedCount'];
        if ($failed > 0) {
            $items[] = Items::make(
                'closeout.evidence_failed', 'closeout', Status::NeedsAttention, Severity::Blocking,
                'intelligence.evidence', 'events.command.items.evidenceFailed',
                ['count' => $failed], $failed, 'evidence',
                ['href' => $href, 'labelKey' => 'events.command.actions.recoverEvidence'],
            );
        }
        if ($items === []) {
            $items[] = Items::make(
                'closeout.evidence_clear', 'closeout', Status::Complete, Severity::Informational,
                'intelligence.evidence', 'events.command.items.evidenceClear',
                ['count' => (int) $summary['committedCount']], (int) $summary['committedCount'], 'evidence', $handoff,
            );
        }
    ...
    /** @param list<array<string,mixed>> $items @param array{href:string,labelKey:string} $handoff */
    private function appendEvidenceItem(array &$items, string $code, int $count, string $messageKey, array $handoff): void
    {
        if ($count <= 0) {
            return;
        }
        $items[] = Items::make(
            $code, 'closeout', Status::NeedsAttention, Severity::Blocking,
            'intelligence.evidence', $messageKey,
            ['count' => $count], $count, 'evidence', $handoff,
        );
    }
    ...
    private function debrief(Event $event, EventOccurrence $occurrence): ?array
    {
        ...
        $handoff = $available && is_string($summary['href'])
            ? ['href' => $summary['href'], 'labelKey' => 'events.command.actions.openDebrief']
            : null;

        return Items::section('debrief', 'events.command.sections.debrief', 'closeout', [
            Items::make(
                'closeout.debrief', 'closeout',
                $available ? Status::Complete : Status::NotApplicable,
                Severity::Informational,
                'readmodels.event_analysis',
                $available ? 'events.command.items.debriefAvailable' : 'events.command.items.debriefUnavailable',
    ...
    {
        return Items::make(
            $code, 'closeout', Status::Unknown, Severity::Blocking,
            $owner, $messageKey,
            handoff: Items::handoff($event, $occurrence, $anchor, 'events.command.actions.openOwnerWorkflow'),
        );
    }
