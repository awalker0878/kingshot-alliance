...
final readonly class EventCommandOperationalReadinessQuery
{
    ...
    public function forOccurrence(Event $event, EventOccurrence $occurrence, array $capabilities, CarbonImmutable $now): array
    {
    ...
    private function participation(Event $event, EventOccurrence $occurrence, array $capabilities, CarbonImmutable $now): array
    {
        ...
        if ($this->has($capabilities, EventCapability::Responses)) {
            ...
            $items[] = Items::make(
                'participation.unanswered', 'readiness',
        ...
        if ($this->has($capabilities, EventCapability::Waitlist)) {
            ...
            $items[] = Items::make(
                'participation.waitlist', 'readiness',
    ...
    private function registration(Event $event, EventOccurrence $occurrence, array $summary, CarbonImmutable $now): array
    {
        ...
        return Items::make(
            'participation.registration', 'readiness', Status::Complete, Severity::Informational,
            'operations.participation', 'events.command.items.registrationState',
            [
                'state' => $state,
    ...
    {
        $summary = $this->owners->read('operations.polls', $event, $occurrence, fn (): array => $this->polls->commandSummary($occurrence));
        if ($summary === null) {
    ...
    private function battlePlan(Event $event, EventOccurrence $occurrence): array
    {
        ...
        if ($invalid > 0) {
            $items[] = Items::make('battle_plan.invalid_assignments', 'readiness', Status::NeedsAttention, Severity::Blocking, 'operations.battle_plans', 'events.command.items.battlePlanInvalidAssignments', ['count' => $invalid], $invalid, handoff: Items::handoff($event, $occurrence, 'battle-plan', 'events.command.actions.openBattlePlan'));
        }
