...
final readonly class EventCommandContextReadinessQuery
{
    ...
    public function forOccurrence(PlayerReference $actor, Event $event, EventOccurrence $occurrence): array
    {
        ...
        if ($event->scopeEnum() !== EventScope::Player) {
        ...
        return Items::section('strategy', 'events.command.sections.strategy', 'readiness', [
            Items::make(
                ...
                [
                    ...
                    'revision' => (int) ($guide['revisionNumber'] ?? 0),
                ],
                1,
                'alliance_strategy',
                [
    ...
    private function territory(PlayerReference $actor, Event $event, EventOccurrence $occurrence): array
    {
        $summary = $this->owners->read(
            ...
            $event,
            $occurrence,
            fn (): array => $this->territory->forOccurrence($actor->playerId, $occurrence),
        );
        if ($summary === null) {
    ...
    private function communications(Event $event, EventOccurrence $occurrence): array
    {
        $reminder = $this->owners->read(
            ...
            $event,
            $occurrence,
            fn (): array => $this->reminders->forEvent((string) $event->id),
        );
        $delivery = $this->owners->read(
            'communications.delivery',
            $event,
            $occurrence,
            fn (): array => $this->deliveries->forEventOccurrence((string) $occurrence->id),
        );
        $items = [];
