...
final readonly class AllianceDataExportService
{
    ...
    public function generate(User $actor, Alliance $alliance): array
    {
        return DB::transaction(function () use ($actor, $alliance): array {
            // PostgreSQL repeatable-read gives the export one cross-table point-in-time
            // view without exclusive locks on every Alliance-owned child table.
            ...
            $payload = [
                ...
                'alliance' => $this->sanitizeRow($currentAlliance->getAttributes()),
            ...
            $this->audit->record('platform.alliance.exported', $context->actor, $currentAlliance, $currentAlliance, [
                'schema_version' => self::SCHEMA_VERSION,
                'row_count' => $rowCount,
                'sha256' => $sha256,
            ...
            return [
                'contents' => $contents,
                'filename' => 'alliance-'.$currentAlliance->id.'-'.$generatedAt->format('Ymd-His').'.json',
                'rowCount' => $rowCount,
                'sha256' => $sha256,
