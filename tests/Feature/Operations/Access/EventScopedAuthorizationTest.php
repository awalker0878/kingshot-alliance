...
namespace Tests\Feature\Operations\Access;
...
final class EventScopedAuthorizationTest extends TestCase
{
    ...
    public function test_player_event_authorization_uses_exact_player_identity_and_roster_manager_permission(): void
    {
        ...
        $entry = AllianceRosterEntry::query()->create([
            'alliance_id' => $alliance->id,
            'player_id' => Player::query()->where('game_player_id', 'player-event-member')->sole()->id,
        ...
        AllianceRosterEntry::query()->create([
            'alliance_id' => $alliance->id,
            'player_id' => Player::query()->where('game_player_id', 'player-event-peer')->sole()->id,
            'observed_name' => 'Peer',
            'state' => RosterState::Active,
            'joined_at' => now(),
            'last_observed_at' => now(),
