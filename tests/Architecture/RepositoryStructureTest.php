...
final class RepositoryStructureTest extends TestCase
{
    ...
    public function test_domain_security_profiles_follow_the_security_documentation_standard(): void
    {
        foreach ($this->directories($this->root().'/app/Domain') as $domain) {
            ...
            self::assertFileExists(
                $path,
                sprintf('Missing domain security profile: docs/domains/%s/security/README.md', $documentationDomain),
            );
    ...
    public function test_required_dcp_p2_focused_security_reviews_exist_and_are_indexed(): void
    {
        foreach ($this->requiredDcpP2FocusedSecurityReviews() as $domain => $files) {
            ...
            foreach ($files as $file) {
                ...
                self::assertFileExists(
                    $path,
                    sprintf('Missing DCP-P2 focused security review: docs/domains/%s/security/%s', $domain, $file),
                );
                self::assertStringContainsString(
                    $file,
                    $profile,
                    sprintf('Domain security profile must index focused review: docs/domains/%s/security/%s', $domain, $file),
                );
            }
        }
    }
