...
$rankAuthorityPatterns = [
    '/\b(?:if|while)\s*\([^\n)]*\brank\b[^\n)]*(?:===|==|!==|!=|>=|<=|>|<|R[1-5])[^\n)]*\)/i',
    '/\b(?:v-if|v-else-if|v-show)="[^"]*\brank\b[^"]*(?:===|==|!==|!=|>=|<=|>|<|R[1-5])[^"]*"/i',
    '/\brank\b\s*(?:===|==|!==|!=|>=|<=|>|<)\s*[\'\"]?R?[1-5]\b/i',
];
...
foreach (new RecursiveIteratorIterator(new RecursiveDirectoryIterator($source)) as $f) {
    ...
    foreach ($rankAuthorityPatterns as $pattern) {
        if (preg_match($pattern, $c) === 1) {
            $v[] = 'Rank-derived frontend authority in '.str_replace($root.'/', '', $f->getPathname()).'; use server-issued capabilities instead';
            break;
        }
    }
}
