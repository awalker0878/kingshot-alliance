...
**Status:** Current — `KINGDOMS-001`, `KINGDOMS-002`, `KINGDOMS-003` Accepted; `KINGDOMS-004` through `K4-P3` validated when the containing P3 evidence head is protected-green  
...
- K4 delegated player-snapshot promotion for an existing roster relationship; and
- K4 delegated game-Alliance factual observation promotion for an existing active tracking relationship.

### Out of scope

- application/membership ownership;
- unapproved sources, scraping/OCR/bots/browser/game-client automation;
- K4 external acquisition/scheduler/worker until P4;
...
K4 adds Alliance-owned `KingdomIngestionSubscription`, `KingdomIngestionBatch`, and `KingdomIngestionCandidate`. Promoted candidates safely reference canonical K1/K3 observation ULIDs; promoted observations copy bounded machine provenance without operational-row FKs.
...
11. K4 production adapters are repository/config allowlisted; current production list is empty.
12. Internal `kingdoms.*` events never automatically become public webhooks.
...
K4 managers control approved subscriptions/rejections. Internal services manage batches/candidates and may promote factual player snapshots or game-Alliance observations only through their accepted business actions after stable-ID/tenant/current-context checks. See [Automated game-data ingestion](automated-ingestion.md).
...
- privileged human mutations require recent password confirmation; and
- Alliance→Kingdom setting uses `alliance.manage`.

Machine promotion derives authority only from already-owned K4 state. Neutral identity, adapter/source identity, candidate state, coordinator/contact responsibility, or display data never grants application authorization.
...
Member-safe presentation, manager-only accepted mutation/query contracts, K4 manager control/status and internal promotion services, and internal durable `kingdoms.*` events. K4 adapter registration is repository/operator configuration, not a public integration contract.
...
K4 normalized candidates are bounded operational data, not raw-source archives. Promoted K1/K3 history stores bounded source provenance independently of candidate retention.
...
K4 uses internal `kingdoms.ingestion_*` events plus the accepted K1/K3 observation events for delegated promotions. No inbound API/webhook or arbitrary endpoint/secret configuration exists.
...
K4 adds manager-only ingestion status/control routes. P2/P3 promotion services add **no HTTP/public API route** for arbitrary source payloads or candidate promotion.

## 11. Background processing

K1–K3 remain primarily synchronous with shared outbox publishing. K4 through P3 has **no ingestion background processing**: no scheduler, acquisition queue job/partition, crawler, scraper, OCR worker, bot, external poller, cursor loop, retry/backoff worker, or replay worker.

Those concerns remain gated to `K4-P4` after this P3 evidence head is accepted.

## 12. Failure, idempotency and concurrency

Existing snapshot/observation/CSV/transfer idempotency remains enforced. K4 source windows/candidate identities are deterministic; exact promoted-candidate retry returns the existing canonical record. Later capture appends history.

Unknown/ambiguous/inactive targets, tenant relationship absence, Kingdom drift, revoked adapters, unsupported/bounded values, and missing stable identity fail closed/quarantine before business mutation.

## 13. Security and privacy

Kingdoms holds high-value tenant operational intelligence. Manager-private notes/reasons/contacts/provenance and K4 operational state must not cross tenant/public boundaries.
...
Use safe tenant/reference/state/count/timing/hash/reason/promoted-record identifiers with request/trace/audit/outbox correlation. Do not log source secrets/raw responses/private text.
...
Suites protect identity/tenancy, authorization/privacy, append history, human-only correction/diplomacy, retry/idempotency, CSV/transfer/K3 invariants, performance/accessibility, public integration exclusion, and K4 allowlist/no-secret/stable-ID/quarantine/no-auto-relationship boundaries.

K4-P3 runtime candidate `8186af9fd7276a20889ca3a25b80172c6fe824d9` passed DR `31541291512`, CodeQL `31541291470`, CI `31541291501`: Pint 515, PHPStan 365/365 zero errors, 417 tests / 9,628 assertions, image/staging/backup/scan success.

## 16. Explicit non-capabilities

The current runtime does not implement a real Kingshot source, arbitrary manager network fetches, scraping/OCR/bots/browser/game-client automation, K4 scheduler/worker/cursor/retry loop, auto roster/tracking/membership/transfer/diplomacy/contact behavior, machine K3 correction/invalidation, cross-Alliance shared intelligence, scoring/ranking/recommendations, or public Kingdoms API/webhook.

Production adapter allowlist is empty. Production source enablement remains separately approved.
