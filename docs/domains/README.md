...
- Historical phase ownership belongs in `../product/`; threat and abuse analysis belongs in `../security/`; operational procedures belong in `../operations/`.
- If a domain guide conflicts with the implementation plan or an accepted ADR, update the guide or record a new ADR rather than silently redefining the architecture here.

## Updating a domain guide

When behavior changes materially, update the guide together with its tests and any affected security, operations, accessibility, or acceptance evidence. Prefer describing stable business rules and invariants over controller-by-controller implementation detail.
