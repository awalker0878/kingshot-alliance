...
Two artwork gates are distinct on purpose. `npm run check:kingdom-map-art` (`--structure-only`) validates registry keys, schema, limits, content-addressed paths and anchors without asserting delivered bytes, and is part of `npm run check`. `npm run check:kingdom-map-art:strict` additionally requires every raster representation to exist, match its recorded `sha256`/`byte_size` and stay inside the delivery root, and therefore exits 1 with `BLOCKED_INPUT` until the artwork master pack is supplied and with `REVIEW_PENDING` while supplied art is not yet human-reviewed. A strict failure is an honest incomplete-coverage report, not a defect.

KM03/KMAP-004 is partially satisfied. The versioned registry, preparation and check scripts, the whole-pack ingestion and approval path and the export/canvas embed path are implemented, tested and proven end to end against generated verification bytes. The outstanding requirement is the artwork itself: rights are confirmed cleared, but no master pack was supplied, so no representation has real source bytes, hashes or inspected output. `npm run art:pack -- <pack-dir>` followed by `npm run art:approve -- <reviewer>` closes the row once the pack exists.
