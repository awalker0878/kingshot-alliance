...
| Kingshot Event Type Framework | Selected extension — implementation present, containing-candidate verification pending | Resolve server-owned verified Event identities to a closed set of application workflow dimensions; keep candidate/conflicting/unsupported profiles disabled and free of unsupported game mechanics. | Operations/Events owns canonical identity/profile truth; owner contexts keep workflow facts; ReadModels composes |
| Rally Roster Builder | Selected extension — implementation present, containing-candidate verification pending | Expose factual Rally assignment gaps, duplicate assignments, missing leads, declined participants and observation freshness inside the existing Event/Rally workflow without optimizing lineups. | Operations/Participation, Rosters and Rallies own state; ReadModels/EventManagement composes |
| Alliance Member Capability Profile | Selected extension — implementation present, containing-candidate verification pending | Show officers what is factually known about one Governor across authorized progression, Event, Bear Hunt, Rally, BattlePlan, Transfer and Evidence histories without a strength score. | Applicable owner contexts retain truth; ReadModels/Roster composes |
| Bear Hunt Performance expansion | Selected extension — implementation present, containing-candidate verification pending | Extend the existing Debrief with accepted run/Governor/Rally/attendance/Evidence history and deterministic bounded comparisons. | ReadModels/EventAnalysis composes existing owners; no BearHunt context |
...
| Transfer Campaign Workspace | Selected extension — implementation present, containing-candidate verification pending | Follow a Recruitment candidate through authorized Transfer evidence/eligibility, coordination and observed membership arrival without copying owner truth. | Alliance/Recruitment, GameWorld/KingdomTransfers, Intelligence/Evidence, Communications and Alliance/Membership |
| Kingdom Intelligence Timeline | Selected extension — implementation present, containing-candidate verification pending | Present authorized owner histories and derived change links chronologically with source, observed time, confidence and scope, without inferring intent. | Intelligence/GameWorld/Alliance/Operations owners + ReadModels composition |
| Territory & Hive Execution | Current complete capability; expansion reconciliation implemented, containing-candidate verification pending | Carry immutable published planning intent through dated observed reality, factual comparison, discrepancy resolution and replacement revision. | Operations/TerritoryPlanning + Intelligence/Evidence/Observations + GameWorld/KingdomMaps + ReadModels |
| Alliance Command | Selected extension — implementation present, containing-candidate verification pending | Tell R4/R5 officers what needs attention across authorized Kingshot owner workflows with recomputable reason codes and navigation-only handoffs. | ReadModels/CommandOverview composition; no task/attention store |
| Officer Briefs & Notifications | Selected extension — implementation present, containing-candidate verification pending | Deliver factual Event, Rally, Transfer, progression, Territory and Bear Hunt triggers/digests with owner handoffs and idempotent delivery. | Owner projections/signals supply facts; Communications owns preferences/delivery/retry |
| Alliance Assistant — Kingshot expansion | Selected extension — implementation present, containing-candidate verification pending | Answer bounded Kingshot Command, Event, Rally, Bear Hunt, Transfer, progression, Intelligence and Territory questions with provenance and zero direct writes. | ReadModels/AllianceAssistant composes authorized owners |
...
| Progression Goal Planner | Current complete capability subject to containing-SHA verification | Compare current observed Governor progression with a user-selected factual target/prerequisite path without inventing recommendation or unqualified cost formulas. | ReadModels composes GameWorld/Progression + Intelligence/Roster; any saved planning intent stays with an existing appropriate Operations owner |
