...
| Alliance Assistant — Game Data and operational self | Selected extension | Add source-backed `game_fact`, Participation self, BattlePlan self, Transfer self and applicable published Territory-revision questions while preserving authorization-before-retrieval, citations and zero direct writes. | `ReadModels/AllianceAssistant` composes GameWorld/Progression, GameWorld/KingdomTransfers and Operations owner Queries |
...
The delivered Alliance Assistant is a constrained read/composition capability. It answers only from current data the active Governor is already authorized to view, cites every substantive source, preserves `operational_fact`, `alliance_strategy` and `observation` provenance, and never becomes an alternate writer. The delivered first release uses deterministic bounded intent resolution with no external model provider or persisted conversation.

`game_fact` is already a reserved provenance classification, but the generic GameWorld question intent is not part of the delivered first release. Its implementation plus Participation/BattlePlan/Transfer/Territory self reads is a **Selected extension** under [Capability Extension Program](capability-extension-program.md). Until those delivery rows close, unsupported questions remain unsupported rather than being answered from model memory.

The delivered requirements and evidence live in [Alliance Assistant](alliance-assistant.md) and the global [capability delivery ledger](capability-delivery-ledger.md).
