...
Code and tests remain authoritative for exact runtime behavior. The [implementation plan](implementation-plan.md) remains authoritative for the completed Phase 0–6 baseline. Accepted post-program implementation evidence is recorded for [`KINGDOMS-001`](kingdoms-roster-intelligence-exit-report.md) and [`KINGDOMS-002`](kingdoms-transfer-planning-exit-report.md). Approved `KINGDOMS-003` scope is recorded in [Kingdom/alliance intelligence and diplomacy](kingdoms-alliance-intelligence-increment.md); `K3-P0` is complete and Slice A / `K3-P1` is validated, while the remaining observation/diplomacy/contact/intelligence slices and whole-increment acceptance are still pending. A real production cutover remains **not yet approved**; see [production launch approval](production-launch-approval.md).
...
| `KINGDOMS-003` — Kingdom/alliance intelligence and diplomacy | **Approved / In progress (`K3-P1` validated)** | Neutral game-side alliance identity and tenant tracking are validated; tenant-owned observations, explicit diplomacy/NAP history, manager-private contacts and descriptive intelligence remain pending slices without ranking/threat scoring | [Scope](kingdoms-alliance-intelligence-increment.md), [implementation plan](kingdoms-alliance-intelligence-implementation-plan.md), [K3-P0 decisions](kingdoms-alliance-intelligence-p0-decisions.md), [Slice A validation](kingdoms-alliance-intelligence-slice-a-validation.md) |

`KINGDOMS-003` is **In progress**, not whole-increment Accepted. `K3-P1` validates only neutral game-side alliance identity plus tenant tracking; observations, diplomacy/NAP, contacts and derived alliance intelligence are not yet implemented by this slice. Automated game-data ingestion, opt-in cross-alliance/shared kingdom intelligence, automated player/alliance scoring and public Kingdoms API/webhook contracts remain separate follow-on scopes and are **not approved implementation scope** unless they receive their own increment record.
...
| `KINGDOMS-003` alliance intelligence/diplomacy | **In progress; `K3-P0` complete; `K3-P1` validated** | Neutral `KingdomAlliance` identity and tenant-owned tracking are implemented/validated on this slice. Alliance observations, diplomacy/NAP state/history, contacts and derived K3 intelligence remain future slices; K3 is not Accepted until `K3-P6`. |
...
| K3 alliance observations/diplomacy/contacts | **Not yet implemented** | Slice A contains no observation facts, diplomacy/NAP lifecycle, contact directory or descriptive trend dashboard. Those remain `K3-P2`–`K3-P5`. |
...
| Automated Kingshot game-data ingestion | **Not approved / not implemented** | K3 Slice A is first-party manual tracking only. Scraping, OCR, bots, automated game ingestion and undocumented/unapproved APIs remain outside current runtime scope. |
| Cross-alliance/shared Kingdom intelligence | **Not approved / not implemented** | K3 tenant tracking does not expose another platform Alliance's tracking/private notes merely because two tenants share the same neutral `KingdomAlliance` reference. |
...
| Alliance threat/rival/desirability scoring | **Not approved / not implemented** | K3 contains no threat score, battle prediction, automated diplomacy inference, punitive ranking or automated recommendation. |
