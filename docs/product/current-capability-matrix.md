...
| --- | --- | --- | --- |
| `KINGDOMS-001` — Kingdoms roster intelligence | **In progress — Slice A validated; Slice B and Slice C1 validated implementation candidates** | Review/accept the roster and append-only snapshot-history candidates into the dependency stack; then implement `K1-P4` intelligence/trends and `K1-P5` CSV migration workflow | [Kingdoms roster intelligence product increment](kingdoms-roster-intelligence-increment.md), [implementation plan](kingdoms-roster-intelligence-implementation-plan.md), [Slice B roster contract](../domains/kingdoms-roster.md), [Slice C1 snapshot contract](../domains/kingdoms-snapshots.md) |
...
| Slice C1 snapshot capability | **Validated implementation candidate / not merged** | The Slice C1 branch adds alliance-scoped append-only player snapshots, signed-64-bit power, manual provenance, deterministic retry idempotency, member history, manager recording, latest-observation projection and snapshot-backed current/stale/missing semantics. Its protected implementation gate is green; it remains a review candidate until accepted into the dependency stack. |
