...
| --- | --- | --- | --- |
| `KINGDOMS-001` — Kingdoms roster intelligence | **In progress — Slice A validated; Slice B validated candidate; Slice C1 snapshot implementation candidate** | Validate/review append-only snapshot history, latest-observation projection and snapshot-backed freshness; then implement `K1-P4` intelligence/trends and `K1-P5` CSV migration workflow | [Kingdoms roster intelligence product increment](kingdoms-roster-intelligence-increment.md), [implementation plan](kingdoms-roster-intelligence-implementation-plan.md), [Slice B roster contract](../domains/kingdoms-roster.md), [Slice C1 snapshot contract](../domains/kingdoms-snapshots.md) |
...
| Slice C1 snapshot capability | **Implementation candidate / not yet promoted** | The Slice C1 branch adds alliance-scoped append-only player snapshots, signed-64-bit power, manual provenance, deterministic retry idempotency, member history, manager recording, latest-observation projection and snapshot-backed current/stale/missing semantics. It is not validated current capability until its protected gate succeeds. |
| Later `KINGDOMS-001` capability | **Not implemented** | Aggregate roster intelligence, total/average/median power, 7/30-day trends, comparative manager views and CSV workflows remain `K1-P4`–`K1-P5`. |
