...
- Latest durable checkpoint: `6457d0a817eb1d04a2f46ac372132acf74ba7e7b` contains participant paging. This coherent slice removes full-plan dashboard hydration through an explicit owner verification preview; its immutable SHA is recorded by the next checkpoint. Preserve concurrent HARD-106 work.
- Current item/state: HARD-095 / In progress. The four participant workspaces, child histories and single-profile lookup are bounded. Dashboard, Assistant and daily brief now use one canonical bounded preview with complete totals and explicit unassessed coverage. Management catalogues/selectors and HARD-106 remain open. Newly discovered screenshot fidelity defect is HARD-107.
...
- Active files: owner Transfer preview Query/DTO, removal of the obsolete eager participant API, CommandOverview/Assistant/brief consumers, Dashboard labels in17 locales, regression tests, ADR-0051 and current owner/product/reference/operations documentation.
...
- Exact next action: publish the preview with its tests/ADR/ledger; fix HARD-107 with bounded captures that cover every intended region and keep semantic assertions; continue remaining catalogue work. No merge until all items and containing gates pass.
