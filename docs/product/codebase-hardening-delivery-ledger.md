...
- Latest pushed durable checkpoint: `6493e08477fd1d613811d252e779da2dc6b2a948`.
...
- Known failures: b3c58f3c completes 979 tests with nine fixture failures: eight assert null against User.getRememberToken, which deliberately returns an empty string for absent tokens; one older login fixture still wraps the HTTP call in a test transaction. The assertions now check the raw persisted token column, and the older session suite uses real commits. All passkey confirmation cases and actual password/MFA success, competing revocation and stale-primary cases pass. Containing verification remains pending.
