...
- Completion evidence: LogoutAccount now replaces controller-side logout. It atomically records current-session revocation and audit under the account lock, then clears only the current maintained guard/session outside database locks without rotating other remembered-browser authority. Persistence failure still clears the current browser before propagating. AccountLogoutV3Test covers current-only revocation/remember preservation and late audit rollback with fail-safe cleanup; containing verification is pending.
