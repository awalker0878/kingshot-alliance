...
- Latest durable checkpoint before this slice: `1a8a3b5830860268c7a218c17a1b3712911fe875` implements HARD-108 core labels; HARD-107 instant scrolling is `a8af9cd1d489c48b498c60623a717631d7b41459`. The following checkpoint records the receipt-preserving capture correction.
...
- Known failures: hosted readiness run34652455100 reaches both final comparisons; only review placeholders fail. Every tile is painted and both new boolean-label assertions pass. Complete image review found that the old test-only receipt normalizer duplicated the Succeeded label; replace only its generated ID before approving the baseline. HARD-095/106 and repository audit remain open.
