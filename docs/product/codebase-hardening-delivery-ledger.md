...
- Latest pushed durable checkpoint: `f60af6b720c3f3888a2ddfe174338697fc0c32ae`.
- Draft PR: [#163](https://github.com/awalker0878/kingshot-alliance/pull/163).
- Current item/state: HARD-038 / In progress (passkey confirmation slice); HARD-037 login completion remains under design. HARD-039 addresses parallel CI PostgreSQL lock capacity; containing gates remain pending.
...
- Active files: VerifyAccountPasskey, maintained-action binding and after-commit proof listener, signed WebAuthn fixture, 15 passkey regressions, Authentication contract and ledger.
...
- Known failures: f60af6b7 runs 949 tests / 75,763 assertions with one reset HTTP case returning 429 because independent tests share the Redis IP limiter. Each reset scenario now has a distinct simulated IP. All eight Google confirmation cases, corrected password proof cases and prior terminal/cleanup/reset cases pass; HARD-039 prevents lock exhaustion. Containing verification pending.
