...
Role administration and assignment choices are paged in groups of 25 with case-insensitive name-prefix search. Active and archived management views are separate; assignment choices include only active roles. Paging does not impose a role-creation quota. New roles remain editable immediately after creation, and rejected edits retain their draft with visible feedback. Permission replacement must be explicit, including an empty list when revoking all permissions; omitted input is rejected. Display names and generated stable keys must fit their existing storage bounds.
