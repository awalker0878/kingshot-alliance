...
- `scripts/kingdom-map-art-pack.mjs` is the whole-pack ingestion path added for this request: `npm run art:pack -- <pack-dir> [--dry-run]` reads a `pack.json` declaration, prepares every named representation, content-addresses the delivery bytes and applies bytes plus registry records atomically, and `npm run art:approve -- <reviewer> [--date YYYY-MM-DD]` is the separate named human review that promotes entries to `reviewed`. Unknown keys/kinds, absolute paths and `..` escapes are rejected before any bytes are read; a rejected pack leaves the registry and delivery tree untouched; identical re-ingestion is idempotent; ingestion can never set `reviewed` itself.
- The strict gate now verifies delivery bytes rather than declarations only: for each representation it checks the content-addressed file exists, does not escape the delivery root, and matches the recorded `sha256` and `byte_size`. `REVIEW_PENDING` reports art that is delivered but unreviewed, distinct from `BLOCKED_INPUT` for art that was never supplied.
- Verification: `tests/ReadModels/TerritoryPlanning/Frontend/TerritoryArtwork.test.ts` (8 tests), `TerritoryArtworkImport.test.ts` (5 tests) and `TerritoryArtworkPack.test.ts` (7 tests). The import suite round-trips prepared bytes through the PNG decoder and asserts the emitted record is accepted by `validateArtworkManifest` unchanged; the pack suite covers ingestion, idempotency, atomic rollback on a rejected pack, declaration and path validation, unauthorized-source rejection, approval gating and the honesty of the shipped registry.
- **Pipeline proven end to end.** A throwaway 24-entry/72-representation pack of generated PNGs was ingested for verification only. The strict gate correctly failed with `REVIEW_PENDING` before approval and passed with "27 reviewed entries" after `npm run art:approve`; `npm run build` then emitted all 72 assets and the `artwork-runtime` chunk referenced all 72 `assets/sha256-…` URLs, confirming the export/canvas embed path resolves real bytes. This verification also exposed a real defect — the delivery root double-appended `assets/` (`assets/assets/sha256-…`) — which is fixed. All generated bytes, the temporary pack and the temporary registry records were removed afterwards; the shipped registry is unchanged and honest.
- **Blocked input.** `npm run check:kingdom-map-art:strict` exits 1 with `BLOCKED_INPUT`, naming every absent icon/sprite/detail representation. Rights for the Kingshot Kingdom Map artwork master pack are confirmed cleared and reconfirmed on 2026-09-14, but the pack **bytes were not supplied** in any accessible location, so the 24 raster entries legitimately remain `awaiting_source`. The ingestion path is implemented, tested and proven; supplying the pack and running the two commands above is the only remaining step. No imagery, hash, provenance or authorization was fabricated to close this gate.
