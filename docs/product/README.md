...
- [Kingdoms transfer planning increment](kingdoms-transfer-planning-increment.md) — **Approved scope / Planned implementation** for alliance-owned transfer cycles, incoming/outgoing/staying participants, destinations, groups, coordinators, readiness/blockers, and explicit roster handoff.
- [KINGDOMS-002 implementation plan](kingdoms-transfer-planning-implementation-plan.md) — planned gated sequence `K2-P0` through `K2-P6` for independently reviewable transfer-planning slices and whole-increment acceptance.
- [Current capability matrix](current-capability-matrix.md) — present-tense navigation across implemented capabilities, approved planned increments, living contracts, and explicit non-capabilities/boundaries.
...
| `KINGDOMS-002` | [Kingdoms transfer planning](kingdoms-transfer-planning-increment.md) | **Approved / Planned** | Alliance-owned transfer cycles with incoming/outgoing/staying intent, destinations, groups/coordinators, manual readiness/blockers, and explicit roster handoff. |

Implementation sequences: [KINGDOMS-001](kingdoms-roster-intelligence-implementation-plan.md) and [KINGDOMS-002](kingdoms-transfer-planning-implementation-plan.md). `KINGDOMS-001` acceptance evidence is recorded in its [exit report](kingdoms-roster-intelligence-exit-report.md). `KINGDOMS-002` does not receive an exit report until its whole-increment acceptance gate passes. Current runtime detail remains in the [Kingdoms domain guide](../domains/kingdoms.md).
...
Do not use **Accepted** and **Approved** interchangeably. `KINGDOMS-001` is Approved scope with an Accepted implementation. `KINGDOMS-002` is Approved scope with Planned implementation. Repository production hardening is Accepted while real production launch remains Not yet approved.
...
There is no Phase 7 in the current baseline. `KINGDOMS-001` and `KINGDOMS-002` are approved post-program product increments, not a continuation of phase numbering.
