...
5. Persisted domain changes and their audit/outbox records must remain transactionally consistent and safe to retry.
...
| Invitation theft, replay, wrong-account acceptance | Unauthorized membership | Random bearer token stored only as hash; expiry/status checks; email binding; row locks; token rotation on resend; one-time acceptance/revocation | Invitation lifecycle abuse tests |
...
| Last-owner removal or lower-rank administration of higher roles | Alliance takeover/lockout | Membership hierarchy guard; last-owner safety; role changes restricted to owner-level permission | Membership administration tests |
...
| Duplicate outbox delivery | Repeated downstream effect | At-least-once publication with globally unique idempotency key propagated to consumers; published timestamp after dispatch | Outbox publisher idempotency test |
...
| Audit event loses actor/tenant/correlation | Weak incident attribution | Shared `AuditRecorder`; actor, alliance, subject, request ID, and trace ID where available | HTTP creation and privileged-action audit assertions |
...
- MFA recovery codes are stored as SHA-256 hashes and displayed only at creation/regeneration.
- Invitation bearer tokens are never stored in plaintext; only token hashes are persisted.
...
No unresolved critical or high-risk Phase 1 threat remains in the implemented scope. Any later domain that introduces new alliance-owned rows, queues, notifications, exports, or storage must preserve the explicit tenant boundary and extend this threat model.
