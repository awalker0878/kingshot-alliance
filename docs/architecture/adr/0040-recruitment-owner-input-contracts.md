...
The first PostgreSQL boundary run exposed that `unsignedSmallInteger` becomes a signed smallint and cannot store the promised position limit of 65,535. Question and onboarding positions now use integer storage in the fresh schema, retaining the existing owner/HTTP/form range. The exact-maximum persistence tests remain unchanged.
