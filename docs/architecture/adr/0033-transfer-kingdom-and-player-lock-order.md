...
Staying completion stabilizes its canonical target Player with FOR SHARE because it records an observation without mutating Player identity. Incoming and outgoing completion retain FOR UPDATE. This permits the actual Player foreign-key references when independent Alliances track each other's officers, while still excluding identity mutation. HARD-083 covers both opposing Staying orders, preserved actor attribution and idempotent retry; the existing archival and rollback cases remain required.
