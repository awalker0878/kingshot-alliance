...
Operational retention processes at most 500 records per category per invocation, ordered by age and ID under skip-locked row locks. Eligibility is rechecked on mutation, preserving the webhook manual-retry boundary. Revoked credentials with external actor link or receipt references are retained as history anchors and excluded before batch selection; they do not prevent unreferenced credentials or other categories from progressing. See [ADR-0058](../../adr/0058-bounded-platform-maintenance-progress.md).
