...
Interactive usage capture enters CaptureAllianceUsage with the operator identity. The Action holds the current Platform grant and Alliance reference through snapshot creation and its audit record. A revoked operator cannot reuse middleware admission; a late failure rolls back both records. PlatformUsageService::capture remains the trusted scheduled primitive and returns the created snapshot identifier.
