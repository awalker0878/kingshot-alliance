...
Cross-context lifecycle barriers use FOR NO KEY UPDATE to serialize mutable account/ownership operations while permitting audit foreign keys to reference the immutable User ID. Finalization acquires its stronger account mutation lock after dependent Alliance and Player cleanup. [ADR-0032](../../adr/0032-account-lifecycle-locks-and-audit-references.md) defines the compatible reference boundary and retained terminal-state checks.
