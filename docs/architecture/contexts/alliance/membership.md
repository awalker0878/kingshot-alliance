...
Bulk status owners normalize duplicate selections and reject empty or more than fifty distinct IDs before scope queries. Outcomes and aggregate receipts use the same canonical selections. Standalone transfer membership handoff takes exclusive Alliance scope before actor and target membership locks, retaining current permission, hierarchy and idempotent completion checks. See [ADR-0035](../../adr/0035-current-recruitment-lifecycle-and-target-authority.md).

Status and rank administration and specialist role assignment/removal also acquire exclusive Alliance scope before actor/target membership rows. The current actor's explicit grants, rank ceiling, target hierarchy and terminal state remain authoritative after acquiring that scope; serialization does not grant additional permissions.
