...
Webhook sweeps reserve only their bounded due page before dispatch and recover at most the same bounded number of stale queued/delivering leases. A job cannot claim before `available_at`. Transport revalidates and pins a public HTTPS destination, disables redirects, and finalizes only the exact persisted attempt token. Do not clear attempt tokens, rewind attempts or force a rejected destination back to Pending; correct or replace the subscription and use the authorized recovery path. See [ADR-0053](../architecture/adr/0053-webhook-egress-destination-security.md) and [ADR-0054](../architecture/adr/0054-fenced-bounded-webhook-recovery.md).
...
Content management reads are independently bounded from announcement background work. Catalogue pages load twenty subjects and only their schedules/counts; run-history requests scope five retained runs to one currently authorized item. The Communications outcome query aggregates all matching retained records for those runs, without audience hydration. Five canonical keyset indexes support Content/media/category/revision/run traversal. No read cache or new persisted outcome counter is introduced. An invalid or stale cursor fails validation; restart from the first page rather than disabling authorization or increasing page limits. [ADR-0052](../architecture/adr/0052-bounded-current-manager-content-workspaces.md) defines this read boundary.
