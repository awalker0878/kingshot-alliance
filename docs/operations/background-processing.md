...
Accounts registration and verification resend persist private account.email.verification_requested intents. The normal outbox publisher delivers branded verification mail after rechecking the current account and requested address fingerprint. Failed SMTP attempts follow the shared bounded backoff; exhausted records use the existing Citadel recovery action. A verified, finalized, missing or changed-address recipient consumes stale intent without sending. Signatures expire from delivery time. Mail acceptance followed by acknowledgement loss can cause a duplicate on retry.
