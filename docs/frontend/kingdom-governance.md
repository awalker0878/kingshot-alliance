...
Governance role, assignment, holder and history catalogues have independent 25-row pages with total counts. Assignment search and role/Governor filters run on the full server catalogue. Governor and role pickers preserve one off-page selection; failed pages retain the current form and provide retry. Bulk selection contains at most 50 explicitly chosen Governors and changing its intent invalidates preview. Actor or Kingdom changes discard prior intent. Holder counts link to the complete filtered assignments instead of embedding every role. Public history excludes operator account identity, arbitrary audit metadata and recovery reasons. See [ADR-0070](../architecture/adr/0070-bounded-current-kingdom-governance-catalogues.md).
