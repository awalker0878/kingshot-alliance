...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::create('external_actor_pairing_codes', function (Blueprint $table): void {
            ...
            $table->index(['alliance_id', 'player_id', 'provider'], 'actor_pairing_player_provider_index');
        });

        Schema::create('external_actor_links', function (Blueprint $table): void {
            $table->ulid('id')->primary();
            $table->foreignUlid('alliance_id')->constrained('alliances')->cascadeOnDelete();
            $table->foreignUlid('player_id')->constrained('players')->cascadeOnDelete();
            $table->foreignUlid('api_credential_id')->constrained('api_credentials')->restrictOnDelete();
            $table->string('provider', 24);
            $table->char('subject_hash', 64);
            $table->string('subject_hint', 16);
        ...
        Schema::create('external_actor_action_receipts', function (Blueprint $table): void {
            $table->ulid('id')->primary();
            $table->foreignUlid('alliance_id')->constrained('alliances')->cascadeOnDelete();
            $table->foreignUlid('external_actor_link_id')->constrained('external_actor_links')->cascadeOnDelete();
            $table->foreignUlid('api_credential_id')->constrained('api_credentials')->restrictOnDelete();
    ...
    public function down(): void
    {
        ...
        Schema::dropIfExists('external_actor_pairing_codes');
