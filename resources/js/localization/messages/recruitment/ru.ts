...
const messages = {
  recruitment: {
    ...
    bulkPreviewHelp: 'Перед применением изменения проверьте, кого можно перевести. Не подходящие кандидаты останутся без изменений.',
    previewBulkAction: 'Проверить изменение этапа',
    bulkPreview: 'Предпросмотр изменения этапа',
    bulkPreviewSummary: '{ready} можно обновить; {blocked} требуют проверки или уже находятся на нужном этапе.',
    ...
    bulkResultSummary: '{succeeded} обновлено. {failed} требуют проверки. {skipped} уже были актуальны.',
    failedItemsSelected: 'Кандидаты, которых не удалось обновить, остаются выбранными для проверки.',
