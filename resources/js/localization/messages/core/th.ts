import type { MessageCatalogue } from '../../types';

const messages = {
  common: {
    language: 'ภาษา',
    signIn: 'เข้าสู่ระบบ',
    signOut: 'ออกจากระบบ',
    createAccount: 'สร้างบัญชี',
    continue: 'ดำเนินการต่อ',
    cancel: 'ยกเลิก',
    save: 'บันทึก',
    close: 'ปิด',
    loading: 'กำลังโหลด',
    menu: 'เมนู',
    openNavigation: 'เปิดเมนูนำทาง',
    closeNavigation: 'ปิดเมนูนำทาง',
    playerAlliance: 'พันธมิตรของผู้เล่นที่ใช้งานอยู่',
    noPlayerAlliance: 'ผู้เล่นที่ใช้งานอยู่ไม่มีสมาชิกภาพพันธมิตรที่ใช้งานอยู่',
    skipToContent: 'ข้ามไปยังเนื้อหา',
  },
  navigation: {
    home: 'หน้าหลัก',
    dashboard: 'แดชบอร์ด',
    alliance: 'พันธมิตร',
    events: 'กิจกรรม',
    roster: 'สมาชิก',
    recruitment: 'รับสมัคร',
    content: 'เนื้อหา',
    contributions: 'ผลงาน',
    kingdom: 'อาณาจักร',
    transfers: 'ย้ายอาณาจักร',
    integrations: 'การเชื่อมต่อ',
    profile: 'โปรไฟล์',
    settings: 'การตั้งค่า',
    allianceOperations: 'การจัดการพันธมิตร',
    kingdomOperations: 'การจัดการอาณาจักร',
    account: 'บัญชี',
  },
  application: {
    dashboard: {
      title: 'แดชบอร์ด',
      eyebrow: 'ศูนย์บัญชาการพันธมิตร',
      welcome: 'ยินดีต้อนรับ {name}',
      verificationPending: 'รอยืนยันอีเมล',
      playerContextTitle: 'ผู้เล่นที่ใช้งานอยู่',
      playerContextIntro:
        'การสลับผู้เล่นจะเปลี่ยนตัวตนในเกมที่ใช้สำหรับสิทธิ์ของพันธมิตรและอาณาจักร',
      playerKingdom: 'อาณาจักร #{kingdom}',
      playerAuthorityIntro:
        'สมาชิกภาพ ยศ บทบาท สิทธิ์ของอาณาจักร และการกระทำในเกมถูกกำหนดจากผู้เล่นนี้เท่านั้น',
      selectPlayer: 'เลือกผู้ว่าการ',
      playerAllianceTitle: 'พันธมิตรของผู้เล่นที่ใช้งานอยู่',
      playerAllianceIntro: 'เครื่องมือพันธมิตรใช้เฉพาะสมาชิกภาพ ยศ และบทบาทของผู้เล่นที่ใช้งานอยู่',
      noPlayerAllianceTitle: 'ผู้เล่นนี้ไม่ได้อยู่ในพันธมิตร',
      noPlayerAllianceIntro:
        'สลับผู้เล่นหรือสร้าง/เข้าร่วมพันธมิตรด้วยผู้เล่นที่ใช้งานอยู่ก่อนเปิดเครื่องมือพันธมิตร',
      openPlayerAlliance: 'เปิดพันธมิตรของผู้เล่น',
      active: 'ใช้งานอยู่',
      roles: 'บทบาท',
      roster: 'รายชื่อ',
      kingdomAlliances: 'พันธมิตรในอาณาจักร',
      transfers: 'การย้าย',
      kingdomSettings: 'การตั้งค่าอาณาจักร',
      createTitle: 'สร้างพันธมิตร',
      createIntro:
        'สร้างพันธมิตรสำหรับผู้เล่นที่ใช้งานอยู่ อาณาจักรของพันธมิตรถูกกำหนดจากผู้เล่นนั้น ซึ่งจะเป็น R5 คนแรก',
      allianceName: 'ชื่อพันธมิตร',
      timezone: 'เขตเวลา',
      create: 'สร้างพันธมิตร',
    },
  },
} satisfies MessageCatalogue;

export default messages;
