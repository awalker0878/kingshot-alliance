...
import type { IntegrationPage } from './integrationPages';
...
<template>
  <nav class="ks-surface mt-4 p-4" :aria-label="label" :data-integration-catalogue="kind">
    <p class="text-sm text-[var(--ks-muted)]" aria-live="polite">
      {{ t('integrationExperience.recordCount', { count: page.total }) }}
    </p>
    ...
    <div v-if="failed" class="mt-3" role="alert">
      <p>{{ t('integrationExperience.historyUnavailable') }}</p>
      <button
        type="button"
      ...
      >
        {{ t('integrationExperience.retryPage') }}
      </button>
    </div>
  </nav>
</template>
