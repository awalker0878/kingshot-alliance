  ...
  >
    <details @toggle="loadEvidence()">
      <summary class="cursor-pointer list-none">
        ...
        <p class="mt-2 max-w-4xl text-sm text-[var(--ks-muted)]">{{ t('kingdomP7D.transferEvidenceHelp') }}</p>
      </summary>

      <div class="mt-5 grid gap-5">
        <form v-if="mutable" class="rounded-xl border border-[var(--ks-border)] bg-black/10 p-4" @submit.prevent="upload">
          <div class="grid gap-3 md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto] md:items-end">
            <label class="text-sm font-semibold">{{ t('kingdomP7D.screenshotClass') }}
              <select v-model="uploadKind" class="mt-1 w-full rounded-lg border border-[var(--ks-border)] bg-[var(--ks-bg)] px-3 py-2">
                <option v-for="schema in schemas" :key="schema.kind" :value="schema.kind">{{ kindLabel(schema.kind) }}</option>
              </select>
            </label>
            <label class="text-sm font-semibold">{{ t('kingdomP7D.uploadScreenshot') }}
              <input accept="image/jpeg,image/png,image/webp" class="mt-1 block w-full rounded-lg border border-[var(--ks-border)] bg-[var(--ks-bg)] px-3 py-2 text-sm" required type="file" @change="chooseFile" />
            </label>
            <button :disabled="!uploadFile" class="rounded-lg bg-[var(--ks-gold)] px-4 py-2 text-sm font-bold text-[var(--ks-ink)] disabled:opacity-40" type="submit">{{ t('kingdomP7D.uploadEvidence') }}</button>
          </div>
        ...
        <div v-if="loading" role="status" class="rounded-xl border border-[var(--ks-border)] p-4 text-sm">{{ t('kingdomP7D.evidenceLoading') }}</div>
        <div v-else-if="loadError" role="alert" class="rounded-xl border border-red-400/30 bg-red-500/10 p-4 text-sm text-red-100">{{ t('kingdomP7D.evidenceLoadFailed') }}</div>
        <div v-else-if="loaded && evidence.length === 0" class="rounded-xl border border-[var(--ks-border)] p-4 text-sm text-[var(--ks-muted)]">{{ t('kingdomP7D.noTransferEvidence') }}</div>

        <article v-for="item in evidence" :key="item.id" class="overflow-hidden rounded-xl border border-[var(--ks-border)] bg-black/10">
          <header class="flex flex-wrap items-start justify-between gap-3 p-4">
            <div>
              ...
              <p class="mt-1 text-xs text-[var(--ks-muted)]">
                {{ t('kingdomP7D.uploadTime') }} {{ timestamp(item.createdAt) }} · {{ t('kingdomP7D.processingStatus') }}: {{ statusLabel(item.status) }}
              </p>
            </div>
            <a v-if="item.hasImage" :href="`${basePath}/${item.id}/image`" class="text-sm font-semibold text-[var(--ks-gold-bright)] underline-offset-4 hover:underline" target="_blank">{{ t('kingdomP7D.evidenceImage') }}</a>
          </header>

          <div v-if="item.hasVisualDuplicate" class="border-y border-amber-400/20 bg-amber-500/10 px-4 py-3 text-sm text-amber-100">
            <strong>{{ t('kingdomP7D.visualDuplicate') }}</strong><p class="mt-1">{{ t('kingdomP7D.visualDuplicateHelp') }}</p>
          </div>
          ...
          <div class="grid gap-4 border-t border-[var(--ks-border)] p-4 lg:grid-cols-2">
            <section>
              ...
              <p v-if="classMismatch(item)" role="alert" class="mt-3 rounded-lg border border-red-400/30 bg-red-500/10 p-3 text-sm text-red-100">{{ t('kingdomP7D.classMismatch') }}</p>
            </section>
            <section v-if="schemaFor(item.expectedKind)">
              ...
              <p class="mt-1 text-xs text-[var(--ks-muted)]">{{ t('kingdomP7D.fixtureCorpus') }}: {{ schemaFor(item.expectedKind)!.fixtureCorpus }} · {{ t('kingdomP7D.fieldConfidence') }} ≥ {{ percent(schemaFor(item.expectedKind)!.minimumFieldConfidence) }}</p>
            </section>
          </div>
          ...
          <section v-if="item.extraction" class="border-t border-[var(--ks-border)] p-4">
            ...
            <div class="mt-3 grid gap-3 md:hidden">
              <article v-for="field in item.extraction.fields" :key="`mobile-${field.key}-${field.ordinal}`" class="rounded-lg border border-[var(--ks-border)] p-3 text-sm">
                <p class="font-semibold break-all">{{ field.key }}</p>
                <dl class="mt-3 grid gap-3">
                  <div><dt class="ks-kicker">{{ t('kingdomP7D.rawObservation') }}</dt><dd class="mt-1 break-words">{{ field.raw }}</dd></div>
                  <div><dt class="ks-kicker">{{ t('kingdomP7D.normalizedValue') }}</dt><dd class="mt-1">{{ displayValue(field.value) }}<span v-if="fieldCorrected(item, field)" class="ml-1 text-xs font-semibold text-amber-200">· {{ t('kingdomP7D.evidenceCorrected') }}</span></dd></div>
                  <div><dt class="ks-kicker">{{ t('kingdomP7D.fieldConfidence') }}</dt><dd class="mt-1">{{ percent(field.confidence) }}</dd></div>
                </dl>
                <p v-if="schemaFor(item.expectedKind) && field.confidence < schemaFor(item.expectedKind)!.minimumFieldConfidence" class="mt-3 text-xs text-amber-200">{{ t('kingdomP7D.belowSchemaConfidence') }}</p>
                <p v-for="warning in field.warnings" :key="warning" class="mt-2 text-xs text-amber-200">{{ warning }}</p>
              </article>
            </div>
            <div class="mt-3 hidden overflow-x-auto md:block">
              <table class="w-full text-left text-sm">
                <thead class="text-xs text-[var(--ks-muted)]"><tr><th class="pr-3 pb-2">{{ t('kingdomP7D.evidenceField') }}</th><th class="pr-3 pb-2">{{ t('kingdomP7D.rawObservation') }}</th><th class="pr-3 pb-2">{{ t('kingdomP7D.normalizedValue') }}</th><th class="pb-2">{{ t('kingdomP7D.fieldConfidence') }}</th></tr></thead>
                <tbody>
                  <tr v-for="field in item.extraction.fields" :key="`${field.key}-${field.ordinal}`" class="border-t border-[var(--ks-border)] align-top">
                    <td class="py-2 pr-3 font-semibold">{{ field.key }}</td><td class="py-2 pr-3 break-words">{{ field.raw }}</td>
                    <td class="py-2 pr-3">{{ displayValue(field.value) }}<span v-if="fieldCorrected(item, field)" class="ml-1 text-xs font-semibold text-amber-200">· {{ t('kingdomP7D.evidenceCorrected') }}</span></td>
                    <td class="py-2">{{ percent(field.confidence) }}<p v-if="schemaFor(item.expectedKind) && field.confidence < schemaFor(item.expectedKind)!.minimumFieldConfidence" class="mt-1 max-w-xs text-xs text-amber-200">{{ t('kingdomP7D.belowSchemaConfidence') }}</p><p v-for="warning in field.warnings" :key="warning" class="mt-1 text-xs text-amber-200">{{ warning }}</p></td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <section v-if="item.extraction && item.status !== 'unsupported' && item.status !== 'deleted'" class="border-t border-[var(--ks-border)] p-4">
            <div class="flex flex-wrap items-start justify-between gap-3">
              <div><h5 class="font-semibold">{{ t('kingdomP7D.reviewEvidence') }}</h5><p class="mt-1 text-sm text-[var(--ks-muted)]">{{ t('kingdomP7D.reviewBeforeCommit') }}</p></div>
              <span v-if="item.review" class="rounded-full border border-[var(--ks-border)] px-3 py-1 text-xs font-semibold">{{ reviewStatusLabel(item.review.status) }} · {{ t('kingdomP7D.evidenceRevision') }} {{ item.review.revision }}</span>
            </div>

            <form v-if="mutable && drafts[item.id]" class="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3" @submit.prevent="review(item)">
              <label class="text-sm font-semibold">{{ t('kingdomP7D.observationTime') }}<input v-model="drafts[item.id]!.observedAt" class="mt-1 w-full rounded-lg border border-[var(--ks-border)] bg-[var(--ks-bg)] px-3 py-2" required type="datetime-local" /></label>
              <label v-if="requiresValidity(item.expectedKind)" class="text-sm font-semibold">{{ t('kingdomP7D.freshnessBoundary') }}<input v-model="drafts[item.id]!.validUntil" class="mt-1 w-full rounded-lg border border-[var(--ks-border)] bg-[var(--ks-bg)] px-3 py-2" required type="datetime-local" /></label>

              <label v-if="item.expectedKind === 'transfer_governor_status'" class="text-sm font-semibold">{{ t('kingdomP7D.observation_governor_power') }}<input v-model="drafts[item.id]!.governorPower" class="mt-1 w-full rounded-lg border border-[var(--ks-border)] bg-[var(--ks-bg)] px-3 py-2" min="0" required type="number" /></label>
              ...
              <template v-else-if="item.expectedKind === 'transfer_invitation'">
                <label class="text-sm font-semibold">{{ t('kingdomP7D.invitationStatus') }}<select v-model="drafts[item.id]!.invitationStatus" class="mt-1 w-full rounded-lg border border-[var(--ks-border)] bg-[var(--ks-bg)] px-3 py-2" required><option value="none">{{ t('kingdomP7D.invitation_none') }}</option><option value="ordinary_received">{{ t('kingdomP7D.invitation_ordinary_received') }}</option><option value="special_pending">{{ t('kingdomP7D.invitation_special_pending') }}</option><option value="special_approved">{{ t('kingdomP7D.invitation_special_approved') }}</option></select></label>
              ...
              <template v-else-if="item.expectedKind === 'transfer_target_kingdom_rules'">
                ...
                <label class="text-sm font-semibold">{{ t('kingdomP7D.targetHeroGeneration') }}<input v-model="drafts[item.id]!.targetHeroGeneration" class="mt-1 w-full rounded-lg border border-[var(--ks-border)] bg-[var(--ks-bg)] px-3 py-2" min="0" required type="number" /></label>
                <label class="text-sm font-semibold">{{ t('kingdomP7D.targetTruegoldLevel') }}<input v-model="drafts[item.id]!.targetTruegoldLevel" class="mt-1 w-full rounded-lg border border-[var(--ks-border)] bg-[var(--ks-bg)] px-3 py-2" min="0" required type="number" /></label>
                <label class="text-sm font-semibold">{{ t('kingdomP7D.targetCharacterAgeThresholdDays') }}<input v-model="drafts[item.id]!.targetCharacterAgeThresholdDays" class="mt-1 w-full rounded-lg border border-[var(--ks-border)] bg-[var(--ks-bg)] px-3 py-2" min="90" max="180" required type="number" /></label>
                <label class="text-sm font-semibold">{{ t('kingdomP7D.kingdomClassification') }}<select v-model="drafts[item.id]!.kingdomClassification" class="mt-1 w-full rounded-lg border border-[var(--ks-border)] bg-[var(--ks-bg)] px-3 py-2"><option value="">{{ t('kingdomP7D.classificationNotProved') }}</option><option value="ordinary">{{ t('kingdomP7D.classification_ordinary') }}</option><option value="leading">{{ t('kingdomP7D.classification_leading') }}</option></select></label>
              </template>
              ...
              <div><button class="rounded-lg border border-[var(--ks-border)] px-4 py-2 text-sm font-semibold" type="submit">{{ t('kingdomP7D.reviewEvidence') }}</button></div>
            </form>
          </section>
          ...
          <section v-if="item.review" class="grid gap-4 border-t border-[var(--ks-border)] p-4 lg:grid-cols-2">
            <div>
              ...
              <dl class="mt-3 grid gap-2 text-sm">
                <div class="rounded-lg border border-[var(--ks-border)] p-3"><dt class="ks-kicker">{{ t('kingdomP7D.transferScore') }}</dt><dd class="mt-1 font-semibold">{{ displayValue(currentTransferScore.value) }}</dd><dd class="mt-1 text-xs text-[var(--ks-muted)]">{{ timestamp(currentTransferScore.observedAt) }}<span v-if="currentTransferScore.validUntil"> · {{ t('kingdomP7D.validUntil') }} {{ timestamp(currentTransferScore.validUntil) }}</span></dd></div>
                <div class="rounded-lg border border-[var(--ks-border)] p-3"><dt class="ks-kicker">{{ t('kingdomP7D.officialTransferGroup') }}</dt><dd class="mt-1 font-semibold">{{ currentOfficialGroup?.label ?? t('kingdomP7D.needsVerification') }}</dd><dd v-if="currentOfficialGroup" class="mt-1 text-xs text-[var(--ks-muted)]">{{ timestamp(currentOfficialGroup.observedAt) }} · {{ currentOfficialGroup.sourceReference }}</dd></div>
                <div class="rounded-lg border border-[var(--ks-border)] p-3"><dt class="ks-kicker">{{ t('kingdomP7D.powerCap') }}</dt><dd class="mt-1 font-semibold">{{ displayValue(currentTargetCondition?.powerCap) }}</dd><dd v-if="currentTargetCondition" class="mt-1 text-xs text-[var(--ks-muted)]">{{ timestamp(currentTargetCondition.observedAt) }} · {{ currentTargetCondition.sourceReference }}</dd></div>
              </dl>
              <details v-if="currentEligibility?.requirements.length" class="mt-3"><summary class="cursor-pointer text-sm font-semibold">{{ t('kingdomP7D.eligibilityRequirements') }}</summary><ul class="mt-2 grid gap-2 text-sm"><li v-for="requirement in currentEligibility.requirements" :key="requirement.key" class="rounded-lg border border-[var(--ks-border)] p-3"><div class="flex justify-between gap-3"><strong>{{ t(`kingdomP7D.requirementKey_${requirement.key}`) }}</strong><span>{{ t(`kingdomP7D.requirement_${requirement.state}`) }}</span></div><p class="mt-1 text-[var(--ks-muted)]">{{ requirement.explanation }}</p><p v-if="requirement.observedAt" class="mt-1 text-xs text-[var(--ks-muted)]">{{ timestamp(requirement.observedAt) }}<span v-if="requirement.validUntil"> · {{ t('kingdomP7D.validUntil') }} {{ timestamp(requirement.validUntil) }}</span></p></li></ul></details>
            </div>
            ...
            <div>
              ...
              <ul class="mt-3 grid gap-2 text-sm">
                ...
                <li v-if="item.review.targetHeroGeneration !== null" class="rounded-lg border border-[var(--ks-border)] p-3">{{ t('kingdomP7D.targetHeroGeneration') }}: <strong>{{ formatNumber(item.review.targetHeroGeneration) }}</strong></li>
                <li v-if="item.review.targetTruegoldLevel !== null" class="rounded-lg border border-[var(--ks-border)] p-3">{{ t('kingdomP7D.targetTruegoldLevel') }}: <strong>{{ formatNumber(item.review.targetTruegoldLevel) }}</strong></li>
                <li v-if="item.review.targetCharacterAgeThresholdDays !== null" class="rounded-lg border border-[var(--ks-border)] p-3">{{ t('kingdomP7D.targetCharacterAgeThresholdDays') }}: <strong>{{ formatNumber(item.review.targetCharacterAgeThresholdDays) }}</strong></li>
                <li v-if="item.review.kingdomClassification" class="rounded-lg border border-[var(--ks-border)] p-3">{{ t('kingdomP7D.kingdomClassification') }}: <strong>{{ t(`kingdomP7D.classification_${item.review.kingdomClassification}`) }}</strong></li>
          ...
          <section v-if="item.review?.status === 'duplicate_blocked'" class="border-t border-amber-400/20 bg-amber-500/10 p-4 text-sm">
            <strong class="text-amber-100">{{ t('kingdomP7D.semanticDuplicate') }}</strong><p class="mt-1 text-amber-50/90">{{ t('kingdomP7D.semanticDuplicateHelp') }}</p>
            <form v-if="mutable" class="mt-3 flex flex-col gap-2 sm:flex-row" @submit.prevent="resolveDuplicate(item)"><input v-model="duplicateJustifications[item.review.id]" :placeholder="t('kingdomP7D.duplicateJustification')" class="min-w-0 flex-1 rounded-lg border border-amber-400/30 bg-[var(--ks-bg)] px-3 py-2" minlength="8" required /><button class="rounded-lg border border-amber-300/40 px-4 py-2 font-semibold text-amber-50" type="submit">{{ t('kingdomP7D.resolveDuplicate') }}</button></form>
          </section>

          <section v-if="item.review?.status === 'approved'" class="border-t border-[var(--ks-border)] p-4">
            <button :disabled="previewLoading[item.id]" class="rounded-lg border border-[var(--ks-border)] px-4 py-2 text-sm font-semibold disabled:opacity-40" type="button" @click="preview(item)">{{ t('kingdomP7D.previewImpact') }}</button>
            <div v-if="previews[item.id]" class="mt-4 grid gap-3 md:grid-cols-2">
              <div class="rounded-xl border border-[var(--ks-border)] p-4"><p class="ks-kicker">{{ t('kingdomP7D.beforeImport') }}</p><p class="mt-1 font-semibold">{{ t(`kingdomP7D.eligibility_${previews[item.id]!.current_outcome}`) }}</p><p class="mt-2 text-sm text-[var(--ks-muted)]">{{ previews[item.id]!.current_primary_action ?? t('kingdomP7D.noRemainingActions') }}</p></div>
              <div class="rounded-xl border border-[var(--ks-gold)]/40 bg-[var(--ks-gold)]/5 p-4"><p class="ks-kicker">{{ t('kingdomP7D.afterImport') }}</p><p class="mt-1 font-semibold">{{ t(`kingdomP7D.eligibility_${previews[item.id]!.after_outcome}`) }}</p><p class="mt-2 text-sm text-[var(--ks-muted)]">{{ previews[item.id]!.after_primary_action ?? t('kingdomP7D.noRemainingActions') }}</p></div>
              <p class="text-xs text-[var(--ks-muted)] md:col-span-2">{{ t('kingdomP7D.reviewedFactKeys') }}: {{ previews[item.id]!.reviewed_fact_keys.join(', ') }}</p>
            </div>
            <div v-if="mutable && previews[item.id] && item.commit?.status !== 'succeeded'" class="mt-4"><button class="rounded-lg bg-[var(--ks-gold)] px-4 py-2 text-sm font-bold text-[var(--ks-ink)]" type="button" @click="commit(item)">{{ t('kingdomP7D.commitEvidence') }}</button></div>
          </section>

          <footer class="flex flex-wrap items-center justify-between gap-3 border-t border-[var(--ks-border)] p-4 text-sm">
            <div><span v-if="item.commit"><strong>{{ commitStatusLabel(item.commit.status) }}</strong><span v-if="item.commit.destinationReceiptId"> · {{ t('kingdomP7D.destinationReceipt') }} {{ item.commit.destinationReceiptId }}</span></span><span v-else-if="item.status === 'unsupported'" class="text-amber-100">{{ t('kingdomP7D.unsupportedEvidence') }}</span></div>
            <div v-if="mutable" class="flex flex-wrap gap-2"><button v-if="item.status === 'failed'" class="rounded-lg border border-[var(--ks-border)] px-3 py-2 font-semibold" type="button" @click="retry(item)">{{ t('kingdomP7D.retryProcessing') }}</button><button v-if="!['classifying', 'extracting', 'committing', 'deleted'].includes(item.status)" class="rounded-lg border border-red-400/30 px-3 py-2 text-red-200" type="button" @click="remove(item)">{{ t('kingdomP7D.deleteEvidence') }}</button></div>
          </footer>
        </article>
