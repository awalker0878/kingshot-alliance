...
import { TransferChoiceLoader } from './transferChoices';
import type { TransferChoice, TransferChoiceKind, TransferChoiceView } from './transferChoices';
...
const { t } = useLocale();
const search = ref('');
const chosen = ref<TransferChoice | null>(null);
const view = ref<TransferChoiceView>({
  result: null,
  busy: false,
  failed: false,
  search: '',
...
<template>
  <div class="space-y-2" data-testid="transfer-choice-picker">
    ...
    >
      ...
      <option v-if="offPage" :value="modelValue">
        {{ selected?.name ?? selectedName ?? t('kingdomP7D.choiceUnavailable') }}
      </option>
    ...
    <div class="flex gap-2">
      <input
        ...
        maxlength="160"
        :disabled="disabled"
        :aria-label="t('kingdomP7D.searchChoices')"
    ...
    <div v-if="view.failed" role="alert" class="text-sm">
      <p>{{ t('kingdomP7D.choicesFailed') }}</p>
      <button
        type="button"
        class="ks-command-link"
        :disabled="disabled || view.busy"
