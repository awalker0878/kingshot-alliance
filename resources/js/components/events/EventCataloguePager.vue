...
function navigate(cursor: string | null): void {
  ...
  if (props.occurrence) url.searchParams.set('occurrence', props.occurrence);
  router.get(
...
<template>
  <nav class="ks-surface mt-4 p-4" :aria-label="label" :data-event-catalogue="kind">
    <p class="text-sm text-[var(--ks-muted)]" aria-live="polite">
      {{ t('events.pagination.recordCount', { count: page.total }) }}
    </p>
    ...
    <div v-if="failed" class="mt-3" role="alert">
      <p>{{ t('events.pagination.unavailable') }}</p>
      <button
        type="button"
