...
<template>
  ...
  >
    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
      ...
      <label
        v-if="command.occurrences.length > 1"
        class="text-sm font-medium text-slate-700 dark:text-slate-200"
      >
        <span class="mb-1 block">{{ t('events.command.occurrence') }}</span>
        <select
          class="min-w-56 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 focus:ring-2 focus:ring-sky-500 focus:outline-none dark:border-slate-600 dark:bg-slate-800 dark:text-white"
          :value="command.selectedOccurrenceId ?? ''"
          @change="selectOccurrence"
        >
          <option
            v-for="occurrence in command.occurrences"
            :key="occurrence.id"
            :value="occurrence.id"
          >
    ...
    <template v-else>
      <div class="mt-5 flex flex-wrap gap-2" aria-live="polite">
        ...
        >
          {{
            phase === 'closeout'
              ? t('events.command.closeoutClear')
              : t('events.command.readinessClear')
          }}
        </span>
      </div>
      ...
      <ul
        ...
        :aria-label="t('events.command.attentionItems')"
      >
        <li
        ...
        >
          <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            ...
            <a
              ...
              class="inline-flex shrink-0 items-center justify-center rounded-lg border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-800 hover:bg-slate-50 focus:ring-2 focus:ring-sky-500 focus:outline-none dark:border-slate-600 dark:text-slate-100 dark:hover:bg-slate-800"
            >
      ...
      <details
        v-if="settledSections.length > 0"
        class="mt-5 rounded-xl border border-slate-200 dark:border-slate-700"
      >
        <summary
        ...
        <div class="border-t border-slate-200 px-4 py-3 dark:border-slate-700">
          <div
            v-for="section in settledSections"
            :key="section.key"
