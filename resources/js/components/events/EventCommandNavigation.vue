...
<template>
  <nav class="ks-tab-strip mt-4" aria-label="Event organizer command sections">
    ...
    <a v-if="has('phases')" href="#phases" class="ks-tab">{{ t('events.phases.manageTitle') }}</a>
    <a v-if="has('polls')" href="#polls" class="ks-tab">{{ t('events.polls.manageTitle') }}</a>
    <a v-if="has('rosters')" href="#rosters" class="ks-tab">{{ t('events.rosters.manageTitle') }}</a>
    <a v-if="has('rally_guidance') || has('formations')" href="#rallies" class="ks-tab">
      {{ t('events.rallies.manageTitle') }}
    </a>
    <a v-if="has('objectives')" href="#battle-plan" class="ks-tab">
