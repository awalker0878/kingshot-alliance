...
import type { PlatformPage } from './platformPages';
...
<template>
  <nav class="ks-surface mt-4 p-4" :aria-label="label" :data-platform-catalogue="kind">
    <p class="text-sm text-[var(--ks-muted)]" aria-live="polite">
      {{ t('platformAdmin.recordCount', { count: page.total }) }}
    </p>
    ...
    <div v-if="failed" class="mt-3" role="alert">
      <p>{{ t('platformAdmin.historyUnavailable') }}</p>
      <button
        type="button"
