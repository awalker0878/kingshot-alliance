...
export function validatePlacement(map: MapData, objects: PlanObject[], preferences: PlanningPreferences): ValidationResult {
  ...
  for (const object of objects) {
    ...
    if (!rect) {
      violations.push(issue('unknown_object_type', 'This object type is not supported by the selected map dataset.', object.key));
      continue;
    }
    ...
    for (const structure of map.structures) {
      ...
      const actual = { x: structure.x, y: structure.y, width: structure.size, height: structure.size };
      const forbidden = {
      ...
      if (intersects(rect, actual) || (intersects(rect, forbidden) && !(object.type === 'governor_city' && structure.city_exempt))) {
        violations.push(issue('structure_exclusion', 'The object overlaps a fixed structure or its no-build zone.', object.key));
        break;
      }
    }
  ...
  entries.forEach(([key, rect], index) => {
  ...
  if (radius && radius > 0) {
    ...
    objects.filter((object) => object.type === 'bear_trap').forEach((trap) => {
      if (!traps.has(trap.alliance_key)) traps.set(trap.alliance_key, trap);
    });
    objects.filter((object) => object.type === 'governor_city').forEach((city) => {
      const trap = traps.get(city.alliance_key);
      if (trap && Math.hypot(city.x - trap.x, city.y - trap.y) > radius) {
        warnings.push(issue('preferred_bear_radius', 'This Governor city is outside the plan preferred Bear Trap radius.', city.key));
      }
    });
  }
...
export function analyzeLayout(map: MapData, objects: PlanObject[], preferences: PlanningPreferences): Record<string, AllianceAnalysis> {
  const groups = new Map<string, PlanObject[]>();
  objects.forEach((object) => groups.set(object.alliance_key, [...(groups.get(object.alliance_key) ?? []), object]));
  const result: Record<string, AllianceAnalysis> = {};

  for (const [allianceKey, allianceObjects] of groups) {
    const counts: Record<string, number> = {};
    allianceObjects.forEach((object) => { counts[object.type] = (counts[object.type] ?? 0) + 1; });
    ...
    const sources = allianceObjects.flatMap((object) => {
      ...
      return [{ x: object.x + definition.size / 2, y: object.y + definition.size / 2, coverage: definition.coverage }];
    });
    const covered = cities.filter((city) => {
      const size = map.object_types.governor_city.size;
      const corners = [[city.x, city.y], [city.x + size, city.y], [city.x, city.y + size], [city.x + size, city.y + size]];
      return corners.every(([x, y]) => sources.some((source) => Math.abs((x ?? 0) - source.x) <= source.coverage && Math.abs((y ?? 0) - source.y) <= source.coverage));
    }).length;
    ...
    sources.forEach((source, start) => {
      ...
      while (queue.length) {
        ...
        sources.forEach((candidate, candidateIndex) => {
          ...
          const distance = Math.max(Math.abs(current.x - candidate.x), Math.abs(current.y - candidate.y));
    ...
    result[allianceKey] = {
      ...
      estimated_march_seconds: seconds ? stats(distances.map((distance) => distance * seconds)) : null,
