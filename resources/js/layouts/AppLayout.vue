...
const rooms: NavigationItem[] = [
  ...
  { key: 'contributions', href: '/alliance/contributions', icon: 'contributions', allianceScoped: true },
  ...
  { key: 'integrations', href: '/alliance/integrations', icon: 'integrations', allianceScoped: true },
];
...
<template>
  ...
  <div class="min-h-screen bg-transparent text-[var(--ks-text)]">
    ...
    >
      <div class="relative overflow-hidden border-b border-[var(--ks-border)] px-5 pb-5 pt-6">
        <div class="pointer-events-none absolute -end-16 -top-20 h-52 w-52 rounded-full bg-[var(--ks-gold-soft)] blur-3xl" />
        <Link href="/dashboard" class="relative flex items-center gap-3">
          <div class="grid h-14 w-12 place-items-center border border-[var(--ks-gold-dark)] bg-[linear-gradient(160deg,#163f3c,#071a19)] [clip-path:polygon(50%_0,95%_16%,86%_77%,50%_100%,14%_77%,5%_16%)]">
            <span class="font-[var(--ks-font-display)] text-xl text-[var(--ks-gold-bright)]">♛</span>
          </div>
          <div>
            <div class="font-[var(--ks-font-display)] text-[1.72rem] font-semibold leading-none tracking-[.06em] text-[var(--ks-gold-bright)]">
              KINGSHOT
            </div>
            <div class="mt-1 text-[.61rem] font-extrabold tracking-[.19em] text-[var(--ks-muted)] uppercase">
      ...
      <nav class="min-h-0 flex-1 overflow-y-auto px-3 pb-5" :aria-label="t('navigation.allianceOperations')">
        <div class="mb-2 px-3 text-[.62rem] font-extrabold tracking-[.18em] text-[var(--ks-gold)] uppercase">
          {{ t('navigation.allianceOperations') }}
        </div>
        <div class="space-y-1">
          <template v-for="room in rooms" :key="room.href">
            ...
            <Link
              v-else
              :href="room.href"
              class="group relative flex min-h-11 items-center gap-3 overflow-hidden rounded-[var(--ks-radius-sm)] border px-3 py-2 font-[var(--ks-font-display)] text-[.9rem] transition"
              :class="
                isActive(room)
            ...
            >
              <NavIcon
                ...
                :class="isActive(room) ? 'text-[var(--ks-gold-bright)]' : 'text-[var(--ks-gold)] opacity-80'"
              />
    ...
    >
      ...
      <div class="min-w-0 flex-1">
        <div class="truncate font-[var(--ks-font-display)] text-base font-semibold text-[var(--ks-gold-bright)]">
          {{ playerAllianceName ?? 'KINGSHOT' }}
        </div>
        <div class="truncate text-[.65rem] text-[var(--ks-muted)]">
          {{ activePlayer?.name ?? user.name }}<template v-if="activePlayer?.kingdomNumber"> · K{{ activePlayer.kingdomNumber }}</template>
        </div>
      </div>
    ...
    <div v-if="mobileOpen" class="fixed inset-0 z-[100] xl:hidden">
      ...
      >
        <div class="flex items-center justify-between border-b border-[var(--ks-border)] p-4">
          <div>
            <div class="font-[var(--ks-font-display)] text-2xl text-[var(--ks-gold-bright)]">KINGSHOT</div>
            <div class="mt-1 text-[.62rem] font-bold tracking-[.16em] text-[var(--ks-muted)] uppercase">
              {{ t('navigation.allianceOperations') }}
            </div>
          </div>
          <button
        ...
        <nav class="min-h-0 flex-1 overflow-y-auto p-3" :aria-label="t('navigation.allianceOperations')">
          <div class="space-y-1">
            <template v-for="room in rooms" :key="room.href">
              <span
                v-if="isDisabled(room)"
              ...
              <Link
                ...
                :class="isActive(room) ? 'border-[rgba(32,178,163,.38)] bg-[var(--ks-teal-soft)] text-[var(--ks-ivory)]' : 'border-transparent text-[var(--ks-text-secondary)]'"
                @click="mobileOpen = false"
              >
    ...
    <div class="xl:ps-[18.5rem]">
        ...
        <div class="flex min-w-0 flex-1 items-center gap-4">
          ...
          <div class="min-w-0">
            ...
            <div class="mt-1 flex min-w-0 flex-wrap items-baseline gap-x-3 gap-y-1">
              <strong class="truncate font-[var(--ks-font-display)] text-xl font-semibold text-[var(--ks-ivory)]">
                {{ activePlayer?.name ?? user.name }}
              </strong>
              <span v-if="playerAllianceName" class="truncate text-sm text-[var(--ks-text-secondary)]">{{ playerAllianceName }}</span>
              <span v-if="activePlayer?.kingdomNumber" class="text-sm font-semibold text-[var(--ks-teal-bright)]">K{{ activePlayer.kingdomNumber }}</span>
            </div>
          </div>
        </div>
      ...
      <main id="main-content" class="relative min-h-screen">
        <div class="mx-auto w-full max-w-[112rem] px-3 py-4 sm:px-5 sm:py-5 lg:px-6 xl:px-7 xl:py-6">
          <slot />
        </div>
      </main>
    </div>
