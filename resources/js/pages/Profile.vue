  ...
  <main class="mx-auto min-h-screen max-w-4xl px-6 py-12 lg:px-8">
    ...
    <section class="mt-8 rounded-2xl border border-slate-800 bg-slate-900/70 p-8">
      ...
      <form class="mt-6 grid gap-5 md:grid-cols-2" @submit.prevent="updateProfile">
        ...
        <div class="flex items-end">
          <button class="rounded-lg bg-cyan-300 px-4 py-2 font-semibold text-slate-950 disabled:opacity-60" :disabled="profileForm.processing" type="submit">
            Save profile
          </button>
        </div>
      </form>
      ...
      <p class="mt-4 text-xs text-slate-500">
        Email verification: {{ props.user.emailVerified ? 'verified' : 'pending' }} · MFA foundation: {{ props.user.twoFactorEnabled ? 'enabled' : 'not enabled' }}
      </p>
    </section>
    ...
    <section class="mt-8 rounded-2xl border border-slate-800 bg-slate-900/70 p-8">
      ...
      <form class="mt-6 grid gap-5" @submit.prevent="updatePassword">
        ...
        <button class="w-fit rounded-lg bg-cyan-300 px-4 py-2 font-semibold text-slate-950 disabled:opacity-60" :disabled="passwordForm.processing" type="submit">
          Update password
        </button>
      </form>
    </section>
    ...
    <section class="mt-8 rounded-2xl border border-slate-800 bg-slate-900/70 p-8">
      ...
      <form class="mt-6 flex flex-col gap-3 sm:flex-row" @submit.prevent="revokeOtherSessions">
        ...
        <button class="rounded-lg border border-rose-800 px-4 py-2 font-semibold text-rose-200 disabled:opacity-60" :disabled="sessionsForm.processing" type="submit">
