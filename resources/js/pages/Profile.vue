...
<template>
  ...
  <main class="mx-auto min-h-screen max-w-4xl px-6 py-12 lg:px-8">
    ...
    <p
      v-if="props.status"
      class="mt-6 rounded-lg border border-emerald-800 bg-emerald-950/30 p-4 text-sm text-emerald-100"
    >
      {{ props.status }}
    </p>
    ...
    <section class="mt-8 rounded-2xl border border-slate-800 bg-slate-900/70 p-8">
      ...
      <form class="mt-6 grid gap-5 md:grid-cols-2" @submit.prevent="updateProfile">
        <div>
          ...
          <input
            ...
            type="text"
          />
          <p v-if="profileForm.errors.name" class="mt-1 text-sm text-rose-300">
        ...
        <div>
          ...
          <input
            ...
            type="email"
          />
          <p v-if="profileForm.errors.email" class="mt-1 text-sm text-rose-300">
        ...
        <div>
          ...
          <input
            ...
            type="text"
          />
          <p v-if="profileForm.errors.timezone" class="mt-1 text-sm text-rose-300">
      ...
      <p class="mt-4 text-xs text-slate-500">
        Email verification: {{ props.user.emailVerified ? 'verified' : 'pending' }} · Two-factor
        authentication:
        {{
          props.user.twoFactorEnabled
            ? 'enabled'
            : props.user.twoFactorPending
              ? 'setup pending'
              : 'not enabled'
        }}
      </p>
    ...
    <section class="mt-8 rounded-2xl border border-slate-800 bg-slate-900/70 p-8">
      ...
      <p class="mt-2 text-sm text-slate-400">
        Protect sign-in with a TOTP authenticator. Recovery codes are shown only when created or
        regenerated.
      </p>

      <button
        v-if="!props.user.twoFactorEnabled && !props.twoFactorSetup"
        class="mt-6 rounded-lg bg-cyan-300 px-4 py-2 font-semibold text-slate-950"
        type="button"
      ...
      <div v-if="props.twoFactorSetup" class="mt-6 rounded-xl border border-slate-700 p-5">
        ...
        <p class="mt-2 font-mono text-sm break-all text-cyan-200">
          {{ props.twoFactorSetup.secret }}
        </p>
        <p class="mt-4 text-xs text-slate-500">Provisioning URI</p>
        <p class="mt-1 font-mono text-xs break-all text-slate-400">
        ...
        <form class="mt-5 flex flex-col gap-3 sm:flex-row" @submit.prevent="confirmTwoFactor">
          <input
            ...
            class="flex-1 rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 font-mono"
            inputmode="numeric"
            maxlength="6"
            placeholder="123456"
            required
        ...
        <p v-if="twoFactorForm.errors.code" class="mt-2 text-sm text-rose-300">
      ...
      <div
        ...
        class="mt-6 rounded-xl border border-amber-800 bg-amber-950/20 p-5"
      >
        <p class="font-semibold text-amber-100">Save these recovery codes now</p>
        <p class="mt-1 text-sm text-amber-200/80">
          Each code works once. Store them somewhere separate from this account.
        </p>
      ...
      <div v-if="props.user.twoFactorEnabled" class="mt-6 flex flex-wrap gap-3">
        ...
        <button
          class="rounded-lg border border-rose-800 px-4 py-2 text-sm font-semibold text-rose-200"
          type="button"
    ...
    <section class="mt-8 rounded-2xl border border-slate-800 bg-slate-900/70 p-8">
      ...
      <p class="mt-2 text-sm text-slate-400">
        Changing your password revokes personal access tokens and invalidates other authenticated
        sessions.
      </p>
      ...
      <form class="mt-6 grid gap-5" @submit.prevent="updatePassword">
        <div>
          ...
          <input
            ...
            type="password"
          />
          <p v-if="passwordForm.errors.current_password" class="mt-1 text-sm text-rose-300">
        ...
        <div>
          ...
          <input
            ...
            type="password"
          />
          <p v-if="passwordForm.errors.password" class="mt-1 text-sm text-rose-300">
        ...
        <button
          class="w-fit rounded-lg bg-cyan-300 px-4 py-2 font-semibold text-slate-950 disabled:opacity-60"
    ...
    <section class="mt-8 rounded-2xl border border-slate-800 bg-slate-900/70 p-8">
      ...
      <p class="mt-2 text-sm text-slate-400">
        Revoke every authenticated session except this device.
      </p>
      ...
      <form class="mt-6 flex flex-col gap-3 sm:flex-row" @submit.prevent="revokeOtherSessions">
        <input
          ...
          autocomplete="current-password"
          class="flex-1 rounded-lg border border-slate-700 bg-slate-950 px-3 py-2"
          placeholder="Current password"
          required
          type="password"
        />
        <button
          class="rounded-lg border border-rose-800 px-4 py-2 font-semibold text-rose-200 disabled:opacity-60"
      ...
      <p v-if="sessionsForm.errors.password" class="mt-2 text-sm text-rose-300">
