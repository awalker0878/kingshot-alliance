    ...
    <div class="mx-auto max-w-7xl space-y-6 px-4 py-8 sm:px-6 lg:px-8">
      <header class="space-y-2">
        ...
        <div class="flex flex-wrap items-baseline gap-2">
          ...
          <span v-if="organization.secondaryLabel" class="text-sm text-slate-400">{{
            organization.secondaryLabel
          }}</span>
        </div>
        <p class="max-w-3xl text-sm text-slate-300">
          History is owned by the original Event target. Current authority controls access, while
          participant names, represented Alliances, and Kingdom context remain frozen at the
      ...
      <section v-if="intelligence.series.length" class="space-y-3">
        ...
        <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          <article
            ...
            class="rounded-xl border border-white/10 bg-slate-950/60 p-4"
          >
            ...
            <div class="mt-3 grid grid-cols-3 gap-2 text-sm">
              ...
              <div>
                <span class="text-slate-500">Samples</span>
                <p class="font-semibold text-white">{{ formatNumber(series.samples) }}</p>
              </div>
            </div>
          ...
          >
            ...
            <ol class="mt-3 space-y-2">
              <li
                ...
                :key="entry.playerId"
                class="flex items-center justify-between gap-3 rounded-lg bg-white/5 px-3 py-2 text-sm"
              >
      ...
      <template v-else>
        ...
        >
          <div class="flex flex-wrap items-start justify-between gap-4">
            <div>
              <div
                class="flex flex-wrap gap-2 text-[11px] font-semibold tracking-wide text-slate-400 uppercase"
              >
            ...
            <div
              v-if="event.result"
              class="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-right"
            >
              <p class="text-xs tracking-wide text-slate-500 uppercase">Result</p>
              <p class="mt-1 text-lg font-semibold text-white">{{ number(event.result.score) }}</p>
              <p class="text-xs text-slate-400 capitalize">{{ humanize(event.result.outcome) }}</p>
            </div>
          </div>
          ...
          >
            <article class="rounded-xl border border-white/10 bg-white/5 p-4">
              <p class="text-xs tracking-wide text-slate-500 uppercase">Attendance</p>
            ...
            <article class="rounded-xl border border-white/10 bg-white/5 p-4">
              <p class="text-xs tracking-wide text-slate-500 uppercase">Roster</p>
              <p class="mt-1 text-xl font-semibold text-white">
            ...
            <article class="rounded-xl border border-white/10 bg-white/5 p-4">
              <p class="text-xs tracking-wide text-slate-500 uppercase">Rally assignments</p>
              <p class="mt-1 text-xl font-semibold text-white">
            ...
            <article class="rounded-xl border border-white/10 bg-white/5 p-4">
              <p class="text-xs tracking-wide text-slate-500 uppercase">Objectives</p>
              <p class="mt-1 text-xl font-semibold text-white">
              ...
              >
                <div class="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p v-if="result.allianceTag" class="text-xs text-slate-500">
                      {{ result.allianceTag }}
                    </p>
                  </div>
                  <p class="font-semibold text-amber-200">{{ number(result.score) }}</p>
                </div>
                <p class="mt-2 text-xs text-slate-400 capitalize">{{ humanize(result.outcome) }}</p>
              </article>
            </div>
          </div>
          ...
          <div class="mt-6">
            ...
            <div
              ...
              class="mt-3 overflow-x-auto rounded-xl border border-white/10"
            >
              <table class="min-w-full divide-y divide-white/10 text-sm">
                <thead class="bg-white/5 text-left text-xs tracking-wide text-slate-500 uppercase">
                  <tr>
            ...
            <p v-else class="mt-3 text-sm text-slate-500">
              No Player context was captured for this occurrence.
            </p>
          </div>
        </section>
      </template>
