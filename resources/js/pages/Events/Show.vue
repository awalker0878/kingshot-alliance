        ...
        <div class="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div class="mb-2 flex flex-wrap items-center gap-2">
              <span
                class="rounded-full border border-[var(--ks-border)] px-2 py-1 text-xs uppercase"
                >{{ t(`events.scope.${event.scope}`) }}</span
              ><span class="text-sm text-[var(--ks-text-muted)]">{{ event.targetLabel }}</span>
            </div>
            <h1 class="ks-display text-3xl font-semibold">{{ title }}</h1>
          ...
          <Link
            ...
            class="rounded-[var(--ks-radius-sm)] bg-[var(--ks-gold)] px-4 py-2 text-sm font-bold text-slate-950"
            >{{ t('events.show.manage') }}</Link
          >
        </div>
      </header>
      ...
      <section v-if="event.operations.polls.length" class="mt-5 space-y-4">
        ...
        >
          ...
          <button
            ...
            type="button"
            class="mt-4 rounded bg-[var(--ks-gold)] px-4 py-2 text-sm font-bold text-slate-950"
