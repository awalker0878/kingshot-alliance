...
const filteredAssignments = computed(() =>
  props.assignments.filter((assignment) => {
    ...
    return (
      (!roleFilter.value || assignment.role.id === roleFilter.value) &&
      (!term ||
        assignment.player.name.toLowerCase().includes(term) ||
        (assignment.player.gamePlayerId ?? '').toLowerCase().includes(term))
    );
  }),
);
const form = useForm({
  player_id: props.players[0]?.id ?? '',
  role_id: activeRoles.value[0]?.id ?? '',
  effective_from: '',
  expires_at: '',
  reason: '',
});
      ...
      <div class="space-y-5">
        <section class="ks-surface p-5">
          ...
          <form class="mt-5 space-y-4" @submit.prevent="assignRole">
            <label class="block text-sm font-semibold"
              >{{ t('governanceExpansion.governor')
            ...
            >
            <label class="block text-sm font-semibold"
              >{{ t('governanceExpansion.role')
              }}<select v-model="form.role_id" class="ks-input mt-2">
        ...
        <section class="ks-surface p-5">
          ...
          <form class="mt-4 space-y-3" @submit.prevent="handoff">
            <select v-model="handoffForm.player_id" class="ks-input">
              <option v-for="player in players" :key="player.id" :value="player.id">
                {{ player.name }}
              </option>
            </select>
      ...
      <div class="space-y-5">
        <section class="ks-surface p-5">
          <div class="flex flex-wrap gap-3">
            <input
              v-model="search"
              class="ks-input min-w-64 flex-1"
              :placeholder="t('governanceExpansion.search')"
            /><select v-model="roleFilter" class="ks-input max-w-64">
              <option value="">{{ t('governanceExpansion.allRoles') }}</option>
              <option v-for="role in activeRoles" :key="role.id" :value="role.id">
                {{ role.name }}
              </option>
            </select>
        ...
        <section class="ks-surface p-5">
          ...
          <div class="mt-4 grid gap-3 sm:grid-cols-2">
            <select v-model="bulkRoleId" class="ks-input">
              <option v-for="role in activeRoles" :key="role.id" :value="role.id">
                {{ role.name }}
              </option></select
            ><select v-model="bulkOperation" class="ks-input">
              <option value="assign">{{ t('governanceExpansion.assign') }}</option>
              <option value="remove">{{ t('governanceExpansion.remove') }}</option>
            </select>
          </div>
          <div class="mt-3 max-h-56 overflow-y-auto rounded border border-[var(--ks-border)] p-3">
            <label v-for="player in players" :key="player.id" class="flex gap-2 py-1 text-sm"
              ><input v-model="selectedPlayers" type="checkbox" :value="player.id" />{{
                player.name
              }}</label
            >
          </div>
          ...
          <div class="mt-3 flex gap-3">
            <button
              ...
              :disabled="previewing || selectedPlayers.length === 0"
              @click="previewBulk"
            >
              {{ t('governanceExpansion.preview') }}</button
            ><button v-if="bulkPreview" class="ks-command-button" @click="commitBulk">
          ...
          <div v-if="bulkPreview" class="mt-3 text-sm">
            ...
            >
              <li v-for="(reason, playerId) in bulkPreview.ineligible" :key="playerId">
                {{ players.find((p) => p.id === playerId)?.name ?? playerId }} — {{ reason }}
              </li>
            </ul>
          </div>
        </section>
