...
const selectedPlayerIds = ref<string[]>(
  props.governors.filter((item) => item.gamePlayerId).map((item) => item.id),
);
...
const currentItem = computed(
  () =>
    props.session?.items.find((item) => ['ready', 'awaiting_confirmation'].includes(item.state)) ??
    null,
);
...
<template>
  ...
  <AppLayout :user="user">
    ...
    <section class="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      <StatSeal
        :label="t('giftCodes.workspace.views.ready')"
        :value="formatNumber(counts.ready ?? 0)"
        icon="◆"
      />
      <StatSeal
        :label="t('giftCodes.workspace.views.expiring')"
        :value="formatNumber(counts.expiring ?? 0)"
        icon="!"
      />
      <StatSeal
        :label="t('giftCodes.workspace.views.retry_ready')"
        :value="formatNumber(counts.retry_ready ?? 0)"
        icon="↻"
      />
      <StatSeal
        :label="t('giftCodes.workspace.views.completed')"
    ...
    <section
      ...
      aria-labelledby="gift-code-run-heading"
    >
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h2 id="gift-code-run-heading" class="ks-display mt-1 text-2xl font-semibold">
            {{
              t('giftCodes.workspace.runProgress', {
      ...
      <div
        class="mt-4 h-2 overflow-hidden rounded-full bg-[var(--ks-surface-inset)]"
        aria-hidden="true"
      >
        <div
          class="h-full bg-[var(--ks-gold-bright)] transition-all"
          :style="{
            width: `${session.totalItems ? Math.round(((session.completedItems + session.skippedItems) / session.totalItems) * 100) : 0}%`,
          }"
        />
      </div>
      ...
      <ActionNotice
        v-if="sessionDone"
        class="mt-4"
        :message="t('giftCodes.workspace.runComplete')"
        tone="success"
      />
      <ActionNotice
      ...
      <article
        v-if="currentItem"
        class="mt-5 rounded-[var(--ks-radius-md)] border border-[var(--ks-border)] p-4 sm:p-5"
      >
        <div class="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p class="ks-kicker">
              {{
                t('giftCodes.workspace.itemStep', {
                  current: currentItem.sequence,
                  total: session.totalItems,
                })
              }}
            </p>
            <h3
              class="mt-1 font-mono text-xl font-bold tracking-[.08em] text-[var(--ks-gold-bright)]"
            >
              {{ currentItem.code }}
            </h3>
            <p class="mt-2 text-sm text-[var(--ks-muted)]">
              {{ t('giftCodes.workspace.governor') }}: {{ currentItem.playerName }}
              <template v-if="currentItem.kingdomNumber">
                · {{ t('giftCodes.kingdom', { kingdom: currentItem.kingdomNumber }) }}</template
              >
            </p>
          </div>
          <span class="ks-status" :data-tone="statusTone(currentItem.state)">{{
            t(`giftCodes.workspace.states.${currentItem.state}`)
          }}</span>
        </div>
        ...
        <div class="mt-4">
          ...
          <ul v-if="currentItem.reward.items.length" class="mt-2 grid gap-2 sm:grid-cols-2">
            <li
              v-for="(reward, index) in currentItem.reward.items"
              :key="`${reward.type}:${reward.key}:${index}`"
              class="rounded-[var(--ks-radius-sm)] border border-[var(--ks-border)] p-3 text-sm"
            >
              {{ rewardLabel(reward) }}
            </li>
          </ul>
          <p v-else class="mt-2 text-sm text-[var(--ks-muted)]">
            {{ t('giftCodes.workspace.rewardUnknown') }}
          </p>
        </div>
        ...
        <div class="mt-4 flex flex-wrap gap-2">
          <AppButton
            type="button"
            variant="secondary"
            :disabled="!currentItem.gamePlayerId"
            @click="copyValue(`player:${currentItem.id}`, currentItem.gamePlayerId ?? '')"
          >
            {{
              copied === `player:${currentItem.id}`
                ? t('giftCodes.copied')
                : t('giftCodes.workspace.copyPlayerId')
            }}
          </AppButton>
          <AppButton
            type="button"
            variant="secondary"
            @click="copyValue(`code:${currentItem.id}`, currentItem.code)"
          >
            {{
              copied === `code:${currentItem.id}`
                ? t('giftCodes.copied')
                : t('giftCodes.workspace.copyCode')
            }}
          </AppButton>
        ...
        <div v-if="currentItem.state === 'ready'" class="mt-4 flex flex-wrap gap-2">
          <AppButton type="button" :busy="busy" @click="prepare(currentItem)">{{
            t('giftCodes.workspace.prepareHandoff')
          }}</AppButton>
          <AppButton type="button" variant="secondary" :busy="busy" @click="skip(currentItem)">{{
            t('giftCodes.workspace.skip')
          }}</AppButton>
        </div>

        <div
          v-else-if="currentItem.state === 'awaiting_confirmation'"
          class="mt-4 grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto_auto]"
        >
          <select
            v-model="selectedOutcome"
            class="ks-input"
            :aria-label="t('giftCodes.workspace.observedOutcome')"
          >
            <option v-for="outcome in outcomes" :key="outcome" :value="outcome">
              {{ t(`giftCodes.workspace.outcomes.${outcome}`) }}
            </option>
          </select>
          <AppButton type="button" :busy="busy" @click="record(currentItem)">{{
            t('giftCodes.workspace.recordContinue')
          }}</AppButton>
          <AppButton type="button" variant="secondary" :busy="busy" @click="skip(currentItem)">{{
            t('giftCodes.workspace.skip')
          }}</AppButton>
        ...
        <div class="mt-4 border-t border-[var(--ks-border)] pt-4">
          ...
          <p v-if="currentSignal" class="mt-2 text-sm text-[var(--ks-muted)]">
            {{
              t('giftCodes.workspace.signalSummary', {
                successRate: currentSignal.successRate,
                samples: currentSignal.sampleCount,
                accounts: currentSignal.distinctAccounts,
              })
            }}
          </p>
          <p v-else class="mt-2 text-sm text-[var(--ks-muted)]">
            {{ t('giftCodes.workspace.signalPrivate') }}
          </p>
        </div>
      </article>
    </section>
    ...
    <section class="ks-surface mt-5 p-5 sm:p-6" aria-labelledby="gift-code-run-builder-heading">
      ...
      <h2 id="gift-code-run-builder-heading" class="ks-display mt-1 text-2xl font-semibold">
        {{ t('giftCodes.workspace.createRun') }}
      </h2>

      <div class="mt-4 flex flex-wrap gap-2">
        <AppButton type="button" :busy="busy" @click="createRun('all_actionable')">{{
          t('giftCodes.workspace.runModes.all_actionable')
        }}</AppButton>
        <AppButton type="button" variant="secondary" :busy="busy" @click="createRun('expiring')">{{
          t('giftCodes.workspace.runModes.expiring')
        }}</AppButton>
        <AppButton
          type="button"
          variant="secondary"
          :busy="busy"
          @click="createRun('retry_ready')"
          >{{ t('giftCodes.workspace.runModes.retry_ready') }}</AppButton
        >
      </div>

      <div class="mt-5 grid gap-5 lg:grid-cols-2">
        <fieldset>
          <legend class="ks-kicker">
            {{ t('giftCodes.workspace.shownCodes', { count: codes.length }) }}
          </legend>
          <div class="mt-3 max-h-64 space-y-2 overflow-y-auto pr-1">
            <label
              v-for="giftCode in codes"
              :key="giftCode.id"
            ...
            >
              ...
              <span v-if="giftCode.expiresAt" class="ml-auto text-xs text-[var(--ks-muted)]">{{
      ...
      <p class="mt-3 text-xs text-[var(--ks-muted)]">
        {{
          t('giftCodes.workspace.selectedSummary', {
            codes: selectedCodeIds.length,
            governors: selectedPlayerIds.length,
          })
        }}
      </p>
      <AppButton
        ...
        <article v-for="giftCode in codes" :key="giftCode.id" class="p-5 sm:p-6">
          <div class="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p class="mt-2 text-xs text-[var(--ks-muted)]">
                ...
                {{
                  giftCode.expiresAt
                    ? t('giftCodes.workspace.expires', { date: formatDate(giftCode.expiresAt) })
                    : t('giftCodes.workspace.noExpiry')
                }}
              </p>
            </div>
            <span v-if="giftCode.personalState" class="ks-status" data-tone="info">{{
              giftCode.personalState.state
            }}</span>
          </div>
          ...
          <div class="mt-3 flex flex-wrap gap-2">
            <AppButton
              ...
              size="sm"
              variant="secondary"
              @click="updateState(giftCode.id, 'pinned')"
              >{{ t('giftCodes.workspace.pin') }}</AppButton
            >
            <AppButton
              type="button"
              size="sm"
              variant="secondary"
              @click="updateState(giftCode.id, 'actionable')"
              >{{ t('giftCodes.workspace.restore') }}</AppButton
            >
            <AppButton
              type="button"
              size="sm"
              variant="secondary"
              @click="updateState(giftCode.id, 'snoozed', inHours(24))"
              >{{ t('giftCodes.workspace.snoozeDay') }}</AppButton
            >
            <AppButton
              type="button"
              size="sm"
              variant="secondary"
              @click="updateState(giftCode.id, 'dismissed')"
              >{{ t('giftCodes.workspace.dismiss') }}</AppButton
            >
            <AppButton
              type="button"
              size="sm"
              ...
              @click="
                updateState(
                  giftCode.id,
                  giftCode.personalState?.state ?? 'actionable',
                  null,
                  inHours(24),
                )
              "
              >{{ t('giftCodes.workspace.remindDay') }}</AppButton
            >
          </div>
        </article>
      </div>
      <p v-else class="p-8 text-center text-sm text-[var(--ks-muted)]">
        {{ t('giftCodes.workspace.noCodes') }}
      </p>

      <div class="flex justify-between border-t border-[var(--ks-border)] p-4">
        <Link
          v-if="pagination.previousCursor"
          :href="viewUrl(view, pagination.previousCursor)"
          class="ks-command-link"
        ...
        <Link
          ...
          :href="viewUrl(view, pagination.nextCursor)"
          class="ks-command-link"
          data-variant="secondary"
          >{{ t('common.next') }}</Link
        >
