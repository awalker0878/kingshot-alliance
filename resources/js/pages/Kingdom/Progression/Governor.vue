      ...
      <div class="grid gap-6 2xl:grid-cols-2">
          ...
          >
            <div class="grid gap-4 sm:grid-cols-2">
              <label class="text-xs text-[var(--ks-muted)]">
                ...
                <input v-model="observationForm.power" required inputmode="numeric" pattern="[0-9]+" maxlength="19" class="mt-1 min-h-11 w-full rounded border border-[var(--ks-border)] bg-black/20 px-3 text-sm" />
              </label>
              <label class="text-xs text-[var(--ks-muted)]">
                <span>{{ t('progression.capturedAt') }}</span>
                <input v-model="observationForm.captured_at" required type="datetime-local" class="mt-1 min-h-11 w-full rounded border border-[var(--ks-border)] bg-black/20 px-3 text-sm" />
              </label>
              <label class="text-xs text-[var(--ks-muted)]">
                <span>{{ t('progression.progressionLevel') }}</span>
                <input v-model="observationForm.progression_level" maxlength="64" class="mt-1 min-h-11 w-full rounded border border-[var(--ks-border)] bg-black/20 px-3 text-sm" />
              </label>
              <label class="flex min-h-11 items-center gap-2 self-end rounded border border-[var(--ks-border)] px-3 text-sm">
            ...
            <div class="mt-5 flex items-center justify-between gap-3">
              <div>
                ...
                <p class="mt-1 text-xs text-[var(--ks-muted)]">{{ t('progression.heroObservationHelp') }}</p>
              </div>
              <button type="button" class="min-h-11 rounded border border-[var(--ks-border)] px-3 text-sm" @click="addObservationHero">
                + {{ t('progression.addHero') }}
              </button>
            </div>

            <div class="mt-3 space-y-3">
              <fieldset v-for="(row, index) in observationRows" :key="index" class="rounded border border-[var(--ks-border)] p-3">
                ...
                <div class="grid gap-2 sm:grid-cols-4">
                  <label class="text-xs text-[var(--ks-muted)] sm:col-span-1">
                    ...
                    <select v-model="row.hero_id" required class="mt-1 min-h-11 w-full rounded border border-[var(--ks-border)] bg-[#091313] px-2 text-sm">
                      <option value="">{{ t('progression.selectHero') }}</option>
                      <option v-for="hero in dataset.heroes" :key="hero.id" :value="hero.id">{{ hero.name }} · G{{ hero.generation }}</option>
                    </select>
                  </label>
                  <label class="text-xs text-[var(--ks-muted)]">
                    <span>{{ t('progression.level') }}</span>
                    <input v-model="row.level" type="number" min="0" max="80" class="mt-1 min-h-11 w-full rounded border border-[var(--ks-border)] bg-black/20 px-2 text-sm" />
                  </label>
                  <label class="text-xs text-[var(--ks-muted)]">
                    <span>{{ t('progression.star') }}</span>
                    <input v-model="row.star" type="number" min="0" max="5" class="mt-1 min-h-11 w-full rounded border border-[var(--ks-border)] bg-black/20 px-2 text-sm" />
                  </label>
                  <div class="grid grid-cols-[1fr_auto] gap-2">
                    <label class="text-xs text-[var(--ks-muted)]">
                      <span>{{ t('progression.widgetLevel') }}</span>
                      <input v-model="row.widget_level" type="number" min="0" max="10" class="mt-1 min-h-11 w-full rounded border border-[var(--ks-border)] bg-black/20 px-2 text-sm" />
                    </label>
                    <button type="button" class="mt-5 min-h-11 rounded border border-red-400/25 px-3 text-sm text-red-200" :aria-label="t('progression.removeHero')" @click="removeObservationHero(index)">×</button>
                  </div>
                </div>
              </fieldset>
            </div>

            <p v-if="Object.keys(observationForm.errors).length" class="mt-4 text-sm text-red-300" role="alert">
              {{ Object.values(observationForm.errors)[0] }}
            </p>
            <button type="submit" class="mt-5 min-h-11 rounded bg-[var(--ks-teal)] px-4 text-sm font-semibold disabled:opacity-50" :disabled="observationForm.processing">
            ...
            <article v-for="observation in observations" :key="observation.id" class="p-5 sm:p-6">
              <div class="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <p class="mt-1 text-xs text-[var(--ks-muted)]">{{ observation.source }} · {{ observation.datasetId ?? t('progression.unpinnedLegacyObservation') }}</p>
                </div>
                <div class="text-end">
                  <p class="ks-display text-xl">{{ observation.power }}</p>
                  <p class="text-xs text-[var(--ks-muted)]">{{ observation.progressionLevel ?? '—' }}</p>
                </div>
              </div>
              <div v-if="observation.heroObservations.length" class="mt-4 flex flex-wrap gap-2">
                <span v-for="hero in observation.heroObservations" :key="hero.hero_id" class="ks-chip">
              ...
              <p v-else class="mt-3 text-xs text-[var(--ks-muted)]">{{ t('progression.noHeroObservations') }}</p>
            </article>
            <p v-if="observations.length === 0" class="p-6 text-sm text-[var(--ks-muted)]">{{ t('progression.noObservations') }}</p>
          </div>
        </section>
        ...
        <section class="ks-surface overflow-hidden" aria-labelledby="loadouts-heading">
          <div class="border-b border-[var(--ks-border)] p-5 sm:p-6">
            ...
            <h2 id="loadouts-heading" class="ks-display mt-1 text-2xl font-semibold">{{ t('progression.savedLoadouts') }}</h2>
            <p class="mt-2 text-sm leading-6 text-[var(--ks-text-secondary)]">{{ t('progression.loadoutHelp') }}</p>
          </div>

          <form class="border-b border-[var(--ks-border)] p-5 sm:p-6" @submit.prevent="saveLoadout">
            <label class="text-xs text-[var(--ks-muted)]">
              <span>{{ t('progression.loadoutName') }}</span>
              <input v-model="loadoutForm.name" required maxlength="120" class="mt-1 min-h-11 w-full rounded border border-[var(--ks-border)] bg-black/20 px-3 text-sm" />
            </label>
            <div class="mt-4 grid grid-cols-3 gap-2">
              <label class="text-xs text-[var(--ks-muted)]"><span>{{ t('progression.infantry') }} %</span><input v-model.number="loadoutForm.infantry_percent" type="number" min="0" max="100" class="mt-1 min-h-11 w-full rounded border border-[var(--ks-border)] bg-black/20 px-2 text-sm" /></label>
              <label class="text-xs text-[var(--ks-muted)]"><span>{{ t('progression.cavalry') }} %</span><input v-model.number="loadoutForm.cavalry_percent" type="number" min="0" max="100" class="mt-1 min-h-11 w-full rounded border border-[var(--ks-border)] bg-black/20 px-2 text-sm" /></label>
              <label class="text-xs text-[var(--ks-muted)]"><span>{{ t('progression.archer') }} %</span><input v-model.number="loadoutForm.archer_percent" type="number" min="0" max="100" class="mt-1 min-h-11 w-full rounded border border-[var(--ks-border)] bg-black/20 px-2 text-sm" /></label>
            </div>
            <p class="mt-2 text-xs" :class="loadoutTotal === 100 ? 'text-green-300' : 'text-red-300'">{{ t('progression.formationTotal', { total: loadoutTotal }) }}</p>

            <fieldset class="mt-4">
              <legend class="text-xs font-semibold text-[var(--ks-muted)]">{{ t('progression.heroesUpToFive') }}</legend>
              <div class="mt-2 grid max-h-64 gap-1 overflow-y-auto rounded border border-[var(--ks-border)] p-2 sm:grid-cols-2">
                <label v-for="hero in dataset.heroes" :key="hero.id" class="flex min-h-10 items-center gap-2 rounded px-2 text-sm hover:bg-white/[.025]">
                  <input type="checkbox" :checked="loadoutForm.heroes.includes(hero.id)" :disabled="!loadoutForm.heroes.includes(hero.id) && loadoutForm.heroes.length >= 5" @change="toggleLoadoutHero(hero.id)" />
                  <span>{{ hero.name }} <small class="text-[var(--ks-muted)]">G{{ hero.generation }}</small></span>
                </label>
              </div>
            </fieldset>
            ...
            <label class="mt-4 block text-xs text-[var(--ks-muted)]">
              ...
              <textarea v-model="loadoutForm.notes" rows="3" maxlength="10000" class="mt-1 w-full rounded border border-[var(--ks-border)] bg-black/20 p-3 text-sm" />
            </label>
            ...
            <p class="mt-2 break-all text-[0.68rem] text-[var(--ks-muted)]">{{ t('progression.pinnedTo') }} {{ dataset.version }} · {{ dataset.checksum }}</p>
            <p v-if="Object.keys(loadoutForm.errors).length" class="mt-3 text-sm text-red-300" role="alert">{{ Object.values(loadoutForm.errors)[0] }}</p>
            <div class="mt-4 flex gap-2">
              <button type="submit" class="min-h-11 rounded bg-[var(--ks-teal)] px-4 text-sm font-semibold disabled:opacity-50" :disabled="loadoutTotal !== 100 || loadoutForm.processing">
                {{ editingLoadoutId ? t('progression.updateLoadout') : t('progression.saveLoadout') }}
              </button>
              <button v-if="editingLoadoutId" type="button" class="min-h-11 rounded border border-[var(--ks-border)] px-4 text-sm" @click="resetLoadout">{{ t('common.cancel') }}</button>
            </div>
          </form>
            ...
            <article v-for="loadout in loadouts" :key="loadout.id" class="p-5 sm:p-6">
              <div class="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p class="mt-1 text-sm">{{ loadout.infantryPercent }} / {{ loadout.cavalryPercent }} / {{ loadout.archerPercent }}</p>
                  <p class="mt-1 text-xs text-[var(--ks-muted)]">{{ t('progression.userPlanningIntent') }} · {{ loadout.datasetId ?? t('progression.unpinnedLegacyLoadout') }}</p>
                </div>
                <span v-if="loadout.isDefault" class="ks-chip">{{ t('progression.defaultLoadout') }}</span>
              </div>
              <div v-if="loadout.heroes.length" class="mt-3 flex flex-wrap gap-2">
                <span v-for="heroId in loadout.heroes" :key="heroId" class="ks-chip">{{ heroName(heroId) }}</span>
              </div>
              <p v-if="loadout.notes" class="mt-3 text-xs leading-5 text-[var(--ks-text-secondary)]">{{ loadout.notes }}</p>
              <div class="mt-4 flex gap-2">
                <button type="button" class="min-h-10 rounded border border-[var(--ks-border)] px-3 text-xs" @click="editLoadout(loadout)">{{ t('progression.editLoadout') }}</button>
                <button type="button" class="min-h-10 rounded border border-red-400/25 px-3 text-xs text-red-200" @click="deleteLoadout(loadout)">{{ t('progression.deleteLoadout') }}</button>
              </div>
            </article>
            <p v-if="loadouts.length === 0" class="p-6 text-sm text-[var(--ks-muted)]">{{ t('progression.noLoadouts') }}</p>
          </div>
        </section>
      </div>
    </div>
