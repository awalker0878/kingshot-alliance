...
const capacityDrafts = reactive(
  Object.fromEntries(
    ...
    string,
    { bucket: CapacityBucket; state: CapacityReservationState; notes: string }
  >,
);
const invitationDrafts = reactive(
  Object.fromEntries(
    ...
    string,
    { kind: InvitationKind; state: InvitationAllocationState; notes: string }
  >,
);
  ...
  <AppLayout :user="user" :player-alliance-name="alliance.name" :has-player-alliance="true">
    ...
    <section v-if="plan" class="ks-surface mt-4 p-4">
      ...
      >
        ...
        <option value="eligible_with_action">{{ t('kingdomP7D.eligibility_eligible_with_action') }}</option>
      ...
      <article v-for="p in filtered" :key="p.id" class="ks-surface overflow-hidden">
        <div class="p-5 sm:p-6">
          <div class="flex flex-wrap items-start justify-between gap-4">
            ...
            <span
              v-if="p.eligibility"
              :class="['rounded-full border px-3 py-1 text-sm font-bold', outcomeTone(p.eligibility.outcome)]"
          ...
          <dl class="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
            ...
            <div class="rounded-lg border border-[var(--ks-border)] p-3">
              ...
              <dd class="mt-1 font-semibold">{{ displayValue(p.targetCondition?.heroGeneration) }}</dd>
            </div>
            <div class="rounded-lg border border-[var(--ks-border)] p-3">
              <dt class="ks-kicker">{{ t('kingdomP7D.targetTruegoldLevel') }}</dt>
              <dd class="mt-1 font-semibold">{{ displayValue(p.targetCondition?.truegoldLevel) }}</dd>
            </div>
            <div class="rounded-lg border border-[var(--ks-border)] p-3">
              <dt class="ks-kicker">{{ t('kingdomP7D.targetCharacterAgeThresholdDays') }}</dt>
              <dd class="mt-1 font-semibold">{{ displayValue(p.targetCondition?.characterAgeThresholdDays) }}</dd>
            </div>
            <div class="rounded-lg border border-[var(--ks-border)] p-3">
              ...
              <dd class="mt-1 text-xs text-[var(--ks-muted)]">
                ...
                {{ p.transferScore.observedAt ? timestamp(p.transferScore.observedAt) : t('kingdomP7D.noObservation') }}
              </dd>
            </div>
          </dl>
        ...
        <section v-if="p.direction !== 'staying'" class="border-t border-[var(--ks-border)] p-5 sm:p-6">
          <div class="flex flex-wrap items-start justify-between gap-3">
            <div>
              <h3 class="text-lg font-semibold">{{ t('kingdomP7D.capacityPlanning') }}</h3>
              <p class="mt-1 max-w-4xl text-sm text-[var(--ks-muted)]">{{ t('kingdomP7D.capacityPlanningHelp') }}</p>
            </div>
            <span
              :class="['rounded-full border px-3 py-1 text-xs font-semibold', p.capacity?.state === 'met' ? 'border-green-400/30 text-green-200' : 'border-amber-400/30 text-amber-100']"
            >{{ p.capacity?.state === 'met' ? t('kingdomP7D.requirement_met') : t('kingdomP7D.needsVerification') }}</span>
          </div>
          ...
          <div v-if="p.capacity" class="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <div class="rounded-xl border border-[var(--ks-border)] p-4">
              ...
              <p class="mt-2 text-sm">{{ t('kingdomP7D.observedRemaining') }}: <strong>{{ displayValue(p.capacity.observedTotalRemaining) }}</strong></p>
              <p class="mt-1 text-sm">{{ t('kingdomP7D.projectedRemaining') }}: <strong>{{ displayValue(p.capacity.projectedTotalRemaining) }}</strong></p>
              <p class="mt-2 text-xs text-[var(--ks-muted)]">{{ t('kingdomP7D.required') }}: {{ displayValue(p.capacity.officialTotalCapacity) }}</p>
            </div>
            <div class="rounded-xl border border-[var(--ks-border)] p-4">
              <p class="ks-kicker">{{ t('kingdomP7D.ordinaryInviteCapacity') }}</p>
              <p class="mt-2 text-sm">{{ t('kingdomP7D.observedRemaining') }}: <strong>{{ displayValue(p.capacity.observedOrdinaryInviteRemaining) }}</strong></p>
              <p class="mt-1 text-sm">{{ t('kingdomP7D.projectedRemaining') }}: <strong>{{ displayValue(p.capacity.projectedOrdinaryInviteRemaining) }}</strong></p>
              <p class="mt-2 text-xs text-[var(--ks-muted)]">
                {{ t('kingdomP7D.observedUsed') }}: {{ displayValue(p.capacity.ordinaryInvitesUsed) }} ·
                {{ t('kingdomP7D.plannedAllocated') }}: {{ p.capacity.plannedOrdinaryInviteReservations }}
              </p>
            </div>
            <div class="rounded-xl border border-[var(--ks-border)] p-4">
              <p class="ks-kicker">{{ t('kingdomP7D.transferOpenCapacity') }}</p>
              <p class="mt-2 text-sm">{{ t('kingdomP7D.observedRemaining') }}: <strong>{{ displayValue(p.capacity.observedTransferOpenRemaining) }}</strong></p>
              <p class="mt-1 text-sm">{{ t('kingdomP7D.projectedRemaining') }}: <strong>{{ displayValue(p.capacity.projectedTransferOpenRemaining) }}</strong></p>
              <p class="mt-2 text-xs text-[var(--ks-muted)]">
                {{ t('kingdomP7D.observedUsed') }}: {{ displayValue(p.capacity.transferOpensUsed) }} ·
                {{ t('kingdomP7D.plannedAllocated') }}: {{ p.capacity.plannedTransferOpenReservations }}
              </p>
            </div>
            <div class="rounded-xl border border-[var(--ks-border)] p-4">
              <p class="ks-kicker">{{ t('kingdomP7D.specialInviteInventory') }}</p>
              <p class="mt-2 text-sm">{{ t('kingdomP7D.observedRemaining') }}: <strong>{{ displayValue(p.capacity.observedSpecialInvitesAvailable) }}</strong></p>
              <p class="mt-1 text-sm">{{ t('kingdomP7D.projectedRemaining') }}: <strong>{{ displayValue(p.capacity.projectedSpecialInvitesAvailable) }}</strong></p>
              <p class="mt-2 text-xs text-[var(--ks-muted)]">{{ t('kingdomP7D.plannedAllocated') }}: {{ p.capacity.plannedSpecialInviteAllocations }}</p>
            </div>
          </div>
          <p v-else class="mt-4 rounded-lg border border-amber-400/30 bg-amber-500/10 p-3 text-sm text-amber-100">
            {{ t('kingdomP7D.capacityUnknown') }}
          </p>
          <p v-if="p.capacity?.sourceReference" class="mt-2 text-xs break-all text-[var(--ks-muted)]">
            {{ sourceLabel(p.capacity.sourceType) }} · {{ timestamp(p.capacity.observedAt) }} · {{ p.capacity.sourceReference }}
          </p>

          <div v-if="plan.mutable && !p.withdrawnAt" class="mt-5 grid gap-4 xl:grid-cols-2">
            <form class="rounded-xl border border-[var(--ks-border)] p-4" @submit.prevent="saveCapacityReservation(p)">
              <h4 class="font-semibold">{{ t('kingdomP7D.capacityReservation') }}</h4>
              <div class="mt-3 grid gap-3 sm:grid-cols-2">
                <label class="text-sm font-semibold">{{ t('kingdomP7D.capacityBucket') }}
                  <select v-model="capacityDrafts[p.id]!.bucket" class="ks-input mt-1 w-full">
                    <option value="ordinary_invite">{{ t('kingdomP7D.bucket_ordinary_invite') }}</option>
                    <option value="transfer_open">{{ t('kingdomP7D.bucket_transfer_open') }}</option>
                  </select>
                </label>
                <label class="text-sm font-semibold">{{ t('kingdomP7D.reservationState') }}
                  <select v-model="capacityDrafts[p.id]!.state" class="ks-input mt-1 w-full">
                    <option v-for="state in capacityStates" :key="state" :value="state">{{ t(`kingdomP7D.reservation_${state}`) }}</option>
                  </select>
                </label>
                <label class="text-sm font-semibold sm:col-span-2">{{ t('kingdomP7D.capacityNotes') }}
                  <textarea v-model="capacityDrafts[p.id]!.notes" class="ks-input mt-1 w-full" rows="2" />
                </label>
              </div>
              <button class="mt-3 rounded-lg border border-[var(--ks-border)] px-4 py-2 text-sm font-semibold" type="submit">
                {{ t('kingdomP7D.saveCapacityReservation') }}
              </button>
            </form>

            <form class="rounded-xl border border-[var(--ks-border)] p-4" @submit.prevent="saveInvitationAllocation(p)">
              <h4 class="font-semibold">{{ t('kingdomP7D.invitationAllocation') }}</h4>
              <div class="mt-3 grid gap-3 sm:grid-cols-2">
                <label class="text-sm font-semibold">{{ t('kingdomP7D.invitationKind') }}
                ...
                <label class="text-sm font-semibold">{{ t('kingdomP7D.invitationAllocationState') }}
                  <select v-model="invitationDrafts[p.id]!.state" class="ks-input mt-1 w-full">
                    <option v-for="state in invitationStates" :key="state" :value="state">{{ t(`kingdomP7D.invitationAllocation_${state}`) }}</option>
                  </select>
                </label>
                <label class="text-sm font-semibold sm:col-span-2">{{ t('kingdomP7D.invitationNotes') }}
                  <textarea v-model="invitationDrafts[p.id]!.notes" class="ks-input mt-1 w-full" rows="2" />
                </label>
              </div>
              <button class="mt-3 rounded-lg border border-[var(--ks-border)] px-4 py-2 text-sm font-semibold" type="submit">
          ...
          <ul class="mt-3 grid gap-3">
            <li v-for="r in p.eligibility.requirements" :key="r.key" class="rounded-xl border border-[var(--ks-border)] p-4">
              ...
              <p v-if="r.actual !== null || r.required !== null" class="mt-2 text-sm">
                {{ t('kingdomP7D.actual') }}: {{ displayValue(r.actual) }} · {{ t('kingdomP7D.required') }}: {{ displayValue(r.required) }}
              </p>
        ...
        <div class="border-t border-[var(--ks-border)] p-5 sm:p-6">
          <details>
            ...
            <form class="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3" @submit.prevent="recordObservation(p)">
              ...
              <label class="text-sm font-semibold sm:col-span-2">{{ t('kingdomP7D.sourceReference') }}
                <input v-model="observationDrafts[p.id]!.source_reference" class="mt-1 w-full rounded-lg border border-[var(--ks-border)] bg-[var(--ks-bg)] px-3 py-2" maxlength="2048" required />
              </label>
              <label class="text-sm font-semibold">{{ t('kingdomP7D.observedAt') }}
                <input v-model="observationDrafts[p.id]!.observed_at" class="mt-1 w-full rounded-lg border border-[var(--ks-border)] bg-[var(--ks-bg)] px-3 py-2" required type="datetime-local" />
              </label>
              <label class="text-sm font-semibold">{{ t('kingdomP7D.validUntil') }}
                <input v-model="observationDrafts[p.id]!.valid_until" class="mt-1 w-full rounded-lg border border-[var(--ks-border)] bg-[var(--ks-bg)] px-3 py-2" type="datetime-local" />
              </label>
              <label class="text-sm font-semibold sm:col-span-2">{{ t('kingdomP7D.details') }}
                <textarea v-model="observationDrafts[p.id]!.details" class="mt-1 w-full rounded-lg border border-[var(--ks-border)] bg-[var(--ks-bg)] px-3 py-2" rows="2" />
              </label>
              <div>
                <button class="rounded-lg bg-[var(--ks-gold)] px-4 py-2 text-sm font-bold text-[var(--ks-ink)]" type="submit">{{ t('kingdomP7D.recordObservation') }}</button>
              </div>
            </form>
          </details>
          <details class="mt-4">
          ...
          <details class="mt-4">
            ...
            <ul class="mt-2 text-sm text-[var(--ks-muted)]">
              <li v-for="h in p.readinessHistory" :key="`${h.changedAt}-${h.to}`">
                {{ readinessLabel(h.from ?? h.to) }} → {{ readinessLabel(h.to) }} · {{ timestamp(h.changedAt) }} · {{ h.actor?.name ?? t('kingdomP7D.unknownActor') }}
              </li>
            </ul>
          </details>
        </div>
