...
const goalCategories = computed(() => props.reporting.progress.filter((item) => item.goal !== null).length);
...
function dataClassTone(value: string): string {
  ...
  if (value === 'subjective_assessment') return 'border-amber-400/20 bg-amber-500/10 text-amber-200';
  return 'border-blue-400/20 bg-blue-500/10 text-blue-200';
}

function explanation(row: RecordRow): string {
  if (row.correctionReason) return t('contributions.correctedReason', { reason: row.correctionReason });
  if (row.reversalReason) return t('contributions.reversedReason', { reason: row.reversalReason });
  if (row.calculationVersion) return t('contributions.calculationVersion', { version: row.calculationVersion });
  return row.evidence || '—';
}
</script>
  ...
  <AppLayout :user="user" :alliance-name="alliance.name" :has-active-alliance="true">
    <header class="flex flex-wrap items-start justify-between gap-4">
      <div class="max-w-3xl">
        ...
        <h1 class="ks-display mt-2 text-3xl font-bold sm:text-4xl">{{ t('contributions.title') }}</h1>
    ...
    <section class="ks-surface-gold mt-6 overflow-hidden" :aria-label="t('contributions.overview')">
      <div class="grid grid-cols-2 divide-x divide-y divide-[var(--ks-border)] lg:grid-cols-4 lg:divide-y-0">
        <article class="p-4 sm:p-5">
          ...
          <p class="ks-display mt-2 text-3xl font-semibold">{{ formatNumber(reporting.progress.length) }}</p>
        </article>
        <article class="p-4 sm:p-5">
          <p class="text-[0.68rem] font-bold tracking-[0.12em] text-[var(--ks-text-muted)] uppercase">
        ...
        <article class="p-4 sm:p-5">
          ...
          <p class="ks-display mt-2 text-3xl font-semibold">{{ formatNumber(selfReportCategories.length) }}</p>
        </article>
        <article class="p-4 sm:p-5">
          <p class="text-[0.68rem] font-bold tracking-[0.12em] text-[var(--ks-text-muted)] uppercase">
            {{ t('contributions.recentRecordsShown') }}
          </p>
          <p class="ks-display mt-2 text-3xl font-semibold">{{ formatNumber(reporting.history.length) }}</p>
        </article>
      </div>
    </section>
    ...
    <div class="mt-6 grid gap-5 xl:grid-cols-3">
      <div class="min-w-0 space-y-5 xl:col-span-2">
        <section aria-labelledby="contribution-progress-heading">
          ...
          <div v-if="reporting.progress.length" class="mt-4 grid gap-4 md:grid-cols-2">
            <article v-for="item in reporting.progress" :key="item.categoryId" class="ks-surface p-5">
              <div class="flex items-start justify-between gap-3">
                ...
                <span class="shrink-0 text-sm font-semibold text-[var(--ks-gold-strong)]">{{ percent(item.progress) }}</span>
              </div>

              <div class="mt-5 flex items-end justify-between gap-4">
                <p>
                  <span class="ks-display text-3xl font-semibold">{{ formatNumber(item.approved, { maximumFractionDigits: 2 }) }}</span>
                ...
                <p class="text-end text-xs text-[var(--ks-text-muted)]">
                  ...
                  <strong class="text-[var(--ks-text-secondary)]">
                    {{ item.goal === null ? t('contributions.notConfigured') : formatNumber(item.goal, { maximumFractionDigits: 2 }) }}
                  </strong>
                </p>
              </div>

              <div v-if="item.progress !== null" class="mt-3 h-1.5 overflow-hidden rounded-full bg-black/30">
                <div class="h-full rounded-full bg-[var(--ks-gold)]" :style="{ width: progressWidth(item.progress) }" />
              </div>

              <p class="mt-4 text-xs text-[var(--ks-text-muted)]">
                {{ dateOnly(item.periodStart) }} – {{ dateOnly(item.periodEnd) }}
              </p>
              <p v-if="item.calculationDescription" class="mt-3 border-t border-[var(--ks-border)] pt-3 text-sm leading-6 text-[var(--ks-text-secondary)]">
        ...
        <section class="ks-surface overflow-hidden" aria-labelledby="contribution-history-heading">
          <div class="border-b border-[var(--ks-border)] p-4 sm:p-5">
            <div class="flex flex-wrap items-end justify-between gap-3">
              ...
              <p class="text-xs text-[var(--ks-text-muted)]">{{ t('contributions.historyHelp') }}</p>
            </div>
          </div>

          <div v-if="reporting.history.length" class="lg:hidden">
            <article v-for="row in reporting.history" :key="row.id" class="border-b border-[var(--ks-border)] p-4 last:border-b-0">
              <div class="flex items-start justify-between gap-3">
                <div class="min-w-0">
                  <p class="truncate font-semibold">{{ row.categoryName ?? '—' }}</p>
                  <p class="mt-1 text-xs text-[var(--ks-text-muted)]">{{ dateTime(row.recordedAt) }}</p>
                </div>
                <div class="shrink-0 text-end">
                  <strong>{{ valueLabel(row.value, row.unit) }}</strong>
                  <span :class="statusTone(row.status)" class="mt-2 block rounded-full border px-2.5 py-1 text-xs font-semibold capitalize">
                    {{ humanize(row.status) }}
                  </span>
                </div>
              </div>
              <dl class="mt-4 grid grid-cols-2 gap-3 text-xs">
                ...
                <div>
                  ...
                  <dd class="mt-1 text-[var(--ks-text-secondary)]">{{ humanize(row.dataClass) }}</dd>
                </div>
          ...
          <div v-if="reporting.history.length" class="hidden overflow-x-auto lg:block">
            <table class="w-full min-w-[64rem] text-sm">
              ...
              <tbody class="divide-y divide-[var(--ks-border)]">
                <tr v-for="row in reporting.history" :key="row.id" class="transition hover:bg-white/[0.025]">
                  <td class="px-4 py-3.5 font-semibold">{{ row.categoryName ?? '—' }}</td>
                  <td class="px-4 py-3.5">{{ valueLabel(row.value, row.unit) }}</td>
                  <td class="px-4 py-3.5">
                    <span :class="statusTone(row.status)" class="rounded-full border px-2.5 py-1 text-xs font-semibold capitalize">
                      {{ humanize(row.status) }}
                    </span>
                  </td>
                  <td class="px-4 py-3.5 text-[var(--ks-text-secondary)]">{{ humanize(row.source) }}</td>
                  <td class="px-4 py-3.5 text-xs text-[var(--ks-text-muted)]">{{ dateTime(row.recordedAt) }}</td>
                  <td class="px-4 py-3.5 text-[var(--ks-text-secondary)]">{{ explanation(row) }}</td>
                </tr>
              </tbody>
            </table>
          </div>

          <p v-if="!reporting.history.length" class="p-8 text-center text-sm text-[var(--ks-text-muted)]">
      ...
      <aside class="space-y-5 xl:col-span-1">
        <section v-if="selfReportCategories.length" class="ks-surface p-5 xl:sticky xl:top-24" aria-labelledby="self-report-heading">
          <p class="text-xs font-bold tracking-[0.15em] text-[var(--ks-gold)] uppercase">
          ...
          <form class="mt-5 space-y-4" @submit.prevent="submit">
            <div>
              <label class="text-xs font-semibold text-[var(--ks-text-secondary)]" for="contribution-category">
              ...
              >
                ...
                <option v-for="category in selfReportCategories" :key="category.categoryId" :value="category.categoryId">
                  {{ category.name }} ({{ category.unit }})
                </option>
              </select>
            </div>
            <div>
              <label class="text-xs font-semibold text-[var(--ks-text-secondary)]" for="contribution-value">
            ...
            <div>
              <label class="text-xs font-semibold text-[var(--ks-text-secondary)]" for="contribution-evidence">
    ...
    <section v-if="reporting.leaderboards.length" class="mt-5" :aria-label="t('contributions.leaderboards')">
      <div class="mb-4 px-1">
        ...
        <h2 class="ks-display mt-1 text-2xl font-semibold">{{ t('contributions.approvedCategoryLeaderboards') }}</h2>
      </div>
        ...
        <article v-for="board in reporting.leaderboards" :key="board.categoryId" class="ks-surface overflow-hidden">
          <div class="border-b border-[var(--ks-border)] p-4 sm:p-5">
            <div class="flex flex-wrap items-start justify-between gap-3">
              ...
              <span v-if="board.calculationVersion" class="rounded-full border border-[var(--ks-border)] bg-black/15 px-2.5 py-1 text-xs text-[var(--ks-text-secondary)]">
                {{ t('contributions.version') }} {{ board.calculationVersion }}
              </span>
            </div>
            <p class="mt-3 text-sm leading-6 text-[var(--ks-text-secondary)]">{{ board.calculationDescription }}</p>
          </div>
          <ol v-if="board.entries.length" class="divide-y divide-[var(--ks-border)]">
            <li v-for="(entry, index) in board.entries" :key="entry.membershipId" class="flex items-center justify-between gap-4 px-4 py-3 sm:px-5">
              <span class="min-w-0 truncate"><strong class="me-2 text-[var(--ks-gold-strong)]">{{ index + 1 }}</strong>{{ entry.name }}</span>
              <strong class="shrink-0">{{ valueLabel(entry.value, board.unit) }}</strong>
            </li>
          </ol>
          <p v-else class="p-5 text-sm text-[var(--ks-text-muted)]">{{ t('contributions.noApprovedRecords') }}</p>
        </article>
      </div>
    </section>
  </AppLayout>
