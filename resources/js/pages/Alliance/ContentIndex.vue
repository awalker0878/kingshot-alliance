...
const publicCount = computed(() => props.content.filter((item) => item.visibility === 'public').length);
const memberCount = computed(() => props.content.filter((item) => item.visibility === 'members').length);
  ...
  <AppLayout :user="user" :alliance-name="alliance.name" :has-active-alliance="true">
    <header class="flex flex-wrap items-start justify-between gap-4">
      <div class="max-w-3xl">
        ...
        <p class="mt-3 text-sm leading-6 text-[var(--ks-text-secondary)]">
          {{ t('contentExperience.hubSubtitle', { alliance: alliance.name }) }}
        </p>
      </div>
      <Link
    ...
    <section class="ks-surface-gold mt-6 overflow-hidden">
      <dl class="grid grid-cols-2 divide-x divide-y divide-[var(--ks-border)] md:grid-cols-4 md:divide-y-0">
        ...
        <div class="p-4 sm:p-5">
          ...
          <dd class="ks-display mt-2 text-3xl font-semibold">{{ formatNumber(categories.length) }}</dd>
        </div>
      </dl>
    </section>

    <form class="ks-surface mt-5 grid gap-4 p-5 sm:grid-cols-2 lg:grid-cols-4" @submit.prevent="applyFilters">
      ...
      <div>
        ...
        <select id="member-type" v-model="filters.type" class="mt-1.5 w-full rounded-[var(--ks-radius-sm)] border border-[var(--ks-border)] bg-[var(--ks-bg)] px-3 py-2.5 text-sm">
      ...
      <div>
        ...
        <select id="member-category" v-model="filters.category" class="mt-1.5 w-full rounded-[var(--ks-radius-sm)] border border-[var(--ks-border)] bg-[var(--ks-bg)] px-3 py-2.5 text-sm">
      ...
      <div class="flex items-end gap-2">
        <button class="min-h-10 rounded-[var(--ks-radius-sm)] bg-[var(--ks-blue)] px-4 py-2 text-sm font-semibold text-white" type="submit">
          {{ t('contentExperience.applyFilters') }}
        </button>
        <button class="min-h-10 rounded-[var(--ks-radius-sm)] border border-[var(--ks-border)] px-4 py-2 text-sm font-semibold" type="button" @click="clearFilters">
    ...
    <section class="mt-6" aria-labelledby="member-content-heading">
      ...
      <div v-if="content.length" class="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        <article v-for="item in content" :key="item.id" class="ks-surface group flex min-h-56 flex-col p-5">
          <div class="flex flex-wrap gap-2 text-[0.68rem] font-bold tracking-[0.08em] text-[var(--ks-text-muted)] uppercase">
            <span>{{ item.typeLabel }}</span>
            <span v-if="item.category">· {{ item.category.name }}</span>
            <span>· {{ item.locale }}</span>
          </div>
          <h3 class="ks-display mt-3 text-xl font-semibold">
            <Link class="transition group-hover:text-[var(--ks-blue-strong)]" :href="`/alliance/content/${item.slug}`">
              {{ item.title }}
            </Link>
          </h3>
          <p v-if="item.summary" class="mt-3 line-clamp-3 text-sm leading-6 text-[var(--ks-text-secondary)]">
            {{ item.summary }}
          </p>
          <div class="mt-auto flex flex-wrap items-center justify-between gap-2 pt-5 text-xs text-[var(--ks-text-muted)]">
            <span v-if="item.visibility === 'members'" class="rounded-full border border-amber-400/25 bg-amber-500/10 px-2.5 py-1 font-semibold text-amber-200">
              {{ t('contentExperience.membersOnly') }}
            </span>
            <span v-else class="rounded-full border border-green-400/25 bg-green-500/10 px-2.5 py-1 font-semibold text-green-200">
