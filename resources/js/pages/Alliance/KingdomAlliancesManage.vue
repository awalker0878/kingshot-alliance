...
<template>
  ...
  <main class="mx-auto min-h-screen max-w-6xl px-6 py-12 lg:px-8">
      ...
      <form class="mt-6 grid gap-5 md:grid-cols-2" @submit.prevent="createTracking">
        ...
        <div>
          ...
          <textarea
            ...
            maxlength="5000"
          />
          <p class="mt-1 text-xs text-slate-500">Private to authorized Kingdoms managers.</p>
        ...
        <table class="min-w-full divide-y divide-slate-800 text-left text-sm">
          ...
          <tbody class="divide-y divide-slate-800">
            <tr v-for="entry in tracking" :key="entry.id">
              ...
              <td class="px-3 py-4 text-slate-300">{{ entry.gameAllianceId ?? 'Unresolved' }}</td>
    ...
    <section
      v-if="editForm.id !== ''"
      class="mt-8 rounded-2xl border border-cyan-900 bg-slate-900/70 p-6"
    >
