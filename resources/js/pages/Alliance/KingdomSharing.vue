...
<template>
  ...
  <main class="mx-auto min-h-screen max-w-6xl px-6 py-12 lg:px-8">
    ...
    <section aria-labelledby="shared-current-heading" class="mt-10 rounded-2xl border border-slate-800 bg-slate-900/70 p-6">
      ...
      <div v-if="current.length" class="mt-6 overflow-x-auto">
        <table class="min-w-full divide-y divide-slate-800 text-left text-sm">
          <caption class="sr-only">Current game-alliance facts shared with this Alliance</caption>
          ...
          <tbody class="divide-y divide-slate-800">
            <tr v-for="entry in current" :key="entry.shareTargetId">
              ...
              <td class="px-3 py-4">
                ...
                <p class="mt-1 text-xs text-slate-400">{{ entry.gameAlliance.tag ?? 'No tag recorded' }}</p>
              </td>
              ...
              <td class="px-3 py-4">
                <Link class="font-semibold text-cyan-300 hover:underline" :href="historyUrl(entry.shareTargetId)">
                  View history
                </Link>
              </td>
            </tr>
      ...
      <p v-else class="mt-6 rounded-xl border border-dashed border-slate-700 p-5 text-sm text-slate-400">
        No active shared targets are available. Removed, revoked, or invalidated sharing is not displayed.
      </p>
    </section>
    ...
    >
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p class="mt-1 text-sm text-slate-400">
            {{ selectedHistory.gameAlliance.name }} · shared by {{ selectedHistory.sourceAlliance.name }}.
            Pages contain at most 50 accepted observations and one traversal stops at 250.
          </p>
        </div>
        <Link class="text-sm font-semibold text-cyan-300 hover:underline" href="/alliance/kingdom-sharing">
          Close history
        </Link>
      </div>
      ...
      <div v-if="selectedHistory.items.length" class="mt-6 overflow-x-auto">
        <table class="min-w-full divide-y divide-slate-800 text-left text-sm">
          <caption class="sr-only">Accepted observations for the selected shared game alliance</caption>
          <thead class="text-xs tracking-wide text-slate-400 uppercase">
            <tr>
          ...
          <tbody class="divide-y divide-slate-800">
            <tr v-for="item in selectedHistory.items" :key="`${item.capturedAt}-${item.observedName}`">
              ...
              <td class="px-3 py-4">
                ...
                <p class="mt-1 text-xs text-slate-400">{{ item.observedTag ?? 'No tag recorded' }}</p>
              </td>
      ...
      <p v-else class="mt-6 text-sm text-slate-400">No accepted observations are available for this target.</p>

      <div class="mt-6 flex items-center justify-between gap-4">
        <p class="text-xs text-slate-500">History continuation is opaque and re-authorized on every page.</p>
        <Link
