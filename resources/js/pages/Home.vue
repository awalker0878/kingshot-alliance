...
<template>
  ...
  <PublicLayout>
    <section class="relative isolate overflow-hidden border-b border-[var(--ks-border)]">
      <div
        class="pointer-events-none absolute inset-0 -z-20 bg-[radial-gradient(circle_at_72%_30%,rgba(226,180,77,0.18),transparent_22rem),radial-gradient(circle_at_18%_15%,rgba(75,143,247,0.18),transparent_25rem),linear-gradient(180deg,#081321_0%,#050b14_100%)]"
      />
      <div
        class="pointer-events-none absolute inset-x-0 bottom-0 -z-10 h-52 opacity-70 [background:linear-gradient(135deg,transparent_0_19%,rgba(226,180,77,0.09)_19.2%_19.8%,transparent_20%_38%,rgba(75,143,247,0.07)_38.2%_38.8%,transparent_39%)]"
      />

      <div class="mx-auto grid min-h-[38rem] max-w-7xl items-center gap-12 px-5 py-16 lg:grid-cols-[0.92fr_1.08fr] lg:px-8 lg:py-24">
        <div class="max-w-2xl">
          ...
          <div class="mt-9 flex flex-wrap gap-3">
            <Link
              href="/login"
              class="rounded-[var(--ks-radius-sm)] bg-[var(--ks-gold)] px-5 py-3 text-sm font-bold text-slate-950 shadow-[0_14px_40px_rgba(226,180,77,0.16)] transition hover:bg-[var(--ks-gold-strong)]"
            >
              {{ t('common.signIn') }}
            </Link>
            <Link
        ...
        <div class="relative mx-auto w-full max-w-2xl lg:mx-0">
          ...
          <div class="ks-surface-gold overflow-hidden">
            <div class="flex items-center justify-between border-b border-[var(--ks-border)] px-4 py-3 sm:px-5">
              <div class="flex items-center gap-2 text-xs font-semibold text-[var(--ks-text-secondary)]">
                <span class="h-2 w-2 rounded-full bg-[var(--ks-green)]" />
                {{ t('home.featuresTitle') }}
              </div>
              <span class="text-xs font-bold tracking-[0.12em] text-[var(--ks-gold)] uppercase">Kingshot</span>
            </div>
            <div class="grid min-h-80 grid-cols-[5.5rem_1fr] sm:grid-cols-[8rem_1fr]">
              ...
              <div class="p-4 sm:p-5">
                <div class="grid gap-3 sm:grid-cols-2">
                  ...
                  >
                    <div class="flex items-center gap-3 text-[var(--ks-gold)]">
                      <NavIcon :name="item.icon" />
                      <h2 class="text-sm font-bold text-[var(--ks-text)]">{{ t(item.titleKey) }}</h2>
                    </div>
                ...
                <div class="mt-3 rounded-[var(--ks-radius-md)] border border-[var(--ks-border)] bg-[rgba(8,17,31,0.75)] p-4">
                  <div class="flex items-center justify-between gap-4">
                    <div>
                      <p class="text-xs text-[var(--ks-text-muted)]">{{ t('home.multilingualTitle') }}</p>
    ...
    <section id="features" class="mx-auto max-w-7xl px-5 py-16 lg:px-8 lg:py-20">
      <div class="mx-auto max-w-2xl text-center">
        <p class="text-xs font-bold tracking-[0.2em] text-[var(--ks-gold)] uppercase">Kingshot Alliance</p>
        <h2 class="ks-display mt-3 text-3xl font-bold sm:text-4xl">{{ t('home.featuresTitle') }}</h2>
      </div>
      ...
      <div class="mt-10 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        >
          <div class="grid h-11 w-11 place-items-center rounded-xl border border-[var(--ks-border-strong)] bg-[var(--ks-gold-soft)] text-[var(--ks-gold)]">
    ...
    <section class="mx-auto max-w-7xl px-5 pb-20 lg:px-8">
      <div class="ks-surface-gold flex flex-col gap-6 p-6 sm:p-8 lg:flex-row lg:items-center lg:justify-between">
        <div class="max-w-2xl">
          <h2 class="ks-display text-3xl font-bold">{{ t('home.ctaTitle') }}</h2>
          <p class="mt-2 text-sm leading-6 text-[var(--ks-text-secondary)]">{{ t('home.ctaBody') }}</p>
        </div>
        <div class="flex shrink-0 flex-wrap gap-3">
          <Link
            href="/login"
            class="rounded-[var(--ks-radius-sm)] bg-[var(--ks-gold)] px-5 py-3 text-sm font-bold text-slate-950 hover:bg-[var(--ks-gold-strong)]"
          >
            {{ t('common.signIn') }}
          </Link>
          <Link
