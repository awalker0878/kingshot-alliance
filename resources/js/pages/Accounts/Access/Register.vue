  ...
  <AuthLayout>
    ...
    <template v-if="props.registrationMode === 'open' || props.invitationToken">
      <a
        ...
        >{{ t('authExperience.social.createWithGoogle') }}</a
      >
      <div v-if="props.googleAuthEnabled" class="my-6 flex items-center gap-3" aria-hidden="true">
        <span class="h-px flex-1 bg-[var(--ks-border)]" /><span
          class="text-xs tracking-[0.16em] text-[var(--ks-muted)] uppercase"
          >{{ t('authExperience.social.or') }}</span
