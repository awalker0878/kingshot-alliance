...
<template>
  ...
  <AuthLayout>
    ...
    <h2 class="ks-display mt-2 text-3xl font-semibold sm:text-4xl">
      {{ t('authExperience.confirm.title') }}
    </h2>
    <p class="mt-3 text-sm leading-6 text-[var(--ks-text-secondary)]">
      {{ t('authExperience.confirm.description') }}
    </p>

    <div class="mt-7 grid gap-4">
      <AppButton
        v-if="props.methods.passkey && passkeySupported"
        type="button"
      ...
      >
        {{
          passkeyLoading
            ...
            : t('authExperience.confirm.usePasskey')
        }}
      </AppButton>
      ...
      <a
        v-if="props.methods.google && props.googleAuthEnabled"
        href="/auth/google/reauthenticate"
        class="ks-command-link justify-center"
        >{{ t('authExperience.confirm.useGoogle') }}</a
      >

      <form v-if="props.methods.password" class="space-y-4" @submit.prevent="submit">
        <label class="block text-sm font-semibold" for="confirm-password">{{
        ...
        <input
          ...
          type="password"
        />
        <p v-if="form.errors.password" class="text-sm text-[var(--ks-red)]" role="alert">
          {{ form.errors.password }}
        </p>
        <AppButton class="w-full" type="submit" :disabled="form.processing">{{
          t('authExperience.confirm.usePassword')
        }}</AppButton>
      </form>
    </div>
  </AuthLayout>
