  ...
  <AppLayout :user="{ name: props.user.name, email: props.user.email }">
    ...
    <section id="profile" class="ks-surface mt-5 scroll-mt-4 p-5 sm:p-6" aria-labelledby="profile-heading">
      ...
      <form class="mt-6 grid gap-5 md:grid-cols-2" @submit.prevent="updateProfile">
        <div>
          ...
          <p v-if="profileForm.errors.name" class="mt-1.5 text-sm text-[var(--ks-red)]" role="alert">
        ...
        <div>
          ...
          <p v-if="profileForm.errors.timezone" class="mt-1.5 text-sm text-[var(--ks-red)]" role="alert">
            {{ profileForm.errors.timezone }}
          </p>
        </div>
        <div class="md:col-span-2">
          <p class="text-sm font-semibold">{{ t('auth.login.email') }}</p>
          <p class="mt-2 break-all text-sm text-[var(--ks-text-secondary)]">{{ props.user.email }}</p>
        </div>
        <div class="md:col-span-2">
          <AppButton type="submit" :disabled="profileForm.processing">{{ t('accountExperience.account.saveProfile') }}</AppButton>
        </div>
      </form>
    </section>
    ...
    <section id="security" class="mt-5 grid scroll-mt-4 gap-5 xl:grid-cols-2" aria-label="Security">
      <div class="ks-surface-gold p-5 sm:p-6">
        ...
        <p v-if="props.user.providerEmail" class="mt-3 break-all text-sm text-[var(--ks-muted)]">
        ...
        <div class="mt-6 border-t border-[var(--ks-border)] pt-5">
          ...
          <p class="mt-2 break-all font-semibold">{{ props.user.email }}</p>
          <div v-if="props.user.pendingEmail" class="mt-4 rounded-[var(--ks-radius-md)] border border-amber-400/25 bg-amber-500/[.07] p-4">
            ...
            <p class="mt-1 break-all text-sm text-amber-100/80">{{ props.user.pendingEmail }}</p>
          ...
          <form v-if="props.user.passwordAuthentication" class="mt-5 grid gap-3" @submit.prevent="requestEmailChange">
      ...
      <div class="ks-surface p-5 sm:p-6">
        ...
        <p v-if="props.user.twoFactorEnabled" class="mt-2 text-xs text-[var(--ks-muted)]">
          {{ t('accountExperience.account.recoveryCodesRemaining') }}: {{ props.user.recoveryCodeCount }}
        </p>

        <AppButton v-if="!props.user.twoFactorEnabled && !props.twoFactorSetup" class="mt-6" type="button" @click="beginTwoFactor">
      ...
      <div v-if="props.user.passwordAuthentication" class="ks-surface p-5 sm:p-6 xl:col-span-2">
        ...
        <form class="mt-5 grid gap-4 lg:grid-cols-3" @submit.prevent="updatePassword">
          <div>
            ...
            <p v-if="passwordForm.errors.current_password" class="mt-1.5 text-sm text-[var(--ks-red)]" role="alert">{{ passwordForm.errors.current_password }}</p>
          </div>
          <div>
            ...
            <p v-if="passwordForm.errors.password" class="mt-1.5 text-sm text-[var(--ks-red)]" role="alert">{{ passwordForm.errors.password }}</p>
          </div>
          <div>
    ...
    <section id="sessions" class="mt-5 grid scroll-mt-4 gap-5 xl:grid-cols-2" aria-label="Sessions">
      <div class="ks-surface p-5 sm:p-6">
        <div class="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h2 class="ks-display mt-1 text-xl font-semibold">{{ t('accountExperience.account.activeSessions') }}</h2>
          </div>
          <AppButton v-if="props.sessions.some((session) => !session.current)" variant="danger" type="button" @click="revokeOtherSessions">
        ...
        <ul class="mt-5 grid gap-3">
          <li v-for="session in props.sessions" :key="session.id" class="rounded-[var(--ks-radius-md)] border border-[var(--ks-border)] bg-black/15 p-4">
            <div class="flex flex-wrap items-start justify-between gap-4">
              <div>
                <p class="font-semibold">{{ session.browser ?? t('accountExperience.account.unknownBrowser') }} · {{ session.platform ?? t('accountExperience.account.unknownPlatform') }}</p>
                <p class="mt-1 text-sm text-[var(--ks-muted)]">{{ session.device ?? t('accountExperience.account.unknownDevice') }}</p>
                <p class="mt-2 text-xs text-[var(--ks-muted)]">{{ t('accountExperience.account.lastSeen') }}: {{ formatDate(session.lastSeenAt) }}</p>
              </div>
              <div class="flex items-center gap-2">
                <span v-if="session.current" class="ks-status" data-tone="success">{{ t('accountExperience.account.currentSession') }}</span>
                <AppButton v-else variant="danger" type="button" @click="revokeSession(session.id)">{{ t('accountExperience.account.signOutSession') }}</AppButton>
              </div>
            </div>
          </li>
        </ul>
      ...
      <div class="ks-surface p-5 sm:p-6">
        ...
        <ul v-if="props.securityActivity.length" class="mt-5 grid gap-3">
          <li v-for="activity in props.securityActivity" :key="activity.id" class="rounded-[var(--ks-radius-md)] border border-[var(--ks-border)] bg-black/15 px-4 py-3">
