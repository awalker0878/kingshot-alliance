  ...
  <main class="mx-auto max-w-3xl space-y-8 p-6">
    <header class="space-y-2">
      ...
      <p class="text-sm text-slate-600">
        Deletion uses a seven-day cooling-off period. Active alliance ownership,
        platform-administrator access, and legal holds prevent processing until resolved. Processed
        accounts are anonymized rather than silently removing audit history.
      </p>
    ...
    <section v-if="request" aria-labelledby="request-heading" class="space-y-3 rounded border p-5">
      ...
      <dl class="grid gap-3 sm:grid-cols-2">
        <div>
          ...
          <dd class="font-semibold">{{ request.status }}</dd>
        </div>
        <div>
          <dt class="text-sm text-slate-500">Eligible at</dt>
          <dd>{{ request.eligibleAt }}</dd>
        </div>
        <div>
          <dt class="text-sm text-slate-500">Requested</dt>
          <dd>{{ request.requestedAt }}</dd>
        </div>
        <div>
          <dt class="text-sm text-slate-500">Processed</dt>
          <dd>{{ request.processedAt || 'Not yet' }}</dd>
        </div>
      </dl>
      <p v-if="request.blockedReason" class="rounded border border-amber-400 p-3 text-sm">
    ...
    <section v-else class="space-y-4 rounded border border-red-400 p-5">
      ...
      <p class="text-sm text-slate-600">
        Transfer ownership of any alliance you own first. The request will not erase records subject
        to legal hold or records that must remain pseudonymized for security and audit integrity.
      </p>
      <button
        type="button"
        class="rounded border border-red-500 px-4 py-2 font-semibold text-red-700"
