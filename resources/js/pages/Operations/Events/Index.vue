...
<template>
  ...
  <AppLayout :user="props.user">
    <RoomBanner eyebrow="Alliance Operations" title="Event Command" subtitle="Prepare for the real Kingshot Event cycle, answer registrations and polls, and open each Event’s command rooms when its capabilities are available." image="/images/kingshot/bear-hunt.svg">
      <template #actions><Link v-if="props.canCreate" href="/events/create" class="ks-command-link">Call an Event</Link></template>
    </RoomBanner>
    <section class="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><StatSeal label="Events" :value="props.events.length" icon="⚔"/><StatSeal label="Needs Attention" :value="props.attention.length" icon="!" tone="teal"/><StatSeal label="Reminders" :value="props.reminders.length" icon="⌛" tone="stone"/><StatSeal label="View" :value="view === 'agenda' ? 'War Clock' : 'Calendar'" icon="◷"/></section>
