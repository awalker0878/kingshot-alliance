...
const canSubmit = computed(
  () => !busy.value && question.value.trim().length >= 2 && question.value.length <= props.maxQuestionLength,
);
...
<template>
  ...
  <AppLayout :user="props.user" :player-alliance-name="props.alliance.name" :has-player-alliance="true">
    <main id="main-content" class="mx-auto w-full max-w-5xl px-4 py-6 sm:px-6 lg:py-10">
      <section class="overflow-hidden rounded-[var(--ks-radius-lg)] border border-[var(--ks-border)] bg-[var(--ks-panel)] shadow-2xl">
        <div class="border-b border-[var(--ks-border)] bg-[linear-gradient(135deg,rgba(18,60,56,.92),rgba(6,18,18,.96))] px-5 py-6 sm:px-8 sm:py-8">
          ...
          <h1 class="mt-2 text-3xl font-[var(--ks-font-display)] font-semibold text-[var(--ks-gold-bright)] sm:text-4xl">
            {{ t('assistant.title') }}
          </h1>
        ...
        <div class="space-y-6 p-4 sm:p-6 lg:p-8">
          ...
          <ol v-else class="space-y-5" :aria-label="t('assistant.conversation')">
            <li v-for="turn in turns" :key="turn.id" class="space-y-3">
              <div class="ms-auto max-w-3xl rounded-[var(--ks-radius-md)] border border-[var(--ks-border)] bg-[rgba(226,180,77,.08)] px-4 py-3">
                ...
                <p class="mt-1 whitespace-pre-wrap text-sm text-[var(--ks-text)]">{{ turn.question }}</p>
              </div>
              ...
              <article class="max-w-4xl rounded-[var(--ks-radius-md)] border border-[var(--ks-border)] bg-[var(--ks-surface)] p-4 sm:p-5">
                ...
                <p class="mt-3 whitespace-pre-wrap text-base leading-7 text-[var(--ks-text)]">
                ...
                <div v-if="turn.response.ambiguity?.length" class="mt-4 rounded-[var(--ks-radius-sm)] border border-[var(--ks-border)] bg-[var(--ks-panel)] p-3">
                  <h3 class="text-sm font-bold text-[var(--ks-text)]">{{ t('assistant.possibleEvents') }}</h3>
                  <ul class="mt-2 space-y-2">
                    <li v-for="candidate in turn.response.ambiguity" :key="candidate.occurrenceId" class="flex flex-wrap items-center justify-between gap-2 text-sm">
                      <span>{{ candidate.title }} · {{ formatDate(candidate.startsAt, { dateStyle: 'medium', timeStyle: 'short' }) }}</span>
                      <Link :href="`/events/${candidate.occurrenceId}`" class="font-bold text-[var(--ks-gold-bright)] underline underline-offset-4">
                ...
                <section v-if="turn.response.citations.length" class="mt-5 border-t border-[var(--ks-border)] pt-4" :aria-label="t('assistant.sourcesHeading')">
                  ...
                  <ul class="mt-3 grid gap-2 sm:grid-cols-2">
                    <li v-for="citation in turn.response.citations" :key="citation.evidenceId" class="rounded-[var(--ks-radius-sm)] border border-[var(--ks-border)] bg-[var(--ks-panel)] p-3">
          ...
          <p v-if="requestError" role="alert" class="rounded-[var(--ks-radius-sm)] border border-red-400/40 bg-red-950/30 px-4 py-3 text-sm font-semibold text-red-100">
            {{ requestError }}
          </p>
          ...
          <form class="sticky bottom-3 rounded-[var(--ks-radius-md)] border border-[var(--ks-border)] bg-[rgba(5,14,14,.96)] p-3 shadow-2xl backdrop-blur" @submit.prevent="submit">
            ...
            <div class="mt-2 flex flex-wrap items-center justify-between gap-3">
              ...
              <button
                type="submit"
                :disabled="!canSubmit"
                class="min-h-11 rounded-[var(--ks-radius-sm)] bg-[var(--ks-gold)] px-5 py-2.5 text-sm font-extrabold text-[#181108] disabled:cursor-not-allowed disabled:opacity-40 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--ks-gold-bright)]"
              >
