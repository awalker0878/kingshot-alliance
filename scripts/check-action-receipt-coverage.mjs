...
function receiptArguments(source, filename) {
  ...
  while ((cursor = source.indexOf('receipt(', cursor)) !== -1) {
    let index = cursor + 'receipt('.length; let depth = 1; let quote = null; let escaped = false;
    for (; index < source.length && depth > 0; index += 1) { const character = source[index]; if (quote !== null) { if (escaped) escaped = false; else if (character === '\\') escaped = true; else if (character === quote) quote = null; continue; } if (character === "'" || character === '"') quote = character; else if (character === '(') depth += 1; else if (character === ')') depth -= 1; }
    if (depth !== 0) throw new Error(`Unclosed receipt call in ${filename}`);
    argumentsList.push(source.slice(cursor + 'receipt('.length, index - 1)); cursor = index;
  }
...
const stale = [...translatedCodes].filter((code) => code !== 'completed' && !usedCodes.has(code)).sort();
if (missing.length > 0 || stale.length > 0) { if (missing.length > 0) console.error(`Missing receipt messages: ${missing.join(', ')}`); if (stale.length > 0) console.error(`Unused receipt messages: ${stale.join(', ')}`); process.exit(1); }
