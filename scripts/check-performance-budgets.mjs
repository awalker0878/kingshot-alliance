...
const appEntrySource = path.resolve('resources/js/app.ts');
const appEntryBytes = (await stat(appEntrySource)).size;
...
const measuredPages = await Promise.all(
  pageAssets.map(async (entry) => ({
    source: entry.src,
    file: entry.file,
    size: await bytes(entry.file),
  })),
);
...
const measurements = [
  [
    ...
    budgets.initialJavaScriptBytes,
    [...initialFiles].join(', '),
  ],
  ['application entry source', appEntryBytes, budgets.appEntryBytes, 'resources/js/app.ts'],
  [
    'largest page chunk',
    largestPage?.size ?? 0,
    budgets.largestPageChunkBytes,
    largestPage?.source ?? 'none',
  ],
  [
    'largest stylesheet',
    largestStylesheet?.size ?? 0,
    budgets.largestStylesheetBytes,
    largestStylesheet?.file ?? 'none',
  ],
];
...
for (const [label, actual, budget, detail] of measurements) {
  ...
  console.log(
    `[${state}] ${label}: ${Math.round(actual / 1024)} KiB / ${Math.round(budget / 1024)} KiB (${detail})`,
  );
}

if (failures.length > 0) {
  throw new Error(
    'Production asset performance budget exceeded. Split or remove the responsible code before raising a reviewed budget.',
  );
}
